...
if require_tool govulncheck "govulncheck"; then
  run_check "govulncheck" "${OUT_DIR}/govulncheck.txt" \
    env GOCACHE="${GO_CACHE}" govulncheck -C "${ROOT_DIR}/backend" ./... || true
fi
...
if require_tool hadolint "hadolint"; then
  ...
  while IFS= read -r -d '' f; do
    dockerfiles+=("$f")
  done < <(find "${ROOT_DIR}" -name 'Dockerfile*' -not -path '*/vendor/*' -print0 2>/dev/null)

...
DOCKERFILE="${DOCKERFILE:-${ROOT_DIR}/deploy/docker/Dockerfile.controller}"
...
IMAGE="${SRE_CONTROLLER_IMAGE:-ai-sre-agent/controller:${VERSION}}"
...
usage() {
  cat <<USAGE
usage: ./scripts/docker-build-controller.sh [--no-cache]

Build the controller image.

Environment:
  DOCKERFILE            Path to Dockerfile (default: deploy/docker/Dockerfile.controller)
  SRE_CONTROLLER_IMAGE  Image tag (default: ai-sre-agent/controller:\${SRE_VERSION})
...
run_build() {
  ...
  echo "standard docker build failed; retrying controller with --network host" >&2
...
echo "Building ${IMAGE}"
run_build
echo "controller image build complete"

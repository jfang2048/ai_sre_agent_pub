...
async function captureBreakdownBestEffort(cdp, label, fileName) {
  try {
    await captureBreakdown(cdp, label, fileName);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.warn(`Skipping ${label} breakdown capture: ${message}`);
  }
}

async function runBestEffort(label, callback) {
  try {
    await callback();
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.warn(`Skipping ${label}: ${message}`);
  }
}
...
async function main() {
  ...
  try {
    ...
    await captureBreakdownBestEffort(cdp, 'CPU', 'cpu-trends.png');
    await captureBreakdownBestEffort(cdp, 'Memory', 'memory-trends.png');
    await captureBreakdownBestEffort(cdp, 'Network', 'network-trends.png');
    await captureBreakdownBestEffort(cdp, 'Disk I/O', 'disk-trends.png');

    await setViewport(cdp, 1920, 1400);
    await runBestEffort('GPU Observability screenshot', async () => {
    ...
    await runBestEffort('Security Dashboard screenshot', async () => {
    ...
    await navigateToPage(cdp, 'Risk Insights');
    await waitForCondition(
      cdp,
      'risk insights ready',
      `(() => {
        ...
        return text.includes('Ranked Potential Risks')
          ...
          && Array.from(root.querySelectorAll('section'))
            .some((section) => (section.textContent ?? '').includes('Ranked Potential Risks') && section.querySelectorAll('button').length > 0);
      })()`,
      LIVE_WAIT_MS,
    );
    await stabilizeView();
    await capture(cdp, 'risk-insights.png');
    ...
    await runBestEffort('Logs screenshot', async () => {

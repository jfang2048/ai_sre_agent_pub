...
test.describe('Knowledge Page', () => {
  test('queries the local knowledge base and renders source-linked hits without runtime or request failures', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Retrieved Operational Knowledge' })).toBeVisible();
    ...
    await expect.poll(async () =>
      page.locator('[data-testid="knowledge-page"] section:has-text("Retrieved Operational Knowledge") button').count(),
    ).toBeGreaterThan(0);
    await expect(page.locator('[data-testid="knowledge-page"] section:has-text("Retrieved Operational Knowledge")')).toContainText(/score=/i);
    await expect(page.locator('[data-testid="knowledge-page"]')).toContainText(/runbook|historical_incident|question_pattern/i);

...
test.describe('Risk, RCA, and Security Pages', () => {
  test('render risk, rca, security, incidents, audit, and logs drilldowns without runtime/network failures', async ({ page }) => {
    ...
    await expect.poll(async () =>
      page.locator('[data-testid="risk-insights-page"] section:has-text("Ranked Potential Risks") button').count(),
    ).toBeGreaterThan(0);
    ...
    await expect.poll(async () =>
      page.locator('[data-testid="joint-risk-page"] section:has-text("Ranked Signals") article').count(),
    ).toBeGreaterThan(0);
    ...
    await expect.poll(async () =>
      page.locator('[data-testid="rca-page"] section:has-text("Ranked Hypotheses") article').count(),
    ).toBeGreaterThan(0);
    ...
    await expect.poll(async () =>
      page.locator('[data-testid="security-dashboard-page"] section:has-text("Security Findings") button').count(),
    ).toBeGreaterThan(0);

...
如果你想接真实 provider，而不是 stub，可以导出 `SRE_AGENT_LLM_API_KEY`，并把 `SRE_AGENT_LLM_PROVIDER` 设成 `gemini` 或 `google`。仓库里的 Google 路径现在默认走当前可用的 `gemini-flash-latest` 别名。

如果你想做一次比“provider 能响应”更像真实检查的 live 验证，可以直接跑带 explanation judge 的 anomaly eval：
...
这条路径仍然使用仓库里确定性的 anomaly case。LLM 只负责判断输出 explanation 是否真的符合 case contract。
...
- 更完整的运维手册见 [../en/21-usage.md](../en/21-usage.md)

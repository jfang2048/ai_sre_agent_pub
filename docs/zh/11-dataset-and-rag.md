...
本页说明 `v0.7` 里仓库本地知识是如何真正进入系统的：仓库自带了什么数据、这些数据如何被解析和索引、检索结果长什么样，以及它们如何影响最终 agent prompt。
...
| --- | --- | --- |
| Prometheus operator runbooks | [`dataset/raw/prometheus/prometheus-operator-runbooks/content/runbooks/`](../../dataset/raw/prometheus/prometheus-operator-runbooks/content/runbooks/) 下 118 个 runbook | 作为高价值运维 runbook，按 `prometheus`、`kubernetes`、`linux_node` 域进行标注和加权 |
| Scoutflo Kubernetes playbooks | [`dataset/raw/runbooks/scoutflo-sre-playbooks/K8s Playbooks/`](../../dataset/raw/runbooks/scoutflo-sre-playbooks/K8s%20Playbooks/) 下 245 个文件 | 作为高价值 Kubernetes / 节点运维 playbook |
| Scoutflo AWS playbooks | [`dataset/raw/runbooks/scoutflo-sre-playbooks/AWS Playbooks/`](../../dataset/raw/runbooks/scoutflo-sre-playbooks/AWS%20Playbooks/) 下 159 个文件 | 保留，但只有在 query 明显像 AWS 事件时才更有机会排到前面 |
| Scoutflo Sentry playbooks | [`dataset/raw/runbooks/scoutflo-sre-playbooks/Sentry Playbooks/`](../../dataset/raw/runbooks/scoutflo-sre-playbooks/Sentry%20Playbooks/) 下 27 个文件 | 保留，主要在 Sentry/告警平台相关事件中使用 |
| 处理后的 GPU 文档 | [`dataset/processed/gpu-docs/`](../../dataset/processed/gpu-docs/) 下 2 个 Markdown 文件 | 作为高价值 GPU 故障文档，并优先于原始 HTML |
| 原始 GPU HTML | [`dataset/raw/gpu-docs/`](../../dataset/raw/gpu-docs/) | 保留来源链路，但当存在处理后的 Markdown 时默认跳过 |
...
- 当 [`dataset/processed/gpu-docs/`](../../dataset/processed/gpu-docs/) 已经有处理后 Markdown 时，对应的原始 GPU HTML

...
本页用真实代码路径和可追踪示例解释 `v0.8` 的数据流。目标不是只讲概念，而是把下面这些问题讲清楚：
...
| 控制面事件化 | [`backend/internal/controller/agentcore/workflow_eventization.go`](../../backend/internal/controller/agentcore/workflow_eventization.go), [`backend/internal/controller/agentcore/workflow_engine.go`](../../backend/internal/controller/agentcore/workflow_engine.go), [`backend/internal/controller/agentcore/behavioral_memory.go`](../../backend/internal/controller/agentcore/behavioral_memory.go) | `NodeSnapshot`、history、risk series、baseline sample、workload 身份 | `TrendAssessment[]`、`BehavioralSignalAssessment[]`、`InvestigationEvent[]`、`RetrievalDecision[]` | 在更深层分析前，把原始状态压缩成趋势、workload 行为记忆、弱信号和检索规划对象 |
...
用来抑制 recurring burst 误报的行为记忆层，位于 controller workflow 路径内，不在 collector，也不在通用 ingest store 中。

这样放置是刻意的：

- collector 的职责是保住证据，而不是判断某次 burst 是否“历史上通常没问题”
- ingest 的职责是维护当前事实和有界近历史，而不是拥有服务特定的分类策略
- workflow 已经拥有 logs、runtime、安全、topology 和 change 上下文，足以判断这次 burst 是孤立资源波动、历史上常见的健康尖峰，还是被其他信号 corroborate 的 incident

controller 现在在生成 `RiskSeries` 之后会做这几步：

1. 从 labels 和 top-process 上下文推导 workload 身份
2. 通过现有的 metric-history 路径查询同一 collector 或 workload 的长窗口历史
3. 把当前 burst 与 recent baseline、long-window baseline 和简单的小时级 recurrence 做比较
4. 把每个活跃信号分类成 `expected_recurring_burst`、`suspicious_deviation`、`correlated_anomaly` 或 `confirmed_anomaly`
5. 把这个分类结果反馈到 risk scoring、trend summary 和 evidence output

这里没有第二套历史数据库。workflow 直接复用趋势分析已经依赖的 metric history 与可选 TSDB 查询，只在内存里保留一个很小的短期 cache，避免在短时间内重复查同一段历史。
...
[`convertProbeCoreBatch`](../../backend/internal/collector/probe_core_convert.go) 会生成后续系统使用的 alias 指标。到 `v0.8` 为止，如果某个 `probe_core_*` 主机/资源指标已经有等价 alias，collector 默认不再把 raw 指标也重复放进每个 batch。
...
- `BehavioralSignalAssessment[]`：针对单个活跃信号说明它更像历史上健康的 recurring burst，还是更像需要升级处理的新异常
...
```json
{
  ...
  "behavioral_assessment": {
    ...
    "reason": "同一服务在相似 hour-of-week 已多次出现短时 CPU burst，且此前没有错误率、日志异常或延迟回归旁证"
  },
  "investigation_event": {
...
- 相同状态反复出现时，现在可以在 event/trend/behavior-memory 层比较，而不是每次都重建全部上下文

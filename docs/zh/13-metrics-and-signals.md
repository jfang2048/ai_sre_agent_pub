...
最完整的指标参考还是 [docs/reference/metrics.md](../reference/metrics.md)。下面这张表只聚焦真正影响 reasoning 的信号家族。
...
这是 `v0.7` 一个很关键的低开销变化：collector 可以保持主遥测 cadence，但不必让每个昂贵 helper 在每轮都重跑。
...
在这之上，`v0.7` 还多了一层 steady-state 进程 payload 缩减：

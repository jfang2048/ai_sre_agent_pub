...
v0.8 emits explicit source labels and collector runtime markers so operators can verify primary-vs-fallback behavior.
...
In `v0.8`, the collector does not duplicate most raw `probe_core_*` host/resource metrics into outbound batches when equivalent `node_*` or `rca_*` aliases are already emitted by [`backend/internal/collector/probe_core_convert.go`](../../backend/internal/collector/probe_core_convert.go).

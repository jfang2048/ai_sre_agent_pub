...
This page traces the real `v0.7` data path with concrete examples. The goal is not only to name the components, but to show:
...
| Control-plane eventization | [`backend/internal/controller/agentcore/workflow_eventization.go`](../../backend/internal/controller/agentcore/workflow_eventization.go), [`backend/internal/controller/agentcore/workflow_engine.go`](../../backend/internal/controller/agentcore/workflow_engine.go) | `NodeSnapshot`, history, risk series, baseline samples | `TrendAssessment[]`, `InvestigationEvent[]`, `RetrievalDecision[]` | compress raw state into trend, weak-signal, and retrieval-planning artifacts before deeper analysis |
...
[`convertProbeCoreBatch`](../../backend/internal/collector/probe_core_convert.go) emits the aliased node metrics that the rest of the system expects. In `v0.7`, it no longer duplicates most raw `probe_core_*` host/resource metrics by default when those aliases already exist.

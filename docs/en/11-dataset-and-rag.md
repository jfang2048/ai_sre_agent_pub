...
This page explains the repository-local knowledge path as it actually exists in `v0.7`: what data ships in the repo, how it is parsed and indexed, what a retrieval hit looks like, and how that hit influences the final agent prompt.
...
| --- | --- | --- |
| Prometheus operator runbooks | 118 files under [`dataset/raw/prometheus/prometheus-operator-runbooks/content/runbooks/`](../../dataset/raw/prometheus/prometheus-operator-runbooks/content/runbooks/) | high-value operational runbooks, tagged and boosted for `prometheus`, `kubernetes`, or `linux_node` incidents |
| Scoutflo Kubernetes playbooks | 245 files under [`dataset/raw/runbooks/scoutflo-sre-playbooks/K8s Playbooks/`](../../dataset/raw/runbooks/scoutflo-sre-playbooks/K8s%20Playbooks/) | high-value operational playbooks, boosted for Kubernetes and node-oriented incidents |
| Scoutflo AWS playbooks | 159 files under [`dataset/raw/runbooks/scoutflo-sre-playbooks/AWS Playbooks/`](../../dataset/raw/runbooks/scoutflo-sre-playbooks/AWS%20Playbooks/) | kept, but penalized unless the query clearly looks AWS-specific |
| Scoutflo Sentry playbooks | 27 files under [`dataset/raw/runbooks/scoutflo-sre-playbooks/Sentry Playbooks/`](../../dataset/raw/runbooks/scoutflo-sre-playbooks/Sentry%20Playbooks/) | kept, but routed mainly for Sentry-style issue queries |
| Processed GPU docs | 2 files under [`dataset/processed/gpu-docs/`](../../dataset/processed/gpu-docs/) | high-value GPU troubleshooting docs and preferred over raw HTML duplicates |
| Raw GPU HTML docs | [`dataset/raw/gpu-docs/`](../../dataset/raw/gpu-docs/) | used for lineage, but skipped when a processed Markdown counterpart exists |
...
- `dataset/raw/prometheus/prometheus-operator-runbooks/content/runbooks/node/...`
  - `source_family=prometheus_operator_runbook`
  - `source_domain=linux_node`
  - `operational_value=high`
- `dataset/raw/runbooks/scoutflo-sre-playbooks/K8s Playbooks/02-Nodes/...`
  - `source_family=scoutflo_k8s_playbook`
  - `source_domain=linux_node`
  - `operational_value=high`
- `dataset/processed/gpu-docs/gpu-operator-troubleshooting.md`

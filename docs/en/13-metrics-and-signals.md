...
The canonical deep reference is [docs/25-metrics-reference.md](25-metrics-reference.md). The table below focuses on the metric families that materially affect reasoning.
...
This is one of the most important low-overhead changes in `v0.8`: the collector can keep its main telemetry cadence without re-running every expensive helper on every loop.

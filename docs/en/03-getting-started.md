...
If you want to exercise a real provider instead of the stub, export `SRE_AGENT_LLM_API_KEY` and set `SRE_AGENT_LLM_PROVIDER=gemini` (or `google`). The built-in Google path now defaults to the current `gemini-flash-latest` alias.

If you want one quick live check that goes beyond "the provider responds", run the anomaly eval with the optional explanation judge:

```bash
cd backend
SRE_AGENT_LLM_API_KEY=... \
SRE_AGENT_LLM_PROVIDER=gemini \
go run ./cmd/evalctl -scope fast -judge-llm -judge-limit 2
```

That path still uses the repository's deterministic anomaly cases. The LLM only grades whether the emitted explanation matches the expected contract.
...
- For a fuller operator runbook, use [21-usage.md](21-usage.md).

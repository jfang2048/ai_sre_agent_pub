...
- 这里强调 `primary`，是为了防止把“系统支持 fallback”误读成“主路径和兼容路径地位相同”。实际上 v0.7 明确偏向 probe-core 和 eBPF。
...
`collection_interval` is the baseline cadence for the main host telemetry push loop. In `v0.7` the default is now `5s`, bounded by:

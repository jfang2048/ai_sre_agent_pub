...
### 6. Optional C++ probe-core path

- [ ] `SRE_COLLECTOR_PROBE_CORE_ENABLED=1` is set on collector nodes that should use probe-core.
- [ ] Probe-core binary exists and is executable (`SRE_COLLECTOR_PROBE_CORE_BINARY_PATH`).
- [ ] `collector_probe_source{source="probe_core"}` is present in `/metrics`.
- [ ] `collector_probe_core_fresh=1` and `collector_probe_core_last_frame_age_seconds` stays below stale threshold.
- [ ] Fallback behavior validated (`SRE_COLLECTOR_PROBE_CORE_FALLBACK_TO_GO`) under probe-core crash/timeout.

### 7. Optional analysis/agent/incidents
...
- [ ] `POST /api/v1/agent/query` returns `explainability` and `actions_expire_at`.
- [ ] Non-dry-run `POST /api/v1/agent/execute` enforces `approval_token` when approval policy is enabled.
- [ ] Query saturation behavior validated (`503` on overload with bounded `SRE_AGENT_MAX_CONCURRENT_QUERIES`).
- [ ] Stale telemetry behavior validated (query reports `telemetry_stale` and suppresses actions by default).
- [ ] Deterministic degraded behavior validated (`SRE_AGENT_SKIP_LLM_ON_STALE_TELEMETRY=1`, `SRE_AGENT_SKIP_LLM_ON_NO_TELEMETRY=1`).
- [ ] Optional webhook integration validated (`SRE_AGENT_EVENT_WEBHOOK_URL`) for query/execute event publishing.
- [ ] Optional native sink adapters validated (`SRE_AGENT_EVENT_SLACK_WEBHOOK_URL`, `SRE_AGENT_EVENT_PAGERDUTY_ROUTING_KEY`) for query/execute event publishing.
- [ ] Event publish retry policy validated (`SRE_AGENT_EVENT_PUBLISH_RETRIES`, `SRE_AGENT_EVENT_RETRY_BACKOFF`).
...
- [ ] AGENT safety metrics are present (`agent_fallback_total`, `agent_action_approval_required_total`, `agent_pending_actions_expired_total`).
- [ ] AGENT data-quality metrics are present (`agent_queries_stale_telemetry_total`).
- [ ] AGENT LLM bypass metrics are present (`agent_llm_bypassed_stale_total`, `agent_llm_bypassed_empty_total`).
- [ ] Action suppression metric is present (`agent_actions_suppressed_total`) and aligns with query responses.
- [ ] AGENT event publish metrics are present (`agent_events_published_total`, `agent_events_publish_fail_total`).

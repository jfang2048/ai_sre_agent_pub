...
class AIServicer:
    ...
    def __init__(
        self, classifier: IssueClassifier, explainer: ExplanationGenerator, llm_client=None
    ):
    ...
    def ClassifyIssue(self, request, context) -> Dict[str, Any]:
        ...
        # Run Pipeline
        raw_metrics = [
            {"name": m.name, "value": m.value, "timestamp": m.timestamp, "labels": m.labels}
            for m in metrics
        ]
        raw_logs = [
            {
        ...
        pipeline_results = self.pipeline.run(
            request.get("node_name", "unknown"), raw_metrics, raw_logs
        )
        ...
        for result in pipeline_results:
            ...
            if "memory" in result.issue_type:
                cat = IssueCategory.MEMORY_PRESSURE
            elif "error" in result.issue_type:
                cat = IssueCategory.APPLICATION_ERROR
    ...
    def SuggestFix(self, request, context) -> Dict[str, Any]:
        ...
        return {"suggestions": suggestions, "analysis_id": request.get("analysis_id", "")}
    ...
    def _generate_suggestions(
        self, category: IssueCategory, severity: Severity, request: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
...
def serve(port: int = 50052, model_path: Optional[str] = None):
    ...
    # v0.5 runs the Python service as HTTP/JSON-RPC for minimal dependencies.

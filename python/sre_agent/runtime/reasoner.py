...
class Reasoner:
    ...
    def _deterministic_reasoning(
        ...
        return ReasoningOutput(
            ...
            debug={
                ...
                "tool_status": {trace.tool_name or trace.step_id: trace.status for trace in traces},
            },
        )
    ...
    def _merge_with_candidate(
        ...
        confidence = _safe_float(
            candidate.get("confidence", baseline.confidence), baseline.confidence
        )
        confidence = max(0.0, min(1.0, confidence))
...
def _remediation_for(issue_type: str) -> str:
    ...
    if issue_type == "cpu_saturation":
        return (
            "Identify top CPU processes, cap runaway jobs, and scale replicas if pressure persists."
        )
    ...
    return (
        "Correlate additional telemetry and run targeted diagnostics before automated remediation."
    )

...
class ContextStore:
    ...
    def _search_lexical(self, query: str, limit: int) -> List[ContextSnippet]:
        ...
        for chunk in chunks:
            ...
            if score > 0:
                scored.append(ContextSnippet(path=chunk.path, content=chunk.content, score=float(score)))
    ...
    def _read_chunks(self, path: str) -> List[str]:
        try:
            ...
            emit(self._logger, "context_file_read_failed", level="warning", path=path, error=str(exc))
            return []
...
def _haystack_available() -> bool:
    return Document is not None and InMemoryDocumentStore is not None and InMemoryBM25Retriever is not None

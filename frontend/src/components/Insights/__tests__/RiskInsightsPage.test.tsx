...
describe('RiskInsightsPage', () => {
    ...
    it('renders ranked risks, evidence, trends, and correlation details', async () => {
        ...
        expect(await screen.findByText('Weak-Signal and Event Summary')).toBeInTheDocument();
        expect(await screen.findByText('Single-Metric Trend Watch')).toBeInTheDocument();
        expect(await screen.findByText('Knowledge Retrieval Decisions')).toBeInTheDocument();
    ...
    it('shows a loading state instead of an empty-state placeholder while fresh findings are still generating', async () => {
        ...
        expect((await screen.findAllByText(/joint risk medium/i)).length).toBeGreaterThan(0);
    });
});

...
import { Bot, BrainCircuit, Database, DollarSign, HardDrive, ShieldCheck } from 'lucide-react';
import {
    fetchAgentStatus,
...
export default function OperationsControlPanel() {
    ...
    const controllerStatusQuery = useQuery({
    ...
    const queryService = agentQuery.data?.query_service;
    const controlPlane = agentQuery.data?.control_plane;
    const reportEngine = agentQuery.data?.report_engine;
    const collectorCoverage = controllerStatusQuery.data?.collector_coverage;
    const degradedContinuity = [
        collectorCoverage && collectorCoverage.state !== 'fresh'
            ? 'Collector coverage is degraded; some nodes are stale, partial, or replaying backlog.'
            : '',
        tsdb?.enabled && (!tsdb.healthy || tsdb.fallback_active) ? 'TSDB is degraded; long-window history is running in memory fallback.' : '',
        rag?.enabled && (!rag.ready || !!rag.last_error) ? 'RAG retrieval is degraded; agent reasoning will continue without static knowledge context.' : '',
        queryService && queryService.analysis_mode !== 'llm_assisted'
            ? 'AGENT query service is running in deterministic-only mode.'
            : '',
    ].filter(Boolean);
    ...
    return (
        <div className="h-full w-full p-3 bg-card/50 flex flex-col gap-3" data-testid="operations-control-panel">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-3 flex-1 min-h-0">
                <section className="rounded-lg border border-border bg-background/40 p-3 space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-2 text-xs">
                        <ContinuityCard
                            title="Coverage"
                            state={!collectorCoverage ? 'unavailable' : collectorCoverage.state === 'fresh' ? 'healthy' : 'degraded'}
                            detail={!collectorCoverage
                                ? 'status unavailable'
                                : `${collectorCoverage.fresh_collectors}/${collectorCoverage.total_collectors} fresh · ${Math.round(collectorCoverage.coverage_percent ?? 0)}% coverage`}
                            subdetail={!collectorCoverage
                                ? undefined
                                : `stale ${formatCount(collectorCoverage.stale_collectors)} · partial ${formatCount(collectorCoverage.partial_collectors)} · backlog ${formatCount(collectorCoverage.backlog_collectors)}`}
                            icon={<ShieldCheck className="w-3.5 h-3.5" />}
                        />
                        <ContinuityCard
                            title="TSDB"
                            state={!tsdb?.enabled ? 'disabled' : tsdb.healthy && !tsdb.fallback_active ? 'healthy' : 'degraded'}
                            detail={!tsdb?.enabled ? 'disabled' : tsdb.fallback_active ? 'memory fallback active' : tsdb.mode}
                            subdetail={tsdb?.degraded_reason || tsdb?.last_health_error || undefined}
                            icon={<Database className="w-3.5 h-3.5" />}
                        />
                        <ContinuityCard
                            title="RAG"
                            state={!rag?.enabled ? 'disabled' : rag.ready && !rag.last_error ? 'healthy' : 'degraded'}
                            detail={!rag?.enabled ? 'disabled' : rag.ready ? rag.retrieval_mode : 'not ready'}
                            subdetail={rag?.last_error || undefined}
                            icon={<BrainCircuit className="w-3.5 h-3.5" />}
                        />
                        <ContinuityCard
                            title="AGENT"
                            state={!queryService ? 'unavailable' : queryService.analysis_mode === 'llm_assisted' ? 'healthy' : 'degraded'}
                            detail={!queryService ? 'status unavailable' : queryService.analysis_mode === 'llm_assisted' ? `${queryService.provider} / ${queryService.model}` : 'deterministic-only'}
                            subdetail={!queryService
                                ? undefined
                                : `fallbacks ${formatCount(queryService.metrics.FallbackTotal)} · stale bypass ${formatCount(queryService.metrics.LLMBypassedStaleTotal)} · trends ${formatCount(controlPlane?.triggered_trends ?? 0)} · events ${formatCount(controlPlane?.investigation_events ?? 0)}${reportEngine ? ` · reports suppressed ${formatCount(reportEngine.report_suppressed_total)}` : ''}`}
                            icon={<Bot className="w-3.5 h-3.5" />}
                        />
                    </div>
                    {degradedContinuity.length > 0 && (
                        <div className="rounded border border-amber-400/40 bg-amber-500/10 px-2 py-2 text-[11px] text-amber-100 space-y-1">
                            {degradedContinuity.map((message) => (
                                <div key={message}>{message}</div>
                            ))}
                            {collectorCoverage?.quality_hint && <div>{collectorCoverage.quality_hint}</div>}
                        </div>
                    )}
            ...
            <div className="text-[11px] text-muted-foreground flex items-center gap-1">
                ...
                Standby mode keeps mutation APIs read-only; retention updates require active controller. Degraded continuity cards show when historical depth, knowledge retrieval, or LLM-assisted reasoning have fallen back.
            </div>
        </div>
    );
}
...
function ContinuityCard({
    ...
    return (
        <div className={`rounded border bg-background/60 px-2 py-2 space-y-1 ${stateClass}`}>
            <div className="flex items-center justify-between gap-2">
            ...
            {subdetail && <div className="text-[10px] opacity-80">{subdetail}</div>}
        </div>
    );
}

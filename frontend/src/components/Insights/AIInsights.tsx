...
const AIInsightsPanel = () => {
    ...
    const { data: incidentData, isLoading: incidentsLoading, isError: incidentsError } = useQuery<AgentIncidentResponse>({
        queryKey: ['agent-incidents-panel'],
        queryFn: async () => (await api.get('/agent/incidents?limit=6')).data,
        ...
        <div className="bg-card text-card-foreground rounded-lg shadow-md h-full flex flex-col border border-border">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <div className="space-y-3 pt-2">
                    ...
                    {incidentsError && (
                        <div className="text-sm text-red-400">Incident diagnosis stream unavailable.</div>
                    )}
                    {incidentsLoading && (
                        <div className="text-sm text-muted-foreground">Fetching incident diagnoses…</div>
                    )}
                    {!incidentsLoading && incidents.length === 0 && !incidentsError && (
                        <div className="text-sm text-muted-foreground">No incident diagnoses yet.</div>
                    )}
                    {incidents.map((incident) => (
                        <div key={incident.alert_id} className="p-3 rounded-md bg-muted/20 border border-border">
                            ...
                            <div className="text-xs text-muted-foreground mt-1">
                                {incident.service} • confidence {(incident.confidence * 100).toFixed(0)}%
                            </div>
                            {incident.diagnosis?.probable_root_cause && (
                                <div className="text-xs text-muted-foreground mt-1">
                                    {incident.diagnosis.probable_root_cause}
                                </div>
                            )}

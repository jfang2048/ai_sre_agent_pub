...
function firstInvestigationEvent(report: RCAReport) {
    return report.context?.investigation_events?.[0]?.title || '';
}

function firstProbableCause(report: RCAReport) {
    return report.context?.investigation_events?.[0]?.probable_cause || '';
}
...
const AIInsightsPanel = () => {
    const { data: agentStatusData, isLoading: statusLoading, isError: statusError } = useQuery({
    ...
    const controlPlane = agentStatusData?.control_plane;
    const retrievalState = controlPlane
        ? controlPlane.retrieval_decisions > 0
            ? `${controlPlane.retrieval_decisions} decisions · ${controlPlane.retrieval_skipped} skipped`
            : 'no recent retrieval planning'
        : 'status unavailable';
    const leadHeadline = compactText(
        controlPlane?.latest_incident_summary ||
            controlPlane?.top_event_title ||
            controlPlane?.probable_cause ||
            '',
        140,
    );
        ...
        <div className="bg-card text-card-foreground rounded-lg shadow-md h-full flex flex-col border border-border">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                <div className="space-y-3">
                    <div className="text-xs uppercase tracking-wider text-muted-foreground">Control-plane focus</div>
                    {statusError && (
                        <div className="text-sm text-red-400">
                            Control-plane summary unavailable. The workflow engine may be disabled or still warming up.
                        </div>
                    )}
                    {statusLoading && <div className="text-sm text-muted-foreground">Fetching control-plane summary…</div>}
                    {!statusLoading && !statusError && controlPlane && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div className="rounded-md border border-border bg-muted/20 p-3">
                                <div className="text-[11px] uppercase tracking-wide text-muted-foreground">Trend watch</div>
                                <div className="mt-1 text-lg font-semibold">{controlPlane.triggered_trends}</div>
                                <div className="text-xs text-muted-foreground">
                                    triggered trends · {controlPlane.high_risk_reports} high-risk reports
                                </div>
                            </div>
                            <div className="rounded-md border border-border bg-muted/20 p-3">
                                <div className="text-[11px] uppercase tracking-wide text-muted-foreground">Weak-signal fusion</div>
                                <div className="mt-1 text-lg font-semibold">{controlPlane.investigation_events}</div>
                                <div className="text-xs text-muted-foreground">
                                    {controlPlane.weak_signal_clusters} clusters · {controlPlane.incidents} workflow incidents
                                </div>
                            </div>
                            <div className="rounded-md border border-border bg-muted/20 p-3">
                                <div className="text-[11px] uppercase tracking-wide text-muted-foreground">Retrieval planning</div>
                                <div className="mt-1 text-sm font-medium text-foreground">{retrievalState}</div>
                                <div className="text-xs text-muted-foreground">
                                    {compactText(
                                        controlPlane.top_retrieval_skip_reason ||
                                            controlPlane.top_retrieval_intent ||
                                            controlPlane.top_retrieval_query,
                                        110,
                                    ) || 'No recent retrieval summary'}
                                </div>
                            </div>
                            <div className="rounded-md border border-border bg-muted/20 p-3">
                                <div className="text-[11px] uppercase tracking-wide text-muted-foreground">Lead issue</div>
                                <div className="mt-1 text-sm font-medium text-foreground">
                                    {leadHeadline || 'No recent control-plane verdict'}
                                </div>
                                <div className="text-xs text-muted-foreground">
                                    {(controlPlane.latest_collector_id || 'fleet')}{' '}
                                    {controlPlane.latest_rca_at || controlPlane.latest_joint_risk_at
                                        ? `• ${formatRelativeTime(controlPlane.latest_rca_at || controlPlane.latest_joint_risk_at)}`
                                        : ''}
                                </div>
                                {controlPlane.top_recommendation && (
                                    <div className="mt-2 text-xs text-muted-foreground">
                                        Next step: {compactText(controlPlane.top_recommendation, 120)}
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {reports.map((report) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {firstInvestigationEvent(report) && (
                                    <div className="text-xs text-cyan-200 mb-1">
                                        Event: {firstInvestigationEvent(report)}
                                    </div>
                                )}
                                {firstProbableCause(report) && (
                                    <div className="text-xs text-muted-foreground mb-1">
                                        Cause: {firstProbableCause(report)}
                                    </div>
                                )}

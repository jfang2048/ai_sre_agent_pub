...
function loopSummary(incident?: AgentIncidentReport): string {
    ...
    return `completed=${loop.completed}, iterations=${loop.iterations}, replans=${loop.replans}, executed=${loop.steps_executed}, verified=${loop.steps_verified}, stop=${loop.stop_reason}`;
}
...
export default function IncidentsPage() {
    ...
    const selectedIncidentID =
        incidents.find((incident) => incident.incident_id === pinnedIncidentID)?.incident_id ??
        ...
        <div className="space-y-4" data-testid="incidents-page">
                    ...
                    <div className="space-y-2">
                        {incidents.map((incident, index) => (
                            <button
                                ...
                                type="button"
                                onClick={() => setPinnedIncidentID(incident.incident_id)}
                    ...
                    {selectedIncident && (
                        <div className="space-y-3">
                            <article className="rounded border border-border bg-background/50 p-3">
                                ...
                                {selectedIncident.synthesized_incident?.summary && (
                                    <div className="text-xs text-muted-foreground mt-1">
                                        grouped incident: {selectedIncident.synthesized_incident.summary}
                                    </div>
                                )}
                                <div className="text-xs text-muted-foreground mt-1">
                            ...
                            {(selectedIncident.synthesized_incident?.grouped_signals ?? []).length > 0 && (
                                <div>
                                    <h4 className="text-sm font-semibold mb-1">Grouped Signals</h4>
                                    <div className="space-y-1">
                                        {(selectedIncident.synthesized_incident.grouped_signals ?? []).slice(0, 6).map((item) => (
                                            <div key={`${item.signal_id}-${item.entity}`} className="rounded border border-border bg-background/40 p-2 text-xs text-muted-foreground">
                                                {item.signal_type} · {item.scope}/{item.entity} · {item.summary}
                                            </div>
                                        ))}
                                    </div>
                                </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                ...
                <section className="rounded-lg border border-border bg-card p-4">
                    ...
                    <div className="space-y-2">
                        {(selectedIncident?.recommendations ?? []).slice(0, 8).map((item) => (
                            <article key={item.id} className="rounded border border-border bg-background/40 p-3">
                                ...
                                <div className="text-xs text-muted-foreground mt-1">
                                    category={item.category || 'n/a'} · priority={item.priority} · risk={item.risk_level || item.priority}
                                </div>
                                {item.rationale && <div className="text-xs text-muted-foreground mt-1">why: {item.rationale}</div>}
                                {item.expected_impact && <div className="text-xs text-muted-foreground mt-1">impact: {item.expected_impact}</div>}
                                {item.rollback_consideration && <div className="text-xs text-muted-foreground mt-1">rollback: {item.rollback_consideration}</div>}
                            </article>
                        ))}
                    ...
                    <h4 className="text-sm font-semibold mt-4 mb-2">Proposed Actions</h4>
                    <div className="space-y-2">
                        {(selectedIncident?.proposed_actions ?? []).slice(0, 6).map((item) => (
                            <article key={item.id} className="rounded border border-border bg-background/40 p-3">
                                <div className="text-sm font-medium">{item.command_preview}</div>
                                <div className="text-xs text-muted-foreground mt-1">
                                    policy={item.policy?.status ?? 'n/a'} · approval_required={String(item.approval_required)} · risk={item.risk_level}
                                </div>
                                {item.rollback_plan && <div className="text-xs text-muted-foreground mt-1">rollback: {item.rollback_plan}</div>}
                            </article>
                        ))}
                        {(selectedIncident?.proposed_actions ?? []).length === 0 && (
                            <div className="text-sm text-muted-foreground">No proposed actions recorded.</div>
                        )}
                    </div>
                    {(selectedIncident?.unresolved_gaps ?? []).length > 0 && (
                        <>
                            <h4 className="text-sm font-semibold mt-4 mb-2">Unresolved Gaps</h4>
                            <div className="space-y-1">
                                {(selectedIncident?.unresolved_gaps ?? []).slice(0, 6).map((item, index) => (
                                    <div key={`${item}-${index}`} className="rounded border border-border bg-background/40 p-2 text-xs text-muted-foreground">
                                        {item}
                                    </div>
                                ))}
                            </div>

...
export default function RiskInsightsPage() {
    ...
    const cachedFindingsQuery = useQuery({
        queryKey: ['risk-insights-findings', selectedCollector, windowSize, 'cached'],
        queryFn: () =>
    ...
    const findingsQuery = useQuery({
        queryKey: ['risk-insights-findings', selectedCollector, windowSize, 'refresh'],
        queryFn: () =>
            fetchPotentialRiskFindings({
                collectorId: selectedCollector || undefined,
                window: windowSize,
    ...
    const cachedFindings = cachedFindingsQuery.data?.findings ?? [];
    const findings = findingsQuery.data?.findings ?? cachedFindings;
    const isPrimingFindings = findings.length === 0
        && (findingsQuery.isLoading || findingsQuery.isFetching || cachedFindingsQuery.isLoading);
    const findingsStatusText = isPrimingFindings
        ? 'Generating latest risk findings...'
        : findingsQuery.isFetching
            ? 'Refreshing risk findings...'
            : '';
    const selectedFinding = findings.find((row) => row.id === pinnedFindingID) ?? findings[0];
    const topFinding = findings[0];

    const chartSeries = useMemo(() => {
        return buildPreferredRiskChartSeries(
            selectedFinding?.series ?? [],
    ...
    return (
        <div className="space-y-4" data-testid="risk-insights-page">
            <section className="rounded-lg border border-border bg-card p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        {findingsStatusText && <p className="text-xs text-muted-foreground mt-2">{findingsStatusText}</p>}
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                <SummaryCard icon={ShieldAlert} label="Findings" value={isPrimingFindings ? 'Loading...' : formatCount(findings.length)} detail="ranked latent risks" />
                <SummaryCard icon={BarChart3} label="Top Confidence" value={isPrimingFindings ? 'Loading...' : formatPercent((topFinding?.confidence_score ?? 0) * 100, 1)} detail={topFinding?.scope ?? 'n/a'} />
                <SummaryCard icon={Link2} label="Correlations" value={isPrimingFindings ? 'Loading...' : formatCount(selectedFinding?.correlations?.length)} detail="co-occurring signals" />
                <SummaryCard icon={SearchCheck} label="Evidence Signals" value={isPrimingFindings ? 'Loading...' : formatCount(selectedFinding?.contributing_signals?.length)} detail="active low-severity inputs" />
            </div>

            <section className="rounded-lg border border-border bg-card p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        <div className="text-sm font-semibold">Lead suspicion</div>
                        <div className="mt-1 text-sm text-muted-foreground">
                            {isPrimingFindings
                                ? 'Generating prioritized finding summary...'
                                : selectedFinding?.risk_summary || 'No latent-risk summary is available.'}
                        </div>
                    </div>
                    {!isPrimingFindings && selectedFinding?.investigation_events?.[0] && (
                        <div className="rounded-lg border border-cyan-500/30 bg-cyan-500/10 px-3 py-2 text-sm text-cyan-100">
                            <div className="font-medium">{selectedFinding.investigation_events[0].title}</div>
                            {selectedFinding.investigation_events[0].probable_cause && (
                                <div className="mt-1 text-xs text-cyan-50/90">Probable cause: {selectedFinding.investigation_events[0].probable_cause}</div>
                            )}
                        </div>
                    )}
                </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                <section className="rounded-lg border border-border bg-card p-4">
                    ...
                    {isPrimingFindings && <div className="text-sm text-muted-foreground">Generating latest risk findings...</div>}
                    {!isPrimingFindings && findings.length === 0 && <div className="text-sm text-muted-foreground">No potential risks generated yet.</div>}
                ...
                <section className="rounded-lg border border-border bg-card p-4">
                    ...
                    {isPrimingFindings && !selectedFinding && <div className="text-sm text-muted-foreground">Generating latest risk findings...</div>}
                    {!isPrimingFindings && !selectedFinding && <div className="text-sm text-muted-foreground">Select a risk finding to inspect evidence.</div>}
                    <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                <InvestigationEventsPanel
                    title="Weak-Signal and Event Summary"
                    events={selectedFinding?.investigation_events}
                    emptyText={isPrimingFindings ? 'Generating investigation events...' : 'This finding did not promote any investigation events.'}
                />
                <TrendWatchPanel
                    title="Single-Metric Trend Watch"
                    trends={selectedFinding?.trend_assessments}
                    emptyText={isPrimingFindings ? 'Generating trend watch...' : 'This finding has no promoted trend assessments.'}
                />
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                <section className="rounded-lg border border-border bg-card p-4 min-h-[320px]">
                    ...
                    {isPrimingFindings ? (
                        <div className="text-sm text-muted-foreground">Generating latest risk trends...</div>
                    ) : chartData.length === 0 ? (
                ...
                <section className="rounded-lg border border-border bg-card p-4">
                    ...
                    {isPrimingFindings ? (
                        <div className="text-sm text-muted-foreground">Generating correlation details...</div>
            ...
            <section className="rounded-lg border border-border bg-card p-4">
                ...
                {isPrimingFindings ? (
                    <div className="text-sm text-muted-foreground">Generating suggested investigation steps...</div>
            ...
            <RetrievalDecisionPanel
                ...
                decisions={selectedFinding?.retrieval_decisions}
                emptyText={isPrimingFindings
                    ? 'Generating retrieval decisions...'
                    : 'No retrieval decision was recorded for this finding.'}
            />

            <KnowledgeEvidencePanel
                title="Retrieved Supporting Knowledge"
                summary={selectedFinding?.retrieval_summary}
                confidence={selectedFinding?.retrieval_confidence}
                evidenceIDs={selectedFinding?.retrieval_evidence_ids}
                docs={(selectedFinding?.retrieved_docs ?? []).map((doc) => ({
                    id: doc.evidence_id,
                    title: doc.title,
                ...
                emptyText={isPrimingFindings
                    ? 'Generating supporting knowledge...'
                    : 'No supporting knowledge was attached to this potential-risk finding.'}
                testId="risk-knowledge-evidence"
            />
        </div>
    );
}

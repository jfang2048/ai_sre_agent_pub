...
export default function JointRiskPage() {
    ...
    const riskQuery = useQuery({
        queryKey: ['joint-risk-reports', selectedCollector, windowSize],
    ...
    const reports = riskQuery.data?.reports ?? [];
    ...
    return (
        <div className="space-y-4" data-testid="joint-risk-page">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                <SummaryCard icon={ShieldAlert} label="Risk Level" value={report?.risk_level?.toUpperCase() ?? 'N/A'} detail={report?.summary ?? 'No report'} />
                <SummaryCard icon={Activity} label="Risk Score" value={formatPercent((report?.risk_score ?? 0) * 100, 1)} detail="weighted model" />
                <SummaryCard icon={Combine} label="Co-occurrences" value={formatCount(report?.cooccurrences?.length)} detail="signal groups" />
                <SummaryCard icon={AlertTriangle} label="Active Signals" value={formatCount((report?.signals ?? []).filter((item) => item.triggered).length)} detail="triggered now" />
            </div>

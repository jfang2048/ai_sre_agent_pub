...
function summaryDisplay(
    ...
    quality: TelemetryQuality | undefined,
    unit: 'percent' | 'bytes_per_second' | 'count' | string,
    key: string,
): string {
    const value = summaryValue(summary, key);
    if (!value.available) {
        return telemetryValueFallback(quality);
    }
    switch (unit) {
        case 'percent':
        ...
        default:
            return formatMetricByUnit(value.value, unit);
    }
}

function telemetryValueFallback(quality?: TelemetryQuality): string {
...
function telemetryDetailFallback(quality?: TelemetryQuality): string {
    return quality?.quality_hint || 'No fresh telemetry for this metric yet';
}

function telemetryQualityBannerClass(state?: string): string {
    switch (state) {
        case 'stale':
        case 'unavailable':
    ...
    const summaryCards = [
        {
            key: 'cpu',
            label: 'CPU',
            value: summaryDisplay(summary, telemetryQuality, 'percent', 'cpu_usage_percent'),
            detail: summaryValue(summary, 'load1').available ? `Load1 ${Number(summary.load1).toFixed(2)}` : telemetryDetailFallback(telemetryQuality),
            icon: Cpu,
        ...
        },
        {
            key: 'memory',
            label: 'Memory',
            value: summaryDisplay(summary, telemetryQuality, 'percent', 'memory_used_percent'),
            detail: summaryValue(summary, 'memory_used_bytes').available && summaryValue(summary, 'memory_total_bytes').available
                ? `${formatBytes(summary.memory_used_bytes)} / ${formatBytes(summary.memory_total_bytes)}`
                : telemetryDetailFallback(telemetryQuality),
            icon: HardDrive,
        ...
        },
        {
            ...
            value: summaryDisplay(summary, telemetryQuality, 'bytes_per_second', 'network_total_bytes_per_second'),
            detail: summaryValue(summary, 'network_rx_bytes_per_second').available && summaryValue(summary, 'network_tx_bytes_per_second').available
                ? `RX ${formatRate(summary.network_rx_bytes_per_second)} · TX ${formatRate(summary.network_tx_bytes_per_second)}`
                : telemetryDetailFallback(telemetryQuality),
            icon: Network,
        ...
        },
        {
            ...
            value: summaryDisplay(summary, telemetryQuality, 'bytes_per_second', 'disk_total_bytes_per_second'),
            detail: summaryValue(summary, 'disk_read_bytes_per_second').available && summaryValue(summary, 'disk_write_bytes_per_second').available
                ? `R ${formatRate(summary.disk_read_bytes_per_second)} · W ${formatRate(summary.disk_write_bytes_per_second)}`
                : telemetryDetailFallback(telemetryQuality),
            icon: Workflow,
        ...
        },
        {
            ...
            value: summaryDisplay(summary, telemetryQuality, 'count', 'disk_total_iops_per_second'),
            detail: summaryValue(summary, 'disk_queue_depth_total').available && summaryValue(summary, 'disk_utilization_peak_percent').available
                ? `Queue ${Number(summary.disk_queue_depth_total).toFixed(2)} · Util ${formatPercent(summary.disk_utilization_peak_percent)}`
                : telemetryDetailFallback(telemetryQuality),
            icon: Activity,
            category: 'disk_io' as ResourceCategory,
        ...
        },
        {
            key: 'fs-pressure',
            label: 'FS Pressure',
            value: summaryDisplay(summary, telemetryQuality, 'percent', 'filesystem_space_pressure_percent'),
            detail: summaryValue(summary, 'filesystem_inode_pressure_percent').available && summaryValue(summary, 'disk_avg_request_latency_ms').available
                ? `Inodes ${formatPercent(summary.filesystem_inode_pressure_percent)} · Lat ${Number(summary.disk_avg_request_latency_ms).toFixed(1)} ms`
                : telemetryDetailFallback(telemetryQuality),
            icon: HardDrive,
        ...
        },
        {
            key: 'procs',
            label: 'Processes',
            value: summaryDisplay(summary, telemetryQuality, 'count', 'procs_running'),
            detail: summaryValue(summary, 'procs_blocked').available
                ? `Blocked ${formatCount(summary.procs_blocked)}`
                : telemetryDetailFallback(telemetryQuality),
            icon: Activity,
            category: 'cpu' as ResourceCategory,
        ...
        <div className="h-full overflow-auto p-4 md:p-6 space-y-4">
            <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
                ...
                {telemetryQuality && telemetryQuality.state !== 'fresh' && (
                    <div className={`mt-3 rounded-lg border px-3 py-3 ${telemetryQualityBannerClass(telemetryQuality.state)}`}>
                        <div className="flex flex-wrap items-center justify-between gap-2">
                            <div className="text-sm font-medium">Telemetry {telemetryQuality.state}</div>
                            <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
                                coverage {Math.round(telemetryQuality.coverage_percent ?? 0)}% · source {telemetryQuality.source_mode || 'unknown'}
                            </div>
                        </div>
                        <div className="mt-1 text-xs text-muted-foreground">{telemetryQuality.quality_hint || 'Telemetry is not fully healthy.'}</div>
                    </div>
                )}
            </div>
                ...
                {operationalInsights.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
                        {operationalInsights.map((insight) => (
                            <OperationalInsightCard key={insight.key} insight={insight} />
                        ))}
                    </div>
                )}
            </div>
...
function OperationalInsightCard({ insight }: { insight: FleetOperationalInsight }) {
    return (
        <div className={`rounded-lg border px-3 py-3 ${operationalInsightClass(insight.severity)}`}>
            ...
            {insight.evidence && insight.evidence.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                    {insight.evidence.map((item) => (
                        <span key={item} className="rounded-full border border-border/70 px-2 py-0.5 text-[10px] text-slate-200">
                            {item}
                        </span>
                    ))}
                </div>
...
function formatTierLabel(tier: string): string {
    switch (tier) {
        case 'tier1_runtime':
            return 'Tier 1 runtime';
        case 'tier2_operational':
            return 'Tier 2 ops';
        case 'tier3_structural':
            return 'Tier 3 structure';
        case 'tier4_event':
            return 'Tier 4 events';
        default:
            return tier;
    }
}

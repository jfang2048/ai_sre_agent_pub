...
import { AlertTriangle, Cpu, HardDrive, Network, Activity, Gauge, ScrollText } from 'lucide-react';
...
export default function ResourceProcessBreakdownPanel({
    ...
    const rows = useMemo<RankedRow[]>(() => {
        ...
        const total = prepared.reduce((sum, row) => sum + row.primaryValue, 0);
        return prepared.map((row, index) => ({
            ...row,
            rank: index + 1,
            sharePercent: total > 0 ? (row.primaryValue / total) * 100 : 0,
        }));
    }, [category, collectorId, query.data?.by_category, query.data?.resource_pages]);
...
function formatCategoryDetails(program: ProgramStats, category: ResourceCategory): string {
    switch (category) {
        case 'cpu':
            return `mem ${formatBytes(program.memory_bytes)} · ${formatRate((program.disk_read_bps ?? 0) + (program.disk_write_bps ?? 0))} I/O`;
        case 'memory':
        ...
        case 'disk_io':
            return `read ${formatRate(program.disk_read_bps)} · write ${formatRate(program.disk_write_bps)}`;
        case 'disk':

...
function runtimeModeSeverity(node: DataPathNodeModel): 'critical' | 'degraded' | 'healthy' {
    if (node.runtime_mode === 'limited') {
        return 'critical';
    }
    if (node.runtime_degraded || node.runtime_mode === 'namespace') {
        return 'degraded';
    }
    return 'healthy';
}
...
export default function DataPathDiagnosticsPage({ onOpenTrends }: DataPathDiagnosticsPageProps) {
    ...
    const copyWorkloadScopeLink = async () => {
        ...
        const copied = await writeClipboardText(typeof window === 'undefined' ? '' : window.location.href);
        setWorkloadScopeLinkStatus(copied ? 'copied' : 'unavailable');
    };
    ...
    const copyWorkloadHandoffMarkdown = async () => {
        ...
        setWorkloadHandoffStatus(copied ? 'copied' : 'unavailable');
    };
    ...
    const copyRootCausePacket = async () => {
        ...
        const copied = await writeClipboardText(artifact.markdown);
        setWorkloadRcaPacketStatus(copied ? 'copied' : 'unavailable');
    };
        ...
        <div className="h-full overflow-auto p-4 md:p-6 space-y-4">
            <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
                ...
                <div className="mt-3 text-xs text-muted-foreground flex flex-wrap gap-x-4 gap-y-1">
                    ...
                    <span>Runtime degraded: {diagnostics?.summary.runtime_degraded_nodes ?? 0}</span>
                    <span>Namespace mode: {diagnostics?.summary.runtime_namespace_nodes ?? 0}</span>
                    <span>Limited mode: {diagnostics?.summary.runtime_limited_nodes ?? 0}</span>
                ...
                <div className="overflow-auto">
                    <table className="w-full text-xs">
                        <thead className="border-b border-border text-muted-foreground">
                            <tr>
                                <th className="text-left py-2 pr-2">Node</th>
                                <th className="text-left py-2 pr-2">Runtime</th>
                            ...
                            {(diagnostics?.data_paths ?? []).map((node) => {
                                ...
                                return (
                                    <tr key={`path-${node.collector_id}`} className="border-b border-border/50">
                                        ...
                                        <td className="py-2 pr-2">
                                            <span className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-wide ${severityClass(runtimeModeSeverity(node))}`}>
                                                {node.runtime_mode || 'host'}
                                            </span>
                                            <div className="text-[11px] text-muted-foreground mt-1">
                                                {(node.runtime_reasons ?? []).slice(0, 1).join(' ') || (node.runtime_degraded ? 'degraded coverage' : 'full host visibility')}
                                            </div>
                                        </td>

...
import { Settings, Bell, LayoutDashboard, Activity, Bot, Moon, Sun, X, Workflow, Gauge, ShieldAlert, BrainCircuit, AlertTriangle, ScrollText, Shield, Siren, ClipboardList } from 'lucide-react';

const queryClient = new QueryClient();
type ActivePage = 'dashboard' | 'trends' | 'analysis' | 'riskInsights' | 'jointRisk' | 'rca' | 'security' | 'incidents' | 'audit' | 'logs' | 'dataPath' | 'gpu' | 'agent';
...
function App() {
    ...
    const pageTitle = useMemo(() => {
        ...
        if (activePage === 'riskInsights') {
            return 'Risk Insights';
        }
        if (activePage === 'jointRisk') {
            return 'Joint Risk';
        }
        if (activePage === 'rca') {
            return 'RCA Workflow';
            ...
            <div className={`flex h-screen bg-background text-foreground ${theme}`}>
                ...
                <aside className="w-16 border-r border-border flex flex-col items-center py-4 gap-6 bg-card z-50">
                    ...
                    <nav className="flex flex-col gap-4">
                        ...
                        <button
                            onClick={() => setActivePage('riskInsights')}
                            title="Risk Insights"
                            className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors ${
                                activePage === 'riskInsights'
                                    ? 'bg-accent text-accent-foreground'
                                    : 'text-muted-foreground hover:bg-muted/50'
                            }`}
                        >
                        ...
                        <button
                            onClick={() => setActivePage('jointRisk')}
                            title="Joint Risk"
                            className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors ${
                                activePage === 'jointRisk'
                                    ? 'bg-accent text-accent-foreground'
                                    : 'text-muted-foreground hover:bg-muted/50'
                            }`}
                        >
                ...
                <main className="flex-1 flex flex-col overflow-hidden relative">
                    ...
                    <div className="flex-1 overflow-auto bg-background p-6 relative">
                        ...
                        {activePage === 'analysis' && (
                            <IncidentAnalysisPage onOpenTrends={openTrendsFromDiagnostics} />
                        )}
                        {activePage === 'riskInsights' && <RiskInsightsPage />}
                        {activePage === 'jointRisk' && <JointRiskPage />}
                        {activePage === 'rca' && <RCAPage />}
                        {activePage === 'security' && <SecurityDashboardPage />}

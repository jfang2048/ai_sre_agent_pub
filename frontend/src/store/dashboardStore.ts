...
const DEFAULT_LAYOUT: LayoutItem[] = [
    ...
    { i: 'k8s-drilldown', x: 0, y: OVERVIEW_METRICS_MIN_HEIGHT + 6, w: 12, h: 6 },
    { i: 'logs-feed', x: 0, y: OVERVIEW_METRICS_MIN_HEIGHT + 12, w: 12, h: 4 },
    { i: 'orchestration-slo', x: 0, y: OVERVIEW_METRICS_MIN_HEIGHT + 16, w: 12, h: 6 },
    { i: 'ops-control', x: 0, y: OVERVIEW_METRICS_MIN_HEIGHT + 22, w: 12, h: 6 },
];

const DEFAULT_WIDGETS = ['overview-metrics', 'ai-insights', 'topology-graph', 'top-programs', 'k8s-drilldown', 'logs-feed', 'orchestration-slo', 'ops-control'];
...
function ensureDashboardLayout(state: Partial<DashboardState>): Partial<DashboardState> {
    ...
    if (!widgets.includes('k8s-drilldown')) {
        widgets.push('k8s-drilldown');
        layout.push({ i: 'k8s-drilldown', x: 0, y: OVERVIEW_METRICS_MIN_HEIGHT + 6, w: 12, h: 6 });
    }
    if (!widgets.includes('orchestration-slo')) {
        widgets.push('orchestration-slo');
        layout.push({ i: 'orchestration-slo', x: 0, y: OVERVIEW_METRICS_MIN_HEIGHT + 16, w: 12, h: 6 });
    }
    if (!widgets.includes('ops-control')) {
        widgets.push('ops-control');
        layout.push({ i: 'ops-control', x: 0, y: OVERVIEW_METRICS_MIN_HEIGHT + 22, w: 12, h: 6 });
    }

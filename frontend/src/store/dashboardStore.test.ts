...
describe('dashboard store migration', () => {
    ...
    it('includes k8s-drilldown widget by default', () => {
        const state = useDashboardStore.getState();
        expect(state.widgets).toContain('k8s-drilldown');
        expect(state.layout.some((l) => l.i === 'k8s-drilldown')).toBe(true);
    });

    it('includes orchestration-slo widget by default', () => {
        const state = useDashboardStore.getState();
        expect(state.widgets).toContain('orchestration-slo');
        expect(state.layout.some((l) => l.i === 'orchestration-slo')).toBe(true);
    });

    it('includes ops-control widget by default', () => {
        const state = useDashboardStore.getState();
        expect(state.widgets).toContain('ops-control');
        expect(state.layout.some((l) => l.i === 'ops-control')).toBe(true);
    });

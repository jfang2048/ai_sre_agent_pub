...
import { appendQueryParam, setPositiveIntParam, setQueryParam, toQuerySuffix } from './query';
...
export async function fetchFleetTimeseries(query: FleetTimeseriesQuery = {}): Promise<FleetTimeseriesResponse> {
    ...
    (query.metrics ?? []).forEach((metric) => {
        appendQueryParam(params, 'metric', metric);
    });

    const { data } = await api.get<FleetTimeseriesResponse>(`/fleet/timeseries${toQuerySuffix(params)}`);
    return data;
}

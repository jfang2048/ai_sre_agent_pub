...
export interface ProgramStats {
    ...
    gpu_util_enc_percent?: number;
    gpu_util_dec_percent?: number;
    gpu_context_active?: number;
    log_errors?: number;
    log_warnings?: number;
    log_events?: number;
...
export async function fetchTopPrograms(input: number | TopProgramsQuery = 15): Promise<TopProgramsResponse> {
    ...
    setPositiveIntParam(params, 'limit', query.limit ?? 15);
    setQueryParam(params, 'collector_id', query.collectorId);
    const { data } = await api.get<TopProgramsResponse>(`/top/programs${toQuerySuffix(params)}`);
    return data;
}

...
export async function fetchDataPathDiagnostics(query: DataPathDiagnosticsQuery = {}): Promise<DataPathDiagnosticsResponse> {
    ...
    const { data } = await api.get<DataPathDiagnosticsResponse>(`/diagnostics/data-path${toQuerySuffix(params)}`);
    return data;
}
...
export async function fetchRootCauseDiagnostics(query: DataPathDiagnosticsQuery = {}): Promise<RootCauseDiagnosticsResponse> {
    const params = new URLSearchParams();
    setQueryParam(params, 'collector_id', query.collectorId);
    const { data } = await api.get<RootCauseDiagnosticsResponse>(`/diagnostics/root-cause${toQuerySuffix(params)}`);
    return data;
}
...
export async function fetchKernelPathDiagnostics(query: DataPathDiagnosticsQuery = {}): Promise<KernelPathDiagnosticsResponse> {
    const params = new URLSearchParams();
    setQueryParam(params, 'collector_id', query.collectorId);
    const { data } = await api.get<KernelPathDiagnosticsResponse>(`/diagnostics/kernel-path${toQuerySuffix(params)}`);
    return data;
}
...
export async function fetchWorkloadPathDiagnostics(query: WorkloadPathDiagnosticsQuery = {}): Promise<WorkloadPathDiagnosticsResponse> {
    const params = new URLSearchParams();
    setQueryParam(params, 'cluster', query.cluster);
    setQueryParam(params, 'namespace', query.namespace);
    setQueryParam(params, 'service', query.service);
    setPositiveIntParam(params, 'limit', query.limit, { floor: true });
    const { data } = await api.get<WorkloadPathDiagnosticsResponse>(`/diagnostics/workload-path${toQuerySuffix(params)}`);
    return data;
}
...
export async function fetchAIInfraStackDiagnostics(query: AIInfraStackDiagnosticsQuery = {}): Promise<AIInfraStackDiagnosticsResponse> {
    ...
    setQueryParam(params, 'namespace', query.namespace);
    setQueryParam(params, 'service', query.service);
    setPositiveIntParam(params, 'workload_limit', query.workloadLimit, { floor: true });
    const { data } = await api.get<AIInfraStackDiagnosticsResponse>(`/diagnostics/ai-infra-stack${toQuerySuffix(params)}`);
    return data;
}
...
export async function fetchRCAPacketExport(query: RCAPacketExportQuery = {}): Promise<RCAPacketExportResponse> {
    ...
    setQueryParam(params, 'namespace', query.namespace);
    setQueryParam(params, 'service', query.service);
    setQueryParam(params, 'sort_key', query.sortKey);
    setQueryParam(params, 'sort_direction', query.sortDirection);
    setPositiveIntParam(params, 'workload_limit', query.workloadLimit, { floor: true });
    const { data } = await api.get<RCAPacketExportResponse>(`/diagnostics/rca-packet${toQuerySuffix(params)}`);
    return data;
}

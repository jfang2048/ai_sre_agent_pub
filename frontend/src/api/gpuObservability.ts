...
export async function fetchGPUTimeline(params: {
    ...
    search.set('collector_id', params.collectorId);
    search.set('gpu_id', params.gpuId);
    search.set('metric', params.metric);
    if (params.window) search.set('window', params.window);
    if (params.limit) search.set('limit', String(params.limit));

    const { data } = await api.get<GPUTimelineResponse>(`/gpu/timeline?${search.toString()}`);
    return data;
}
...
export async function fetchGPUProcessTimeline(params: {
    ...
    search.set('collector_id', params.collectorId);
    search.set('gpu_id', params.gpuId);
    search.set('pid', params.pid);
    search.set('metric', params.metric);
    if (params.window) search.set('window', params.window);
    if (params.limit) search.set('limit', String(params.limit));

    const { data } = await api.get<GPUProcessTimelineResponse>(`/gpu/process-timeline?${search.toString()}`);
    return data;
}
...
export async function fetchGPUEvents(params: {
    ...
    const search = new URLSearchParams();
    search.set('collector_id', params.collectorId);
    if (params.gpuId) search.set('gpu_id', params.gpuId);
    if (params.severity) search.set('severity', params.severity);
    if (params.window) search.set('window', params.window);
    if (params.limit) search.set('limit', String(params.limit));

    const { data } = await api.get<GPUEventsResponse>(`/gpu/events?${search.toString()}`);
    return data;
}
...
export async function fetchGPUProcesses(params: {
    ...
    const { data } = await api.get<GPUProcessesResponse>(`/gpu/processes?${search.toString()}`);
    return data;
}

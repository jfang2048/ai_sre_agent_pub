...
export async function fetchAnalysisIncidents(params: { node?: string; limit?: number } = {}): Promise<AnalysisIncidentsResponse> {
    ...
    if (params.node) {
        query.set('collector_id', params.node);
    }
    if (params.limit && params.limit > 0) {
        query.set('limit', String(params.limit));
    }
    const suffix = query.toString();
    const { data } = await api.get<AnalysisIncidentsResponse>(`/analysis/incidents${suffix ? `?${suffix}` : ''}`);
    return data;
}
...
export async function fetchAnalysisCorrelations(params: { node?: string } = {}): Promise<AnalysisCorrelationsResponse> {
    ...
    const suffix = query.toString();
    const { data } = await api.get<AnalysisCorrelationsResponse>(`/analysis/correlations${suffix ? `?${suffix}` : ''}`);
    return data;
}

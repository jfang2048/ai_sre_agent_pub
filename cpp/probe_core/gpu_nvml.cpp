...
constexpr unsigned long long kNVMLClocksThrottleReasonNone = 0x0000000000000000ULL;
constexpr unsigned long long kNVMLClocksThrottleReasonGpuIdle = 0x0000000000000002ULL;
constexpr unsigned long long kNVMLClocksThrottleReasonSwPowerCap = 0x0000000000000008ULL;
constexpr unsigned long long kNVMLClocksThrottleReasonHwSlowdown = 0x0000000000000010ULL;
constexpr unsigned long long kNVMLClocksThrottleReasonSwThermalSlowdown = 0x0000000000000040ULL;
constexpr unsigned long long kNVMLClocksThrottleReasonHwThermalSlowdown = 0x0000000000000080ULL;
constexpr unsigned long long kNVMLClocksThrottleReasonHwPowerBrakeSlowdown = 0x0000000000000100ULL;

constexpr int kNVMLFeatureEnabled = 1;
constexpr int kNVMLMaxNvLinks = 32;
...
struct NVMLApi {
  ...
  nvmlReturn_t (*device_get_current_clocks_throttle_reasons)(nvmlDevice_t,
                                                             unsigned long long*) = nullptr;
  nvmlReturn_t (*device_get_total_energy_consumption)(nvmlDevice_t, unsigned long long*) = nullptr;
  nvmlReturn_t (*device_get_nvlink_state)(nvmlDevice_t, unsigned int, int*) = nullptr;
};
...
bool loadNVMLApi(NVMLApi* api) {
  ...
  api->device_get_current_clocks_throttle_reasons =
      loadSymbol<nvmlReturn_t (*)(nvmlDevice_t, unsigned long long*)>(
          handle, {"nvmlDeviceGetCurrentClocksThrottleReasons"});
  api->device_get_total_energy_consumption =
      loadSymbol<nvmlReturn_t (*)(nvmlDevice_t, unsigned long long*)>(
          handle, {"nvmlDeviceGetTotalEnergyConsumption"});
  api->device_get_nvlink_state =
      loadSymbol<nvmlReturn_t (*)(nvmlDevice_t, unsigned int, int*)>(
          handle, {"nvmlDeviceGetNvLinkState"});
  ...
  for (unsigned int index = 0; index < count; ++index) {
    ...
    if (api.device_get_current_clocks_throttle_reasons != nullptr) {
      ...
      if (api.device_get_current_clocks_throttle_reasons(device, &reasons) == kNVMLSuccess) {
        sample.throttle_idle =
            (reasons & kNVMLClocksThrottleReasonGpuIdle) ? 1.0 : 0.0;
        sample.throttle_power =
            (reasons & (kNVMLClocksThrottleReasonSwPowerCap |
                        kNVMLClocksThrottleReasonHwPowerBrakeSlowdown)) ? 1.0 : 0.0;
        sample.throttle_thermal =
            (reasons & (kNVMLClocksThrottleReasonHwSlowdown |
                        kNVMLClocksThrottleReasonSwThermalSlowdown |
                        kNVMLClocksThrottleReasonHwThermalSlowdown)) ? 1.0 : 0.0;
        sample.throttle_active_any =
            (sample.throttle_power > 0.0 || sample.throttle_thermal > 0.0) ? 1.0 : 0.0;
      }
    }

    if (api.device_get_total_energy_consumption != nullptr) {
      unsigned long long mj = 0;
      if (api.device_get_total_energy_consumption(device, &mj) == kNVMLSuccess) {
        sample.energy_consumption_joules = static_cast<double>(mj) / 1000.0;
      }
    }

    if (api.device_get_nvlink_state != nullptr) {
      unsigned int active_links = 0;
      for (unsigned int link = 0; link < kNVMLMaxNvLinks; ++link) {
        int is_active = 0;
        if (api.device_get_nvlink_state(device, link, &is_active) == kNVMLSuccess) {
          if (is_active == kNVMLFeatureEnabled) {
            active_links++;
          }
        }
      }
      sample.nvlink_links = static_cast<double>(active_links);
    }

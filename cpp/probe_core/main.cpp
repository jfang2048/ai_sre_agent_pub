...
using probe_core::NetSnapshot;
using probe_core::NetlinkLinkData;
using probe_core::ParsedEBPFEvent;
using probe_core::SocketQueue;
using probe_core::readNetlinkLinkData;
using probe_core::readSocketQueuesByInode;
...
double pcieLaneBandwidthMBps(double gen) {
  if (gen >= 6.0) return 7876.0;
  if (gen >= 5.0) return 3938.0;
  if (gen >= 4.0) return 1969.0;
  if (gen >= 3.0) return 984.6;
  if (gen >= 2.0) return 500.0;
  if (gen >= 1.0) return 250.0;
  return 0.0;
}

double pcieBandwidthMBps(double gen, double width) {
  if (gen <= 0.0 || width <= 0.0) return 0.0;
  return pcieLaneBandwidthMBps(gen) * width;
}
...
std::optional<HostCollectionMode> parseHostMode(const std::string& raw) {
  ...
  if (mode == "proc" || mode == "proc_fallback") return HostCollectionMode::kProc;
  return std::nullopt;
}
...
std::unordered_set<std::string> listSocketInodesForPid(int pid) {
  ...
  probe_core::ScopedDir dir(opendir(fd_path.c_str()));
...
class ProbeCoreRuntime {
 public:
  explicit ProbeCoreRuntime(Options options)
      : opts_(std::move(options)),
        ...
        effective_process_interval_samples_(std::max(1, opts_.process_interval_samples)),
        effective_host_proc_fallback_interval_samples_(
            std::max(1, opts_.host_proc_fallback_interval_samples)),
        effective_pressure_interval_samples_(std::max(1, opts_.pressure_interval_samples)),
        effective_netlink_interval_samples_(std::max(1, opts_.netlink_interval_samples)),
        effective_gpu_interval_samples_(std::max(1, opts_.gpu_interval_samples)),
        host_pressure_level_(0),
        sampling_backoff_events_(0),
        host_pressure_events_(0),
        process_kernel_collector_(std::make_unique<probe_core::ProcessKernelCollector>(
            probe_core::ProcessKernelCollectorOptions{
                .topk = opts_.topk,
                .watchlist_limit = std::max(64, opts_.topk * 8),
                .clock_ticks = std::max<long>(sysconf(_SC_CLK_TCK), 1),
                .online_cpus = std::max<int>(1, static_cast<int>(sysconf(_SC_NPROCESSORS_ONLN))),
            })),
 ...
 private:
  ...
  std::unique_ptr<probe_core::ProcessKernelCollector> process_kernel_collector_;
  ...
  uint32_t last_process_reconcile_seq_ = 0;
  ...
  int effective_process_interval_samples_;
  int effective_host_proc_fallback_interval_samples_;
  int effective_pressure_interval_samples_;
  int effective_netlink_interval_samples_;
  int effective_gpu_interval_samples_;
  int host_pressure_level_;
  uint64_t sampling_backoff_events_;
  uint64_t host_pressure_events_;
  std::string cgroup_path_;
  std::atomic<bool> ebpf_receiver_running_{false};

  void setupEBPFSocket() {
    if (opts_.ebpf_socket_path.empty()) return;
    probe_core::ScopedFD sock(socket(AF_UNIX, SOCK_DGRAM, 0));
  ...
  void ebpfReceiverLoop() {
    ...
    while (ebpf_receiver_running_.load(std::memory_order_relaxed) && ebpf_sock_fd_ >= 0) {
      ...
      while (true) {
        ...
        if (n > 0) {
          ParsedEBPFEvent evt;
          if (probe_core::parseKernelEventPayload(std::string_view(buf, static_cast<size_t>(n)),
                                                  &evt)) {
            recordEBPFEvent(evt);
          }
          event_count++;
  ...
  void recordEBPFEvent(const ParsedEBPFEvent& evt) {
    ...
    if (process_kernel_collector_ != nullptr && evt.pid > 0) {
      process_kernel_collector_->noteEBPFActivity(evt.pid, "");
      if (evt.has_resource_snapshot) {
        process_kernel_collector_->noteEBPFResourceSnapshot(
            evt.pid, evt.cpu_user_ms, evt.cpu_sys_ms, evt.rss_bytes);
      }
    }
  ...
  void applyBackoff(int64_t collect_ms) {
    ...
    if (host_pressure_level_ >= 3) {
      const int grown = std::min(max_interval, std::max(base_interval_ms_ * 2, effective_interval_ms_ + effective_interval_ms_ / 2));
    ...
    if (host_pressure_level_ >= 2) {
      const int grown = std::min(max_interval, std::max(base_interval_ms_, effective_interval_ms_ + effective_interval_ms_ / 4));
      if (grown > effective_interval_ms_) {
        effective_interval_ms_ = grown;
        sampling_backoff_events_++;
      }
  ...
  std::string collectFrame() {
    ...
    std::string frame;
    if (!env.SerializeToString(&frame)) {
      return {};
    }
    return frame;
  }
  ...
  static double metricValueAny(const std::vector<MetricPoint>& metrics,
  ...
  static double overThresholdScore(double value, double threshold, double span = 1.0) {
    if (threshold <= 0.0 || value <= threshold) return 0.0;
    if (span <= 0.0) span = 1.0;
    return std::clamp((value - threshold) / (threshold * span), 0.0, 1.0);
  }

  void updateAdaptiveSamplingState(const std::vector<MetricPoint>& metrics) {
    const double cpu_usage = metricValueAny(metrics, {"probe_core_cpu_usage_percent"});
    const double mem_percent = metricValueAny(metrics, {"probe_core_memory_used_percent"});
    const double mem_pressure =
        metricValueAny(metrics, {"probe_core_pressure_memory_some_avg10", "probe_core_pressure_memory_full_avg10"});
    const double io_pressure =
        metricValueAny(metrics, {"probe_core_pressure_io_some_avg10", "probe_core_pressure_io_full_avg10"});
    const double disk_queue = metricValueAny(metrics, {"probe_core_disk_queue_depth"});
    const double retrans =
        metricValueAny(metrics, {"probe_core_network_tcp_retransmissions_per_sec"});
    const double softnet_drops =
        metricValueAny(metrics, {"probe_core_network_softnet_dropped_total"});
    const double gpu_mem_pressure =
        metricValueAny(metrics, {"probe_core_gpu_memory_pressure_percent"});
    const double gpu_throttle =
        metricValueAny(metrics, {"probe_core_gpu_throttle_active", "probe_core_gpu_throttle_thermal_active",
                                 "probe_core_gpu_throttle_power_active"});

    int level = 0;
    if (cpu_usage >= 95.0 || mem_percent >= 92.0 || io_pressure >= 20.0) {
      level = 3;
    } else {
      int pressure_domains = 0;
      if (cpu_usage >= 80.0) pressure_domains++;
      if (mem_percent >= 85.0 || mem_pressure >= 10.0) pressure_domains++;
      if (io_pressure >= 10.0 || disk_queue >= 16.0) pressure_domains++;
      if (retrans >= 0.5 || softnet_drops >= 100.0) pressure_domains++;
      if (gpu_mem_pressure >= 90.0 || gpu_throttle > 0.0) pressure_domains++;
      if (pressure_domains >= 2) {
        level = 2;
      } else if (pressure_domains == 1) {
        level = 1;
      }
    }

    if (level > host_pressure_level_) {
      host_pressure_events_++;
    }
    host_pressure_level_ = level;
    ...
    if (host_pressure_level_ >= 2) {
      effective_process_interval_samples_ *= 2;
      effective_host_proc_fallback_interval_samples_ *= 2;
      effective_pressure_interval_samples_ *= 2;
      effective_netlink_interval_samples_ *= 2;
      effective_gpu_interval_samples_ *= 2;
    }
    if (host_pressure_level_ >= 3) {
      effective_process_interval_samples_ *= 2;
      effective_host_proc_fallback_interval_samples_ +=
  ...
  const NetlinkLinkData& getNetlinkSnapshot() {
    ...
    if (netlink_cache_.stats.empty() ||
        shouldRefresh(last_netlink_refresh_seq_, effective_netlink_interval_samples_)) {
  ...
  void collectCoreHostMetrics(double elapsed, std::vector<MetricPoint>* metrics) {
    ...
    if (kernel_ok) {
      if (shouldRefresh(last_proc_host_refresh_seq_, effective_host_proc_fallback_interval_samples_) ||
          cached_host_proc_metrics_.empty()) {
        refreshProcHostSupplementCache();
        last_proc_host_refresh_seq_ = sample_seq_;
      }
      if (shouldRefresh(last_psi_refresh_seq_, effective_pressure_interval_samples_) ||
          cached_psi_metrics_.empty()) {
        refreshPressureCache();
        last_psi_refresh_seq_ = sample_seq_;
      }
      if (shouldRefresh(last_cgroup_refresh_seq_, effective_pressure_interval_samples_) ||
      ...
      addMetric(metrics, "probe_core_host_proc_refresh_interval_samples",
                static_cast<double>(std::max(1, effective_host_proc_fallback_interval_samples_)));
      ...
      addMetric(metrics, "probe_core_cgroup_refresh_interval_samples",
                static_cast<double>(std::max(1, effective_pressure_interval_samples_)));
  ...
  probeipc::v1::ProbeBatch collectBatch() {
    ...
    updateAdaptiveSamplingState(metrics);

    std::vector<ProcRow> processes;
    if (collectorEnabled(opts_, "process")) {
      const bool lightweight_enrichment = host_pressure_level_ >= 2;
      const bool should_collect =
          shouldRefresh(last_process_collect_seq_, effective_process_interval_samples_) ||
          cached_process_rows_.empty();
      if (should_collect) {
        cached_process_rows_ = collectProcessMetrics(elapsed, &metrics, lightweight_enrichment);
      ...
      addMetric(&metrics, "probe_core_process_sampling_interval_samples",
                static_cast<double>(std::max(1, effective_process_interval_samples_)));
      addMetric(&metrics, "probe_core_process_sampling_age_samples",
                static_cast<double>(sample_seq_ - last_process_collect_seq_));
      addMetric(&metrics, "probe_core_process_enrichment_lightweight",
                lightweight_enrichment ? 1.0 : 0.0);
      addMetric(&metrics, "probe_core_process_kernel_available",
                (process_kernel_collector_ != nullptr &&
                 process_kernel_collector_->available())
                    ? 1.0
                    : 0.0);
      if (process_kernel_collector_ != nullptr &&
          !process_kernel_collector_->available() &&
          !process_kernel_collector_->failureReason().empty()) {
        addMetric(&metrics, "probe_core_process_kernel_reason", 1.0,
                  {{"reason", process_kernel_collector_->failureReason()}});
      }
    }
    ...
    addMetric(&metrics, "probe_core_sampling_pressure_level",
              static_cast<double>(host_pressure_level_));
    addMetric(&metrics, "probe_core_sampling_host_pressure_events_total",
              static_cast<double>(host_pressure_events_));
    ...
    addMetric(&metrics, "probe_core_netlink_refresh_interval_samples",
              static_cast<double>(std::max(1, effective_netlink_interval_samples_)));
    addMetric(&metrics, "probe_core_netlink_refresh_age_samples",
              static_cast<double>(sample_seq_ - last_netlink_refresh_seq_));
    addMetric(&metrics, "probe_core_host_proc_refresh_effective_interval_samples",
              static_cast<double>(std::max(1, effective_host_proc_fallback_interval_samples_)));
    addMetric(&metrics, "probe_core_pressure_refresh_effective_interval_samples",
              static_cast<double>(std::max(1, effective_pressure_interval_samples_)));
    addMetric(&metrics, "probe_core_gpu_sampling_effective_interval_samples",
              static_cast<double>(std::max(1, effective_gpu_interval_samples_)));
  ...
  void refreshEthtoolMetricsCache() {
    ...
    probe_core::ScopedFD fd(socket(AF_INET, SOCK_DGRAM, 0));
    if (!fd.valid()) return;
    ...
    for (const auto& [iface, _] : net) {
      ...
      if (ioctl(fd.get(), SIOCETHTOOL, &ifr) == 0) {
        addMetric(&cached_ethtool_metrics_, "probe_core_network_link_up",
                  static_cast<double>(eval.data), MetricLabels{{"iface", iface}});
      }
      ...
      if (ioctl(fd.get(), SIOCETHTOOL, &ifr) == 0) {
        ...
        addMetric(&cached_ethtool_metrics_, "probe_core_network_speed_mbps",
                  static_cast<double>(ethtool_cmd_speed(&ecmd)),
                  MetricLabels{{"iface", iface}});
#endif
      }
#endif
    }
  ...
  void collectEBPFMetrics(std::vector<MetricPoint>* metrics) {
    ...
    const auto pid_desc = [](const auto& left, const auto& right) {
      return left.second > right.second;
    };
    if (top_pids.size() > 16) {
      auto nth = top_pids.begin() + 16;
      std::nth_element(top_pids.begin(), nth, top_pids.end(), pid_desc);
      top_pids.resize(16);
    }
    std::sort(top_pids.begin(), top_pids.end(), pid_desc);
  ...
  void collectGPUMetrics(std::vector<MetricPoint>* metrics) {
    if (effective_gpu_interval_samples_ <= 0) effective_gpu_interval_samples_ = 1;
    const bool refresh =
        cached_gpu_metrics_.empty() ||
        (sample_seq_ - last_gpu_collect_seq_) >=
            static_cast<uint32_t>(effective_gpu_interval_samples_);
    if (refresh) {
      cached_gpu_metrics_.clear();

      const auto readGPUProcessName = [](const std::string& pid) {
        if (pid.empty()) return std::string();
        std::string raw;
        if (!readFile("/proc/" + pid + "/comm", &raw)) return std::string();
        return trim(raw);
      };

      const auto gpuLabels = [](const std::string& gpu, const std::string& uuid = std::string(),
                                const std::string& name = std::string(),
                                const std::string& driver_version = std::string(),
                                const std::string& pci_bus_id = std::string()) {
        MetricLabels labels;
        if (!gpu.empty()) labels.emplace_back("gpu", gpu);
        if (!uuid.empty()) labels.emplace_back("gpu_uuid", uuid);
        if (!name.empty()) labels.emplace_back("name", name);
        if (!driver_version.empty()) labels.emplace_back("driver_version", driver_version);
        if (!pci_bus_id.empty()) labels.emplace_back("pci_bus_id", pci_bus_id);
        return labels;
      };

      const auto addCachedMetric = [this](const std::string& name, double value, MetricLabels labels = {}) {
        if (value < 0.0) return;
        cached_gpu_metrics_.push_back(MetricPoint{name, value, std::move(labels)});
      };

      const auto addGPUDeviceMetrics = [&](const std::string& gpu, const std::string& uuid,
                                           const std::string& name,
                                           const std::string& driver_version,
                                           const std::string& pci_bus_id, double util_sm_percent,
                                           double util_mem_percent, double mem_total_mib,
                                           double mem_used_mib, double mem_free_mib,
                                           double mem_used_percent, double temperature_celsius,
                                           double power_draw_watts, double power_limit_watts,
                                           double pcie_gen, double pcie_width, double pcie_gen_max,
                                           double pcie_width_max, double pcie_rx_mb_s,
                                           double pcie_tx_mb_s, double bar1_total_mib,
                                           double bar1_used_mib, double bar1_free_mib,
                                           double ecc_single_bit_errors_total,
                                           double ecc_double_bit_errors_total, double process_count,
                                           double context_count,
                                           double throttle_active_any, double throttle_thermal,
                                           double throttle_power, double throttle_idle,
                                           double nvlink_links, double energy_consumption_joules) {
        const MetricLabels base_labels =
            gpuLabels(gpu, uuid, name, driver_version, pci_bus_id);
        cached_gpu_metrics_.push_back({"probe_core_gpu_info", 1.0, base_labels});

        addCachedMetric("probe_core_gpu_utilization_sm_percent", util_sm_percent,
                        gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_utilization_memory_percent", util_mem_percent,
                        gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_memory_total_mib", mem_total_mib, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_memory_used_mib", mem_used_mib, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_memory_free_mib", mem_free_mib, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_memory_used_percent", mem_used_percent,
                        gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_temperature_celsius", temperature_celsius,
                        gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_power_draw_watts", power_draw_watts,
                        gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_power_limit_watts", power_limit_watts,
                        gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_pcie_gen", pcie_gen, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_pcie_width", pcie_width, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_pcie_gen_max", pcie_gen_max, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_pcie_width_max", pcie_width_max, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_pcie_rx_mb_s", pcie_rx_mb_s, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_pcie_tx_mb_s", pcie_tx_mb_s, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_bar1_memory_total_mib", bar1_total_mib,
                        gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_bar1_memory_used_mib", bar1_used_mib,
                        gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_bar1_memory_free_mib", bar1_free_mib,
                        gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_ecc_single_bit_errors_total",
                        ecc_single_bit_errors_total, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_ecc_double_bit_errors_total",
                        ecc_double_bit_errors_total, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_process_count", process_count, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_context_count", context_count, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_kernel_active_contexts", context_count,
                        gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_throttle_active_any", throttle_active_any, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_throttle_thermal_any", throttle_thermal, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_throttle_power_any", throttle_power, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_throttle_idle", throttle_idle, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_nvlink_links", nvlink_links, gpuLabels(gpu, uuid));
        addCachedMetric("probe_core_gpu_energy_consumption_joules", energy_consumption_joules, gpuLabels(gpu, uuid));

        const double current_bw = pcieBandwidthMBps(pcie_gen, pcie_width);
        if (current_bw > 0.0) {
          addCachedMetric("probe_core_gpu_pcie_bandwidth_theoretical_mb_s", current_bw,
                          gpuLabels(gpu, uuid));
          addCachedMetric("probe_core_gpu_pcie_rx_utilization_percent",
                          (pcie_rx_mb_s >= 0.0 ? std::clamp((pcie_rx_mb_s / current_bw) * 100.0, 0.0, 100.0)
                                               : -1.0),
                          gpuLabels(gpu, uuid));
          addCachedMetric("probe_core_gpu_pcie_tx_utilization_percent",
                          (pcie_tx_mb_s >= 0.0 ? std::clamp((pcie_tx_mb_s / current_bw) * 100.0, 0.0, 100.0)
                                               : -1.0),
                          gpuLabels(gpu, uuid));
          addCachedMetric("probe_core_gpu_pcie_link_utilization_percent",
                          ((pcie_rx_mb_s >= 0.0 || pcie_tx_mb_s >= 0.0)
                               ? std::clamp(((std::max(pcie_rx_mb_s, 0.0) +
                                              std::max(pcie_tx_mb_s, 0.0)) /
                                             (2.0 * current_bw)) *
                                                100.0,
                                            0.0, 100.0)
                               : -1.0),
        ...
        const double max_bw = pcieBandwidthMBps(pcie_gen_max, pcie_width_max);
        if (max_bw > 0.0) {
          addCachedMetric("probe_core_gpu_pcie_bandwidth_max_mb_s", max_bw,
                          gpuLabels(gpu, uuid));
        }
      };

      bool collected = false;
      std::string collection_source = "unavailable";
      std::unordered_map<std::string, std::string> gpu_index_by_uuid;

      probe_core::GPUNVMLSamples nvml_samples;
      if (probe_core::CollectNVMLSamples(&nvml_samples) && !nvml_samples.devices.empty()) {
        collection_source = "nvml";
        collected = true;
        cached_gpu_metrics_.push_back({"probe_core_gpu_count",
                                       static_cast<double>(nvml_samples.devices.size()), {}});
        for (const auto& sample : nvml_samples.devices) {
          if (!sample.uuid.empty()) {
            gpu_index_by_uuid[sample.uuid] = sample.index;
          }
          addGPUDeviceMetrics(
              sample.index, sample.uuid, sample.name, sample.driver_version, sample.pci_bus_id,
              sample.util_sm_percent, sample.util_mem_percent, sample.memory_total_mib,
              sample.memory_used_mib, sample.memory_free_mib, sample.memory_pressure_percent,
              sample.temperature_celsius, sample.power_draw_watts, sample.power_limit_watts,
              sample.pcie_gen, sample.pcie_width, sample.pcie_gen_max, sample.pcie_width_max,
              sample.pcie_rx_mb_s, sample.pcie_tx_mb_s, sample.bar1_total_mib,
              sample.bar1_used_mib, sample.bar1_free_mib, sample.ecc_single_bit_errors_total,
              sample.ecc_double_bit_errors_total, sample.process_count, sample.context_count,
              sample.throttle_active_any, sample.throttle_thermal, sample.throttle_power,
              sample.throttle_idle, sample.nvlink_links, sample.energy_consumption_joules);
        }
        for (const auto& sample : nvml_samples.processes) {
          MetricLabels labels;
          labels.emplace_back("pid", sample.pid);
          if (!sample.gpu_index.empty()) labels.emplace_back("gpu", sample.gpu_index);
          if (!sample.gpu_uuid.empty()) labels.emplace_back("gpu_uuid", sample.gpu_uuid);
          const std::string process_name = readGPUProcessName(sample.pid);
          if (!process_name.empty()) labels.emplace_back("process", process_name);
            addCachedMetric("probe_core_gpu_process_memory_mib", sample.memory_used_mib, labels);
            addCachedMetric("probe_core_gpu_process_mem_util_percent", sample.memory_util_percent,
                          labels);
            addCachedMetric("probe_core_gpu_process_context_active", sample.context_active, labels);
          }
      } else {
        collection_source = "nvml_unavailable";
      }

      cached_gpu_metrics_.push_back(
          {"probe_core_gpu_collection_source", collected ? 1.0 : 0.0,
           {{"source", collection_source}}});
      cached_gpu_metrics_.push_back({"probe_core_gpu_probe_success", collected ? 1.0 : 0.0, {}});
  ...
  std::vector<ProcRow> collectProcessMetrics(double elapsed, std::vector<MetricPoint>* metrics,
    ...
    if (process_kernel_collector_ != nullptr && process_kernel_collector_->available() &&
        opts_.host_mode != HostCollectionMode::kProc) {
      if (shouldRefresh(last_process_reconcile_seq_,
                        effective_host_proc_fallback_interval_samples_) ||
          cached_process_rows_.empty()) {
        process_kernel_collector_->reconcileFromProc();
        last_process_reconcile_seq_ = sample_seq_;
      }

      auto kernel_rows =
          process_kernel_collector_->collect(elapsed_safe, lightweight_enrichment);
      if (!kernel_rows.empty()) {
        std::vector<ProcRow> rows;
        rows.reserve(kernel_rows.size());
        for (const auto& sample : kernel_rows) {
          ProcRow row;
          row.pid = sample.pid;
          row.name = sample.name;
          row.cpu_percent = sample.cpu_percent;
          row.rss_bytes = sample.rss_bytes;
          row.io_read_bps = sample.io_read_bps;
          row.io_write_bps = sample.io_write_bps;
          row.voluntary_ctx = sample.voluntary_ctx;
          row.nonvoluntary_ctx = sample.nonvoluntary_ctx;
          row.minor_faults = sample.minor_faults;
          row.major_faults = sample.major_faults;
          row.sched_run_seconds_total = sample.sched_run_seconds_total;
          row.sched_wait_seconds_total = sample.sched_wait_seconds_total;
          row.sched_wait_ratio = sample.sched_wait_ratio;
          row.block_io_delay_seconds_total = sample.block_io_delay_seconds_total;
          row.block_io_delay_seconds_per_second = sample.block_io_delay_seconds_per_second;
          row.pss_bytes = sample.pss_bytes;
          row.net_connections = sample.net_connections;
          row.net_tx_queue_bytes = sample.net_tx_queue_bytes;
          row.net_rx_queue_bytes = sample.net_rx_queue_bytes;
          rows.push_back(std::move(row));
        }
        for (const auto& row : rows) {
          emitProcessRowMetrics(row, metrics);
        }
        addMetric(metrics, "probe_core_process_collection_source", 1.0,
                  {{"source", "kernel_taskstats"}});
        return rows;
      }
    }
    ...
    probe_core::ScopedDir proc(opendir(kProcPath));
    ...
    const auto cpu_desc = [](const ProcRow& a, const ProcRow& b) {
      return a.cpu_percent > b.cpu_percent;
    };
    if (static_cast<int>(rows.size()) > opts_.topk) {
      auto nth = rows.begin() + opts_.topk;
      std::nth_element(rows.begin(), nth, rows.end(), cpu_desc);
      rows.resize(opts_.topk);
    }
    std::sort(rows.begin(), rows.end(), cpu_desc);

    if (!lightweight_enrichment) {
      const auto socket_queues = readSocketQueuesByInode();
      for (auto& row : rows) {
        row.pss_bytes = readPssBytes(row.pid);
        const auto inodes = listSocketInodesForPid(row.pid);
        for (const auto& inode : inodes) {
          const uint64_t inode_value = parseU64(inode);
          const auto it = socket_queues.find(inode_value);
          if (it == socket_queues.end()) continue;
          row.net_connections++;
...
void printUsage() {
  ...
  std::cerr << "Usage: sre-probe-core [options]\n"
            ...
            << "  --host-mode <auto|kernel|proc_fallback> host collector mode (default auto)\n"
            ...
            << "  --gpu-interval-samples <n>  GPU polling frequency in samples (default 1)\n"

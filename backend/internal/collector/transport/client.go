...
var (
	...
	// ErrUnexpectedAckBatchID indicates controller acknowledged a different batch than was sent.
	ErrUnexpectedAckBatchID = errors.New("unexpected ack batch id")
)
...
// AuthConfig controls bearer-token authentication for collector -> controller ingest.
type AuthConfig struct {
	Enabled     bool
	BearerToken string
}

// DrainOptions bounds per-cycle backlog draining so controller/network stalls
// do not monopolize collector time.
...
func New(cfg Config, logger *zap.Logger) (*Client, error) {
	...
	if !cfg.TLS.Enabled && !cfg.AllowPlaintext {
		return nil, &Error{Kind: ErrorKindConfig, Err: errors.New("transport plaintext is disabled but tls is not enabled")}
	}
	if cfg.Auth.Enabled && strings.TrimSpace(cfg.Auth.BearerToken) == "" {
		return nil, &Error{Kind: ErrorKindConfig, Err: errors.New("transport auth is enabled but bearer token is empty")}
	}
	client := &Client{
		cfg:    cfg,
		logger: logger.With(zap.String("component", "transport")),
		conns:  make(map[string]*grpc.ClientConn, len(cfg.Endpoints)),
	}
...
func (c *Client) ApplyConfig(cfg Config) error {
	...
	if !cfg.TLS.Enabled && !cfg.AllowPlaintext {
		return &Error{Kind: ErrorKindConfig, Err: errors.New("transport plaintext is disabled but tls is not enabled")}
	}
	if cfg.Auth.Enabled && strings.TrimSpace(cfg.Auth.BearerToken) == "" {
		return &Error{Kind: ErrorKindConfig, Err: errors.New("transport auth is enabled but bearer token is empty")}
	}
...
// Close releases cached transport resources.
...
func (c *Client) Drain(ctx context.Context, sp *spool.Spool, send func([]byte) (string, error)) error {
	return c.DrainWithOptions(ctx, sp, send, DrainOptions{})
}

// DrainWithOptions attempts to flush the spool while honoring per-cycle limits.
func (c *Client) DrainWithOptions(ctx context.Context, sp *spool.Spool, send func([]byte) (string, error), options DrainOptions) error {
	start := time.Now()
	drained := 0
	for {
		if err := ctx.Err(); err != nil {
		...
		if options.MaxRecords > 0 && drained >= options.MaxRecords {
			return nil
		}
		if options.MaxDuration > 0 && time.Since(start) >= options.MaxDuration {
			return nil
		}

		payload, nextOffset, err := sp.Next()
		if err != nil {
			if errors.Is(err, spool.ErrCorruptSegment) {
				c.logger.Warn("spool tail was truncated or corrupted; continuing after recovery")
				continue
			}
...
func (c *Client) sendToEndpoint(ctx context.Context, attempt int, endpoint string, payload []byte) (*telemetryv1.Ack, error) {
	...
	defer cancel()
	if cfg.Auth.Enabled {
		rpcCtx = metadata.AppendToOutgoingContext(rpcCtx, "authorization", "Bearer "+strings.TrimSpace(cfg.Auth.BearerToken))
	}
	...
	stream, err := client.Push(rpcCtx, callOpts...)
	if err != nil {
	...
	if err := validateAck(batch.GetBatchId(), ack); err != nil {
		return nil, &Error{Kind: ErrorKindReceive, Endpoint: endpoint, Attempt: attempt, Err: err}
	}
...
func validateAck(batchID string, ack *telemetryv1.Ack) error {
	if ack == nil || strings.TrimSpace(ack.BatchId) == "" {
		return ErrEmptyAckBatchID
	}
	if expected := strings.TrimSpace(batchID); expected != "" && !strings.EqualFold(strings.TrimSpace(ack.BatchId), expected) {
		return fmt.Errorf("%w: expected %q got %q", ErrUnexpectedAckBatchID, expected, strings.TrimSpace(ack.BatchId))
	}
	return nil
}
...
func (c *Client) dial(ctx context.Context, endpoint string) (*grpc.ClientConn, error) {
	c.connMu.Lock()
	if conn := c.conns[endpoint]; conn != nil {
		c.connMu.Unlock()
		return conn, nil
	}
	c.connMu.Unlock()
...
func normalizeConfig(cfg Config) Config {
	...
	cfg.Auth.BearerToken = strings.TrimSpace(cfg.Auth.BearerToken)
	return cfg
}
...
func (c *Client) closeConnections() {
	...
	for endpoint, conn := range c.conns {
		...
		if err := conn.Close(); err != nil {
			c.logger.Debug("close cached transport connection failed",
				zap.String("endpoint", endpoint),
				zap.Error(err),
			)
		}

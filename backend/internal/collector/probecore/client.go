...
func (c *Client) runOnce(ctx context.Context) error {
	...
	if c.cfg.Nice != 0 && cmd.Process != nil {
		if err := unix.Setpriority(unix.PRIO_PROCESS, cmd.Process.Pid, c.cfg.Nice); err != nil {
			c.logger.Debug("failed to lower probe-core scheduling priority",
				zap.Int("pid", cmd.Process.Pid),
				zap.Int("nice", c.cfg.Nice),
				zap.Error(err),
			)
		}
	}
...
func (c *Client) buildArgs() []string {
	...
	args = append(args,
		...
		"--process-interval-samples", strconv.Itoa(maxInt(1, c.cfg.ProcessIntervalSamples)),
		"--host-proc-fallback-interval-samples", strconv.Itoa(maxInt(1, c.cfg.HostProcFallbackIntervalSamples)),
		"--pressure-interval-samples", strconv.Itoa(maxInt(1, c.cfg.PressureIntervalSamples)),
		"--netlink-interval-samples", strconv.Itoa(maxInt(1, c.cfg.NetlinkIntervalSamples)),
...
func (cfg Config) validate() error {
	...
	if cfg.ProcessIntervalSamples <= 0 {
		return fmt.Errorf("probe-core process_interval_samples must be > 0")
	}
	if cfg.HostProcFallbackIntervalSamples <= 0 {
		return fmt.Errorf("probe-core host_proc_fallback_interval_samples must be > 0")
	}
	if cfg.PressureIntervalSamples <= 0 {
		return fmt.Errorf("probe-core pressure_interval_samples must be > 0")
	}
	if cfg.NetlinkIntervalSamples <= 0 {
		return fmt.Errorf("probe-core netlink_interval_samples must be > 0")
	}
	if cfg.QueueDepth <= 0 {
	...
	if cfg.Nice < -20 || cfg.Nice > 19 {
		return fmt.Errorf("probe-core nice must be between -20 and 19")
	}
...
func (cfg Config) withDefaults() Config {
	if cfg.ProcessIntervalSamples <= 0 {
		cfg.ProcessIntervalSamples = 2
	}
	if cfg.HostProcFallbackIntervalSamples <= 0 {
		cfg.HostProcFallbackIntervalSamples = 10
	}
	if cfg.PressureIntervalSamples <= 0 {
		cfg.PressureIntervalSamples = 3
	}
	if cfg.NetlinkIntervalSamples <= 0 {
		cfg.NetlinkIntervalSamples = 2
	}
	if cfg.GPUIntervalSamples <= 0 {
		cfg.GPUIntervalSamples = 1
	}
	return cfg
}

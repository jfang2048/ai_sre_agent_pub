...
func resolveLiveProbeCoreBinaryPath(t *testing.T) string {
	...
	if path := os.Getenv("SRE_PROBE_CORE_BINARY_PATH"); path != "" {
		if stat, err := os.Stat(path); err == nil && !stat.IsDir() && probeCoreBinaryRunnable(path) {
			return path
		}
	...
	for i := 0; i < 8; i++ {
		...
		if stat, statErr := os.Stat(candidate); statErr == nil && !stat.IsDir() && probeCoreBinaryRunnable(candidate) {
			return candidate
		}
...
func probeCoreBinaryRunnable(path string) bool {
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	cmd := exec.CommandContext(ctx, path, "--list-collectors")
	return cmd.Run() == nil
}

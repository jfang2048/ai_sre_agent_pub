...
func TestLoadConfigAppliesTransportAuthEnvOverrides(t *testing.T) {
	t.Setenv("SRE_COLLECTOR_TRANSPORT_AUTH_ENABLED", "true")
	t.Setenv("SRE_COLLECTOR_TRANSPORT_AUTH_BEARER_TOKEN_ENV", "TEST_SRE_COLLECTOR_INGEST_TOKEN")
	t.Setenv("TEST_SRE_COLLECTOR_INGEST_TOKEN", "collector-ingest-token")

	cfg, err := LoadConfig("")
	require.NoError(t, err)
	require.True(t, cfg.Transport.Auth.Enabled)
	require.Equal(t, "TEST_SRE_COLLECTOR_INGEST_TOKEN", cfg.Transport.Auth.BearerTokenEnv)
	require.Equal(t, "collector-ingest-token", cfg.Transport.Auth.BearerToken)
}

func TestLoadConfigRejectsEnabledTransportAuthWithoutToken(t *testing.T) {
	t.Setenv("SRE_COLLECTOR_TRANSPORT_AUTH_ENABLED", "true")
	t.Setenv("SRE_COLLECTOR_TRANSPORT_AUTH_BEARER_TOKEN_ENV", "TEST_SRE_COLLECTOR_MISSING_TOKEN")

	_, err := LoadConfig("")
	require.Error(t, err)
	require.ErrorContains(t, err, "transport auth token env")
	require.ErrorContains(t, err, "TEST_SRE_COLLECTOR_MISSING_TOKEN")
}

func TestLoadConfigAppliesMinimalPrivilegeProfile(t *testing.T) {
	t.Setenv("SRE_COLLECTOR_PRIVILEGE_PROFILE", "minimal")

	cfg, err := LoadConfig("")
	require.NoError(t, err)
	require.Equal(t, PrivilegeProfileMinimal, cfg.PrivilegeProfile)
	require.Equal(t, runtimeModeLimited, cfg.RuntimeMode)
	require.False(t, cfg.EBPF.Enabled)
	require.False(t, cfg.ProbeCore.Enabled)
	require.False(t, cfg.Security.Enabled)
	require.Empty(t, cfg.LogPaths)
}

func TestLoadConfigRejectsTransportWhenPlaintextDisabledWithoutTLS(t *testing.T) {
	t.Setenv("SRE_COLLECTOR_ALLOW_PLAINTEXT", "false")
	t.Setenv("SRE_COLLECTOR_TLS_ENABLED", "false")

	_, err := LoadConfig("")
	require.Error(t, err)
	require.ErrorContains(t, err, "transport plaintext is disabled")
}

...
func (g *GPUCollector) collectGPUProcesses(now time.Time, gpus []gpuInfo, detail bool) []Metric {
	...
	if output != "" {
		...
		for scanner.Scan() {
			...
			p := ensure(gpuID, pid)
			p.name = normalizeGPUProcessName(parts[2], pid)
	...
	if detail {
		for key, pm := range g.collectGPUProcessPMON() {
			...
			if p.name == "" && pm.command != "" {
				p.name = normalizeGPUProcessName(pm.command, pm.pid)
			}
			if pm.contextType != "" {
...
func (g *GPUCollector) collectGPUProcessPMON() map[string]*gpuPMONSample {
	...
	for scanner.Scan() {
		...
		sample := &gpuPMONSample{
			...
			command:     normalizeGPUProcessName(strings.Join(parts[7:], " "), pid),
		}
		if !isNA(parts[3]) {
...
func normalizeGPUProcessName(raw, pid string) string {
	...
	const maxProcessNameRunes = 64
	runes := []rune(name)
	if len(runes) > maxProcessNameRunes {
		name = string(runes[:maxProcessNameRunes])
	}

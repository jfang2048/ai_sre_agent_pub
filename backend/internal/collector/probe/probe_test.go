...
func TestCollectorRateCalculation(t *testing.T) {
	...
	hasRateMetric := false
	hasIOWaitMetric := false
	for _, m := range batch.Metrics {
		if strings.Contains(m.Name, "per_second") {
			hasRateMetric = true
		}
		if m.Name == "node_cpu_iowait_percent" {
			hasIOWaitMetric = true
		}
	}
	...
	if !hasIOWaitMetric {
		t.Fatal("expected node_cpu_iowait_percent metric after baseline scrape")
	}
}
...
func TestStatusResponse(t *testing.T) {
	...
	if resp.Version != cfg.Version {
		t.Errorf("Version = %s, want %s", resp.Version, cfg.Version)
	}
...
func TestWeightedLatencyQuantile(t *testing.T) {
	samples := []latencySample{
		{LatencySeconds: 0.001, Ops: 100},
		{LatencySeconds: 0.01, Ops: 10},
		{LatencySeconds: 0.1, Ops: 1},
	}
	...
	tests := []struct {
		...
		{name: "p50", quantile: 0.50, want: 0.001},
		{name: "p90", quantile: 0.90, want: 0.001},
		{name: "p95", quantile: 0.95, want: 0.01},
		{name: "p99", quantile: 0.99, want: 0.01},
		{name: "p100", quantile: 1.0, want: 0.1},
		{name: "clamp_over_one", quantile: 2.0, want: 0.1},
		{name: "clamp_below_zero", quantile: -1.0, want: 0.001},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := weightedLatencyQuantile(samples, tt.quantile)
			if got != tt.want {
				t.Fatalf("weightedLatencyQuantile(%v) = %v, want %v", tt.quantile, got, tt.want)
			}
		})
	}

	if got := weightedLatencyQuantile(nil, 0.99); got != 0 {
		t.Fatalf("weightedLatencyQuantile(nil, 0.99) = %v, want 0", got)
	}
}

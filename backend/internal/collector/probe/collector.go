...
type Collector struct {
	...
	lastSecurityCollect   time.Time
	securityMetricsCache  []Metric
	securityPortFirstSeen map[int]time.Time
	securityFileSizes     map[string]int64
	securityAuditInterval time.Duration
}
...
func NewCollector(opts ...CollectorOption) (*Collector, error) {
	...
	c := &Collector{
		...
		topNProcesses:         10,
		securityPortFirstSeen: make(map[int]time.Time),
		securityFileSizes:     make(map[string]int64),
		securityAuditInterval: 5 * time.Minute,
	}
	...
	// eBPF is mandatory in v0.5 (kernel-first runtime). Even when configuration
	// previously disabled it, we initialize the collector with fallback mode.
	{
		cfg := c.ebpfConfig
		cfg.Enabled = true
...
func (c *Collector) Collect() (*MetricBatch, error) {
	...
	if c.level >= 4 {
		...
		metrics = append(metrics, c.collectSecurityMetrics(now)...)
	}

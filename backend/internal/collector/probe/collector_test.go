...
func TestCompatibilityProbeDoesNotEmitSecurityAuditMetrics(t *testing.T) {
	c, err := NewCollector(WithLevel(4))
	if err != nil {
		t.Fatalf("NewCollector failed: %v", err)
	}
	...
	for _, metric := range batch.Metrics {
		if strings.HasPrefix(metric.Name, "node_security_") {
			t.Fatalf("compatibility probe emitted collector-owned security metric %q", metric.Name)
		}
	}
}

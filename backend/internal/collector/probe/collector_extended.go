...
func (c *Collector) collectExtendedRuntime(now time.Time, elapsed float64) []Metric {
	var metrics []Metric
...
func (c *Collector) collectHardwareSignals(now time.Time, elapsed float64) []Metric {
	var metrics []Metric
...
func (c *Collector) collectExtended(now time.Time, elapsed float64) []Metric {
	metrics := c.collectExtendedRuntime(now, elapsed)
	metrics = append(metrics, c.collectHardwareSignals(now, elapsed)...)
	return metrics
}

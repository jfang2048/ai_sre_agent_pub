...
type collectorRuntimeInspection struct {
	...
	CAPSysAdmin         bool
	CanUseEBPF          bool
	CanUsePerfEvents    bool
	CanUseTaskstats     bool
	Degraded            bool
	Reasons             []string
}
...
func detectCollectorRuntimeInspectionWithDeps(cfg Config, deps runtimeInspectorDeps) collectorRuntimeInspection {
	...
	inspection.CAPPerfmon = hasEffectiveCapability(deps, unix.CAP_PERFMON)
	inspection.CAPNetAdmin = hasEffectiveCapability(deps, unix.CAP_NET_ADMIN)
	...
	inspection.CanUsePerfEvents = inspection.CAPPerfmon || inspection.CAPSysAdmin
	inspection.CanUseTaskstats = inspection.CAPNetAdmin || inspection.CAPSysAdmin
	...
	if !inspection.CanUsePerfEvents {
		inspection.Reasons = append(inspection.Reasons, "perfmon_capability_unavailable")
	}
	if !inspection.CanUseTaskstats {
		inspection.Reasons = append(inspection.Reasons, "net_admin_capability_unavailable")
	}

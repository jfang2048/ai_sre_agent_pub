...
type sourcePipelineStatus struct {
	PrimaryStarted      bool
	CompatibilityActive bool
...
func (p *sourcePipeline) collectFromPrimary(now time.Time, cfg Config) (sourceCollection, error) {
	...
	if batch, ok := p.primary.Latest(cfg.ProbeCore.StaleAfter); ok && batch != nil {
		metrics, processes := convertProbeCoreBatch(batch, now, cfg.ProbeCore.EmitRawAliasedMetrics)
		return sourceCollection{
			metrics:         metrics,
			processes:       processes,
			source:          "probe_core",
...
func (p *sourcePipeline) status() sourcePipelineStatus {
	if p == nil {
	...
	defer p.mu.RUnlock()
	return sourcePipelineStatus{
		PrimaryStarted:      p.primaryStarted,
		CompatibilityActive: p.compatibilityActive,
		FallbackReason:      p.lastFallbackReason,
	}
}

...
func TestBuildLabelsSanitizesLengthAndControlRunes(t *testing.T) {
	longValue := strings.Repeat("x", maxLabelValueRunes+30)
	labels := buildLabels(map[string]string{
		" key\n": "\t" + longValue + "\r",
	})
	require.Len(t, labels, 1)
	require.Equal(t, "key", labels[0].Key)
	require.Len(t, []rune(labels[0].Value), maxLabelValueRunes)
}

func TestBuildLabelsCapsByteLengthForMultiByteInput(t *testing.T) {
	labels := buildLabels(map[string]string{
		"k": strings.Repeat("测", maxLabelValueRunes),
	})
	require.Len(t, labels, 1)
	require.True(t, utf8.ValidString(labels[0].Value))
	require.LessOrEqual(t, len(labels[0].Value), maxLabelValueRunes)
}

func TestSanitizeTelemetryMetricsEnforcesLabelByteLimits(t *testing.T) {
	metrics := []*telemetryv1.Metric{
		{
			Name: "test_metric",
			Labels: []*telemetryv1.Label{
				{Key: "k", Value: strings.Repeat("测", maxLabelValueRunes)},
				{Key: "normal", Value: "ok"},
			},
		},
	}
	...
	for _, label := range metrics[0].Labels {
		require.LessOrEqual(t, len(label.Key), maxLabelKeyRunes)
		require.LessOrEqual(t, len(label.Value), maxLabelValueRunes)
		require.True(t, utf8.ValidString(label.Value))
	}
}

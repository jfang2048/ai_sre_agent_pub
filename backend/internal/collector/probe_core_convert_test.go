...
func TestConvertProbeCoreBatchAddsCompatibilityAliases(t *testing.T) {
	batch := &probeipcv1.ProbeBatch{
		...
		Metrics: []*probeipcv1.Metric{
			...
			{Name: "probe_core_gpu_throttle_active_any", Value: 1, Labels: []*probeipcv1.Label{{Key: "gpu", Value: "0"}, {Key: "gpu_uuid", Value: "GPU-123"}}},
			{Name: "probe_core_gpu_energy_consumption_joules", Value: 1000, Labels: []*probeipcv1.Label{{Key: "gpu", Value: "0"}, {Key: "gpu_uuid", Value: "GPU-123"}}},
			{Name: "probe_core_gpu_nvlink_links", Value: 6, Labels: []*probeipcv1.Label{{Key: "gpu", Value: "0"}, {Key: "gpu_uuid", Value: "GPU-123"}}},
	...
	metrics, processes := convertProbeCoreBatch(batch, time.Now(), false)
	...
	require.Contains(t, metricMap, "node_gpu_throttle_active_any")
	require.Contains(t, metricMap, "node_gpu_energy_consumption_joules")
	require.Contains(t, metricMap, "node_gpu_nvlink_links")
	...
	require.NotContains(t, metricMap, "probe_core_cpu_usage_percent")
	require.NotContains(t, metricMap, "probe_core_memory_total_bytes")
	require.NotContains(t, metricMap, "probe_core_disk_await_ms")
	require.NotContains(t, metricMap, "probe_core_network_tcp_retransmissions_per_sec")
	require.NotContains(t, metricMap, "probe_core_gpu_count")
}

func TestConvertProbeCoreBatchCanRetainRawAliasedMetricsWhenEnabled(t *testing.T) {
	batch := &probeipcv1.ProbeBatch{
		CollectedAtUnixNano: time.Now().UnixNano(),
		Metrics: []*probeipcv1.Metric{
			{Name: "probe_core_cpu_usage_percent", Value: 88.5},
			{Name: "probe_core_pressure_io_full_avg10", Value: 3.1},
			{Name: "probe_core_backpressure_queue_depth", Value: 2},
		},
	}

	metrics, _ := convertProbeCoreBatch(batch, time.Now(), true)
	metricMap := flattenMetrics(metrics)
	require.Contains(t, metricMap, "probe_core_cpu_usage_percent")
	require.Contains(t, metricMap, "probe_core_pressure_io_full_avg10")
	require.Contains(t, metricMap, "probe_core_backpressure_queue_depth")
	require.Contains(t, metricMap, "node_cpu_usage_percent")
	require.Contains(t, metricMap, "node_pressure_io_full_avg10")
}

func TestConvertProbeCoreBatchKeepsNonAliasedRawMetrics(t *testing.T) {
	batch := &probeipcv1.ProbeBatch{
		CollectedAtUnixNano: time.Now().UnixNano(),
		Metrics: []*probeipcv1.Metric{
			{Name: "probe_core_backpressure_queue_depth", Value: 4},
			{Name: "probe_core_cgroup_cpu_throttled_ratio", Value: 0.12},
		},
	}

	metrics, _ := convertProbeCoreBatch(batch, time.Now(), false)
	metricMap := flattenMetrics(metrics)
	require.Contains(t, metricMap, "probe_core_backpressure_queue_depth")
	require.Contains(t, metricMap, "probe_core_cgroup_cpu_throttled_ratio")
}

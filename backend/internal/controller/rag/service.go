...
func NewService(cfg Config, logger *zap.Logger) (*Service, error) {
	...
	if index, err := loadIndex(cfg.IndexPath); err == nil {
		...
		startupIndexWarning = err.Error()
		if quarantinedPath, qErr := quarantineInvalidIndex(cfg.IndexPath); qErr != nil {
			service.logger.Warn("rag index failed validation and could not be quarantined",
				zap.String("path", cfg.IndexPath),
				zap.Error(err),
				zap.Error(qErr),
			)
		} else if strings.TrimSpace(quarantinedPath) != "" {
			startupIndexWarning = fmt.Sprintf("%s (quarantined to %s)", err.Error(), quarantinedPath)
			service.stats.LastError = startupIndexWarning
			service.logger.Warn("rag index failed validation; quarantined and continuing without it",
				zap.String("path", cfg.IndexPath),
				zap.String("quarantined_path", quarantinedPath),
				zap.Error(err),
			)
		}
	}
	...
	if strings.TrimSpace(startupIndexWarning) != "" {
		service.mu.Lock()
		service.stats.LastError = startupIndexWarning
		service.mu.Unlock()
	}
...
func quarantineInvalidIndex(indexPath string) (string, error) {
	...
	dir := storagePath(indexPath)
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return "", err
	}
	target := filepath.Join(dir, fmt.Sprintf("index.corrupt-%s.json", time.Now().UTC().Format("20060102T150405.000000000Z")))
...
// IsStale reports whether the index is older than the configured staleness threshold.
// Default threshold is 24h if StalenessThreshold is not set.
func (s *Service) IsStale() bool {
	...
	threshold := s.cfg.StalenessThreshold
	if threshold <= 0 {
		threshold = 24 * time.Hour
	}
	return time.Since(s.index.UpdatedAt) > threshold
}

// LastIndexedAt returns the last time the index was updated.
func (s *Service) LastIndexedAt() time.Time {
	s.mu.RLock()
	defer s.mu.RUnlock()
	if s.index == nil {
		return time.Time{}
...
func (s *Service) Query(ctx context.Context, req QueryRequest) (QueryResult, error) {
	...
	lexicalScores := index.lexicalScores(plan.Tokens)
	...
	if strings.EqualFold(cfg.RetrievalMode, "lexical") {
		...
		queryVector, err := embedder.EmbedQuery(ctx, plan.NormalizedQuery)
		...
		if err != nil {
			...
			s.logger.Warn("rag external vector search failed; falling back to local vector index",
				...
				zap.Error(err))
			vectorScores = index.vectorScores(plan.NormalizedQuery, cfg, embedder)
		} else {
	...
	result := index.buildQueryResult(plan, cfg, topK, combineScores(cfg.RetrievalMode, lexicalScores, vectorScores))
...
func (s *Service) embedChunks(ctx context.Context, chunks []Chunk) error {
	...
	for _, chunk := range chunks {
		texts = append(texts, effectiveChunkEmbeddingText(chunk))
	}
...
func (s *Service) refreshStatsLocked(lastError string) {
	...
	if s.index == nil {
		...
		s.stats.KnowledgeTypes = nil
		s.stats.CaseTypes = nil
		return
	}
	...
	s.stats.KnowledgeTypes = make(map[string]int, len(s.index.knowledgeTypeCount))
	for key, value := range s.index.knowledgeTypeCount {
		s.stats.KnowledgeTypes[key] = value
	}
	s.stats.CaseTypes = make(map[string]int, len(s.index.caseTypeCount))
	for key, value := range s.index.caseTypeCount {
		s.stats.CaseTypes[key] = value
	}
}
...
func cloneStats(in Stats) Stats {
	...
	if in.KnowledgeTypes != nil {
		out.KnowledgeTypes = make(map[string]int, len(in.KnowledgeTypes))
		for key, value := range in.KnowledgeTypes {
			out.KnowledgeTypes[key] = value
		}
	}
	if in.CaseTypes != nil {
		out.CaseTypes = make(map[string]int, len(in.CaseTypes))
		for key, value := range in.CaseTypes {
			out.CaseTypes[key] = value
		}
	}
	return out
}

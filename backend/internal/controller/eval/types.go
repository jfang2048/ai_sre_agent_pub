...
type WorkflowCaseRunOptions struct {
	ConfigOverride WorkflowConfigOverride
}

// WorkflowCaseExecution exposes the real runtime artifacts produced by one
// workflow evaluation case so higher-level evaluators can score the full system.
type WorkflowCaseExecution struct {
	Case            IncidentCase                      `json:"case"`
	Result          WorkflowCaseResult                `json:"result"`

...
func (c *Controller) buildDataPathDiagnostics(collectorID string) dataPathDiagnosticsResponse {
	...
	for _, node := range filtered {
		...
		history := c.ingestStore.MetricHistory(node.CollectorID, time.Now().Add(-dataPathAnomalyWindow), dataPathAnomalySamples)
...
func probeCorePressureScore(node *ingest.NodeSnapshot) (float64, map[string]float64, []string, bool) {
	...
	if source != "probe_core" {
		factors = append(factors, "Collector is not using probe-core as active source; running on Go fallback path.")
	}

...
func TestIncidentReportsClassifyStorageBottleneckFromCoupledPressure(t *testing.T) {
	cfg := DefaultConfig()
	cfg.AnalysisInterval = time.Hour
	...
	for i := 0; i < 24; i++ {
		...
		samples = append(samples,
			MetricSample{Name: "node_cpu_usage_percent", Value: cpu, Timestamp: ts},
			MetricSample{Name: "node_cpu_iowait_percent", Value: iowait, Timestamp: ts},
			MetricSample{Name: "node_disk_request_latency_p99_seconds", Value: latency, Timestamp: ts},
			MetricSample{Name: "node_disk_queue_depth_total", Value: queueDepth, Timestamp: ts},
			MetricSample{Name: "node_memory_Used_bytes", Value: 18 * 1024 * 1024 * 1024, Timestamp: ts},
			MetricSample{Name: "node_memory_MemTotal_bytes", Value: 64 * 1024 * 1024 * 1024, Timestamp: ts},
		)
	}

	engine.IngestMetrics("node-storage", samples)
	engine.runAnalysis()

	reports := engine.GetIncidentReports("node-storage", 10)
	if len(reports) == 0 {
		t.Fatalf("expected incident reports")
	}
	report := reports[0]
	if report.Classification != IncidentClassStorageBottleneck {
		t.Fatalf("expected %s classification, got %s", IncidentClassStorageBottleneck, report.Classification)
	}
	if !strings.Contains(strings.ToLower(report.ProbableCause), "storage") && !strings.Contains(strings.ToLower(report.ProbableCause), "disk") {
		t.Fatalf("expected storage-related probable cause, got %q", report.ProbableCause)
	}
}

...
func classifyIncident(metrics map[string]float64, trends map[string]string, anomalies []Anomaly, correlations []Correlation, logs []LogSummary) (IncidentClass, string, float64, string, []string) {
	...
	communicationCongestion := netThroughput >= 50*1024*1024 || netDrops > 0 || retransRate >= 0.5 || hasCorrelation(correlations, "network", "latency")
	storageBottleneck := ioHigh && (diskQueueDepth >= 30.0 || diskLatency >= 80.0)
	...
	gpuStarvation := gpuLowUtil && (cpuHigh || ioHigh || communicationCongestion || pcieRx > 8000 || pcieTx > 8000)
	...
	case gpuStarvation:
		return IncidentClassGPUStarvation,
			"GPU workers are allocated but remain underutilized due to upstream contention",
			0.76,
			"node_gpu_utilization_sm_avg_percent",
			[]string{
				"profile input pipeline and host preprocessing",
				"reduce host contention or pin feeder threads",
				"verify PCIe and network path to GPU workers",
			}
	case dataPipelineStall:
		return IncidentClassDataPipelineStall,
			"data pipeline is stalling before GPU/compute stages",
			0.72,
	...
	case storageBottleneck:
		return IncidentClassStorageBottleneck,
			"storage queueing and latency indicate backend disk saturation",
			0.80,
			"disk_request_latency_p99_ms",
			[]string{
				"reduce random IO and batch writes",
				"move hot workload to faster storage tier",
				"inspect top processes by IO wait and queue depth",
			}
	case ioHigh:
		return IncidentClassIOBottleneck,
			"disk IO throughput/latency is constraining workload progress",
			0.71,
	...
	case communicationCongestion:
		return IncidentClassCommunicationCongest,
			"network path shows congestion signs (throughput, drops, retransmits, or latency coupling)",
			0.74,
			"node_network_receive_bytes_per_second",
			[]string{
				"inspect retransmissions/drops and link errors",
	...
	case memoryPressure:
		return IncidentClassMemoryPressure,
			"memory headroom is exhausted and reclaim/swap or OOM is active",
			0.79,
			"memory_used_percent",
			[]string{
				"identify leaking or over-allocating processes",
				"tighten memory requests/limits with workload profiling",

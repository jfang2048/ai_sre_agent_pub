...
func NewManager(cfg Config, static []StaticProbe, store SnapshotProvider, logger *zap.Logger) *Manager {
	...
	for _, item := range static {
		...
		item.Metadata = cloneLabels(item.Metadata)
...
// Reload updates hot-reloadable inventory settings and static targets in place.
func (m *Manager) Reload(cfg Config, static []StaticProbe) {
	if m == nil {
		return
	}
	if cfg.HeartbeatTTL <= 0 {
...
func (m *Manager) mergeProbes(now time.Time) []Probe {
	...
	for id, item := range staticCopy {
		...
		entry.Hostname = firstNonEmpty(strings.TrimSpace(item.Hostname), entry.Hostname)
		entry.Address = firstNonEmpty(StaticProbeEndpoint(item), entry.Address)
		entry.Port = maxInt(entry.Port, item.Port)
		entry.Enabled = item.Enabled
		entry.Configured = true
		entry.Labels = mergeLabels(entry.Labels, item.Labels)
		entry.Tags = mergeStrings(entry.Tags, item.Tags)
		entry.Auth = mergeAuth(entry.Auth, item.Auth)
		entry.Metadata = mergeLabels(entry.Metadata, item.Metadata)
		addSource(id, "static")
	}
	...
	for id, entry := range probes {
		...
		entry.Healthy = !lastSeen.IsZero() && now.Sub(lastSeen) <= cfg.HeartbeatTTL
		if !entry.Enabled {
			entry.Healthy = false
		}
		...
		entry.Metadata = cloneLabels(entry.Metadata)
...
func mergeStrings(dst, src []string) []string {
	return cloneStrings(append(append([]string(nil), dst...), src...))
}

func mergeAuth(dst, src TargetAuth) TargetAuth {
	if strings.TrimSpace(dst.Mode) == "" {
		dst.Mode = strings.TrimSpace(src.Mode)
	}
	if strings.TrimSpace(dst.ServerName) == "" {
		dst.ServerName = strings.TrimSpace(src.ServerName)
	}
	if strings.TrimSpace(dst.TokenEnv) == "" {

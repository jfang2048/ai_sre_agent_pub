...
func TestPlaybookRunnerRunMarksReadAndWriteStepsSuccessful(t *testing.T) {
	executor := NewExecutor(Config{DryRun: true, MaxConcurrent: 2}, zap.NewNop())
	runner := NewPlaybookRunner(executor, zap.NewNop())
	result, err := runner.Run(t.Context(), &Playbook{
		Name: "test",
		Steps: []PlaybookStep{
			{Name: "inspect", Type: "read", Command: "kubectl get pods"},
			{Name: "restart", Type: "write", Command: "restart_pod", Parameters: map[string]interface{}{"target": "web-0"}},
		},
	}, false)
	require.NoError(t, err)
	...
	assert.Contains(t, result.Steps[1].Output, "Dry run")
}

func TestPlaybookRunnerStopsOnUnknownStep(t *testing.T) {
	executor := NewExecutor(Config{DryRun: true, MaxConcurrent: 1}, zap.NewNop())
	runner := NewPlaybookRunner(executor, zap.NewNop())
	result, err := runner.Run(t.Context(), &Playbook{
		Name:  "bad",
		Steps: []PlaybookStep{{Name: "boom", Type: "mystery", Command: "noop"}},
	}, false)
	require.Error(t, err)

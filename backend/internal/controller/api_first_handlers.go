...
func (c *Controller) handleControllerTelemetryMetrics(w http.ResponseWriter, r *http.Request) {
	...
	history := c.ingestStore.MetricHistory(collectorID, time.Now().Add(-window), limit)
	writeJSON(w, map[string]any{
...
func resolveLLMMode() string {
	if strings.TrimSpace(os.Getenv("SRE_AGENT_LLM_ENABLED")) == "true" {
	...
	if strings.Contains(strings.TrimSpace(os.Getenv("SRE_AGENT_WORKFLOW_INSIGHTS_PROVIDER")), "stub") {
		return "stub"
	}
	return "disabled"
}

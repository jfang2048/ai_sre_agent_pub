...
func (c *Controller) handleControllerAuditLog(w http.ResponseWriter, r *http.Request) {
	...
	for _, record := range records {
		...
		if statusFilter != "" &&
			strings.ToLower(strings.TrimSpace(record.Status)) != statusFilter &&
			strings.ToLower(strings.TrimSpace(record.Outcome)) != statusFilter {
			continue
		}
...
func (c *Controller) appendControllerAudit(r *http.Request, record ControllerAuditRecord) {
	...
	if r != nil {
		if actor, ok := identity.FromContext(r.Context()); ok {
			record.Actor = firstNonEmpty(strings.TrimSpace(actor.Subject), strings.TrimSpace(r.Header.Get("X-SRE-Actor")), strings.TrimSpace(r.Header.Get("X-User")), "anonymous")
			if record.Input == nil {
				record.Input = map[string]string{}
			}
			record.Input["authn_method"] = string(actor.AuthnMethod)
			record.Input["actor_type"] = string(actor.ActorType)
			if roles := actor.RolesCSV(); roles != "" {
				record.Input["roles"] = roles
			}
		} else {
	...
	if strings.TrimSpace(record.Outcome) == "" {
		record.Outcome = controllerAuditOutcome(record.Status)
	}
...
func controllerAuditOutcome(status string) string {
	switch strings.ToLower(strings.TrimSpace(status)) {
	case agentcore.WorkflowToolOutcomeExecutedSuccess,
		agentcore.WorkflowToolOutcomeExecutedFailure,
		agentcore.WorkflowToolOutcomeBlocked,
		agentcore.WorkflowToolOutcomeSkipped,
		agentcore.WorkflowToolOutcomePlannedOnly,
		agentcore.WorkflowToolOutcomeProposedOnly,
		agentcore.WorkflowToolOutcomeReadOnlySuccess,
		agentcore.WorkflowToolOutcomeReadOnlyFailure:
		return strings.ToLower(strings.TrimSpace(status))
	case "success", "completed", "approved":
		return agentcore.WorkflowToolOutcomeExecutedSuccess
	case "failed", "error":
		return agentcore.WorkflowToolOutcomeExecutedFailure
	case "forbidden", "rejected":
		return agentcore.WorkflowToolOutcomeBlocked
	case "stopped":
		return agentcore.WorkflowToolOutcomeSkipped
	default:
		return ""
	}
}

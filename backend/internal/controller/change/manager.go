...
// ChangeManager tracks change windows, approvals, and canary progress.
...
// Config sets window and approval defaults.
type Config struct {
	DefaultChangeWindowDuration time.Duration `yaml:"default_change_window_duration"`
	RequireApprovalForTier      int           `yaml:"require_approval_for_tier"`
}

// Change is one planned change and its guardrails.
type Change struct {
	ID          string `json:"id"`
...
// SLOTier marks the service tier.
type SLOTier int
...
// ChangeType names the change shape.
...
// RiskLevel names the change risk.
...
// ApprovalStatus names the approval state.
...
// ChangeStatus names the change state.
...
// ValidationResult records one validation pass.
...
// Check records one check result.
type Check struct {
...
// CanaryConfig controls a stepped rollout.
...
// CanaryStatus tracks a stepped rollout.
...
// CanaryDeploymentStatus names the rollout state.
...
// SuccessCriteria sets the rollback threshold.
...
// ApprovalPolicy defines who can approve.
...
// NewChangeManager creates one in-memory change registry.
...
// RegisterChange stores one new change request.
...
// ApproveChange marks a pending change approved.
...
// RejectChange marks a pending change rejected.

...
// GetLinks retrieves all links where source is the given node
func (ts *TopologyStore) GetLinks(source string) []ServiceLink {
	ts.mu.RLock()
	defer ts.mu.RUnlock()

	links := make([]ServiceLink, 0)
...
func (ts *TopologyStore) UpdateNodeMetadata(name string, metadata map[string]string) {
	ts.mu.Lock()
	defer ts.mu.Unlock()
...
// UpdateNodeStatus updates the status for a node
func (ts *TopologyStore) UpdateNodeStatus(name, status string) {
	ts.mu.Lock()
	defer ts.mu.Unlock()
...
// Clear removes all nodes and links from the store
func (ts *TopologyStore) Clear() {
	ts.mu.Lock()
	defer ts.mu.Unlock()

	ts.nodes = make(map[string]*ServiceNode)
	ts.links = make(map[string]*ServiceLink)
}

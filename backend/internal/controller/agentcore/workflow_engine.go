...
func (s *workflowState) applyKnowledgeResult(result workflowToolResult) {
	...
	if data.Confidence > 0 {
		s.retrievalConfidence = data.Confidence
	}
	if len(data.Hits) > 0 {
		s.retrievedDocs = append([]RetrievedDocumentEvidence(nil), data.Hits...)
	}
	if len(data.EvidenceIDs) > 0 {
		s.retrievalEvidenceIDs = append([]string(nil), data.EvidenceIDs...)
	}
}
...
func buildKnowledgeQuery(state *workflowState, phase string) map[string]string {
	if state == nil {
		return nil
	}
	parts := make([]string, 0, 16)
	parts = append(parts, state.workflowType, phase, state.collectorID)
	for _, signal := range topTriggeredSignals(state.riskSignals, 4) {
		parts = append(parts, signal.Name, signal.Entity)
	}
...
func (e *WorkflowEngine) stepCollectSignals(ctx context.Context, state *workflowState) error {
	...
	knowledgeResult, err := state.callTool(ctx, "collect_signals", ToolKnowledge, buildKnowledgeQuery(state, "collect_signals"))
	if err != nil {
		state.limitations = append(state.limitations, "knowledge retrieval unavailable")
	} else {
...
func (e *WorkflowEngine) stepJointRiskRecommendations(ctx context.Context, state *workflowState) error {
	...
	if strings.TrimSpace(state.retrievalSummary) != "" {
		recs = append(recs, recommendationFromFields(
			...
			"medium",
			"Use retrieved knowledge to scope next checks",
	...
	for i, hit := range state.retrievedDocs {
		if i >= 2 {
			break
		}
		recs = append(recs, recommendationFromFields(
			fmt.Sprintf("risk-knowledge-%d", i+1),
			"structural_prevention",
			"medium",
			fmt.Sprintf("Review retrieved evidence: %s", firstNonEmpty(hit.Title, hit.SourcePath)),
			fmt.Sprintf("%s [%s] %s", hit.SourcePath, hit.EvidenceID, truncateString(hit.Snippet, 220)),
			"knowledge_base",
			[]string{"validate whether the retrieved guidance matches the active signal pattern", "capture any matching mitigation steps before execution"},
			true, true, false, true,
			"read-only knowledge review",
			"",
			"runbook and historical context should be checked before any action",
...
func (e *WorkflowEngine) stepGatherRCAContext(ctx context.Context, state *workflowState) error {
	knowledgeResult, err := state.callTool(ctx, "context_gathering", ToolKnowledge, buildKnowledgeQuery(state, "context_gathering"))
	if err != nil {
		state.limitations = append(state.limitations, "knowledge retrieval unavailable during RCA context gathering")
	} else {
		state.applyKnowledgeResult(knowledgeResult)
	}
...
func buildInitialPlanSteps(state *workflowState) []AgentPlanStep {
	...
	if needs.knowledge {
		addStep(
			"plan-knowledge",
			"Retrieve relevant runbooks and prior cases",
			"Pull dataset-backed architecture, runbook, and historical knowledge that matches the current signal mix.",
			ToolKnowledge,
			buildKnowledgeQuery(state, "plan_act_verify_loop"),
			false,
		)
	}
	if needs.topology {
...
func verifyPlanStep(step AgentPlanStep, result workflowToolResult, state *workflowState) (bool, string, []string) {
	...
	case ToolKnowledge:
		...
		if len(data.Hits) == 0 {
			return false, "knowledge retrieval returned no matching evidence", evidenceIDs
		}
		return true, fmt.Sprintf("knowledge hits=%d confidence=%.2f", len(data.Hits), data.Confidence), append(evidenceIDs, data.EvidenceIDs...)
	case ToolTopology:
...
func (e *WorkflowEngine) stepRCARecommendations(_ context.Context, state *workflowState) error {
	...
	for _, hypothesis := range state.hypotheses {
		recs = append(recs, recommendationFromFields(
			...
			firstNonEmpty(state.collectorID, "fleet"),
			checksForHypothesis(hypothesis.Title),
			true, true, false, true,
	...
	if strings.TrimSpace(state.retrievalSummary) != "" {
		recs = append(recs, recommendationFromFields(
			...
			"structural_prevention",
			"medium",
			"Cross-check RCA against retrieved knowledge base evidence",
			state.retrievalSummary,
			"knowledge_base",
	...
	for i, hit := range state.retrievedDocs {
		...
		recs = append(recs, recommendationFromFields(
			fmt.Sprintf("rca-knowledge-%d", i+1),
			"structural_prevention",
			"medium",
			fmt.Sprintf("Inspect retrieved runbook/case: %s", firstNonEmpty(hit.Title, hit.SourcePath)),
			fmt.Sprintf("%s [%s] %s", hit.SourcePath, hit.EvidenceID, truncateString(hit.Snippet, 220)),
			"knowledge_base",
			[]string{"compare remediation steps to the current hypothesis", "verify preconditions before applying any runbook action"},
			true, true, false, true,
			"read-only knowledge review",
			"",
			"runbook evidence must be compared against the current incident before execution",

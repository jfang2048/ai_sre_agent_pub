...
func NewWorkflowEngine(cfg WorkflowConfig, store *ingest.MemoryStore, idx *logindex.Index, topology TopologyProvider, logger *zap.Logger) *WorkflowEngine {
	...
	if cfg.WorkflowStorePath != "" {
		if boltStore, err := NewBoltDurableStore(cfg.WorkflowStorePath); err == nil {
			wfStore = boltStore
			engine.logger.Info("using persistent workflow store", zap.String("path", cfg.WorkflowStorePath))
		} else {
			engine.logger.Error("failed to open persistent workflow store, falling back to in-memory", zap.Error(err))
	...
	engine.memoryStore = NewWorkflowMemoryStore(cfg.WorkflowDataPath, logger)
	engine.changeStore = changeintel.NewStore(cfg.WorkflowDataPath, logger)
	engine.evidenceBuilder = newWorkflowEvidenceBuilder(cfg.WorkflowDataPath, logger)
...
func (e *WorkflowEngine) EvaluateJointRisk(ctx context.Context, req WorkflowRequest) (JointRiskAssessment, error) {
	...
	if state.durableRun != nil {
		...
		if path := e.attachEvidencePackage(ctx, state.workflowID, report); strings.TrimSpace(path) != "" {
			report.EvidencePackagePath = path
		}
	}
...
func (e *WorkflowEngine) BuildRCAWorkflow(ctx context.Context, req WorkflowRequest) (RCAWorkflowReport, error) {
	...
	pipeline := deterministicPipeline{
		...
		steps: []pipelineStep{
			...
			{name: "llm_analysis", run: e.stepLLMAnalysis},
			{name: "analysis_handoff_finalize", run: e.stepAnalysisHandoffFinalize},
		},
	}
...
func (p deterministicPipeline) run(ctx context.Context, state *workflowState) error {
	for _, step := range p.steps {
		...
		if state.durableRun != nil {
			_ = state.engine.orchestrator.RecordStepTransition(ctx, state.workflowID, step.name)
			_ = state.engine.orchestrator.LogEvent(ctx, state.workflowID, "stage_started", map[string]any{"stage": step.name})
		}
		...
		if state.durableRun != nil {
			_ = state.engine.orchestrator.LogEvent(ctx, state.workflowID, "stage_completed", map[string]any{"stage": step.name, "summary": stage.Summary})
		}
	}
	return nil
}
...
func actorForWorkflowStage(stage string) string {
	...
	case "collect_signals", "score_signals", "correlate_signals", "incident_synthesis", "context_gathering", "hypothesis_generation", "evidence_collection", "llm_analysis", "analysis_handoff_finalize":
...
func (e *WorkflowEngine) stepPlanActVerifyLoop(ctx context.Context, state *workflowState) error {
	...
	for idx := 0; idx < len(state.planSteps); idx++ {
		...
		if step.Tool == ToolRemediation && !state.dryRun {
			...
			verificationResult, vErr := e.verifyRemediationEffect(ctx, state)
			if vErr != nil {
				e.logger.Warn("verification failed", zap.Error(vErr))
				step.Status = "verification_failed"
				step.VerificationNote = fmt.Sprintf("Verification failed: %v", vErr)
				if state.durableRun != nil {
				...
				step.VerificationNote = "Remediation verified: risk level decreased below threshold."
				if state.durableRun != nil {
					_ = e.orchestrator.RecordVerification(ctx, state.workflowID, step.ID, DurableVerificationRecord{
						Outcome:      "resolved",
						Success:      true,
						...
						Note:         step.VerificationNote,
						Window:       e.cfg.VerificationWindow.String(),
						PreviousRisk: maxFloat(state.incident.Confidence, topHypothesisConfidence(state.hypotheses)),
						CurrentRisk:  verificationResult.CurrentRisk,
				...
				step.Status = "verification_failed"
				step.VerificationNote = fmt.Sprintf("Remediation failed to resolve issue. Current risk: %.2f", verificationResult.CurrentRisk)
				if state.durableRun != nil {
					_ = e.orchestrator.RecordVerification(ctx, state.workflowID, step.ID, DurableVerificationRecord{
						Outcome:      "unresolved",
						Success:      false,
...
func (e *WorkflowEngine) verifyRemediationEffect(ctx context.Context, state *workflowState) (verificationResult, error) {
	// Simple verification: trigger a fresh risk assessment
	req := WorkflowRequest{
		CollectorID: state.collectorID,
...
func (e *WorkflowEngine) stepCompensate(ctx context.Context, state *workflowState, step *AgentPlanStep) {
	...
	if e.playbookRunner != nil && step.OriginalAction != nil && step.OriginalAction.RollbackCommand != "" {
		res := e.playbookRunner.ExecuteRollback(ctx, *step.OriginalAction, e.cfg.DryRun)
...
func (e *WorkflowEngine) recordSuccessfulRemediation(ctx context.Context, state *workflowState, step *AgentPlanStep) {
	...
	path, err := e.memoryStore.Append(WorkflowMemoryRecord{
		...
		ResolutionSummary:   step.ResultSummary,
		VerificationSummary: step.VerificationNote,
		...
		Actions:             []string{firstNonEmpty(step.Query["action"], step.Title)},
		EvidenceIDs:         append([]string(nil), step.EvidenceIDs...),
	...
	if e.orchestrator != nil {
		_ = e.orchestrator.AppendMemoryRecord(ctx, state.workflowID, path)
	}
}
...
func (e *WorkflowEngine) persistRCAArtifacts(ctx context.Context, state *workflowState) {
	...
	if e.memoryStore != nil {
		path, err := e.memoryStore.Append(WorkflowMemoryRecord{
			...
			Hypotheses:          append([]RCAHypothesis(nil), state.hypotheses...),
			PlanSteps:           append([]AgentPlanStep(nil), state.planSteps...),
			ActionOutcomes:      incidentMemoryActionOutcomes(state.planSteps),
		})
		if err == nil {
			if e.telemetry != nil {
				e.telemetry.recordMemoryWriteback()
...
func (e *WorkflowEngine) attachEvidencePackage(ctx context.Context, workflowID string, report any) string {
	...
	ref, err := e.evidenceBuilder.Write(run, trace, e.AuditRecords(400, workflowID), report)
	if err != nil {
		e.logger.Warn("failed to write workflow evidence package", zap.String("workflow_id", workflowID), zap.Error(err))
		return ""
	}
	_ = e.orchestrator.AttachEvidencePackage(ctx, workflowID, ref)
	if e.telemetry != nil {
...
func (e *WorkflowEngine) stepValidationActionLoop(ctx context.Context, state *workflowState) error {
	...
	report := agent.Run(ctx, state, state.analysisHandoff)
	...
	state.planCompleted = !strings.Contains(strings.ToLower(report.StopReason), "budget")
	state.planStopReason = firstNonEmpty(report.StopReason, "validation completed")
	...
	if state.durableRun != nil {
		...
		_ = e.orchestrator.AttachValidationReport(ctx, state.workflowID, report)
	}
	return nil
}
...
func validationResultsToPlanSteps(report ValidationActionReport) []AgentPlanStep {
	...
	for idx, result := range report.Results {
		...
		steps = append(steps, AgentPlanStep{
			...
			VerificationNote: result.StopReason,
...
func (e *WorkflowEngine) stepPostActionValidation(ctx context.Context, state *workflowState) error {
	...
	for idx := len(state.toolCalls) - 1; idx >= 0; idx-- {
		call := state.toolCalls[idx]
		if call.Tool != ToolRemediation || strings.TrimSpace(call.Status) == "" {
			continue
		}
		verification, err := e.verifyRemediationEffect(ctx, state)
		summary := PostActionValidationSummary{
			Verdict:    ValidationVerdictInsufficientEvidence,
			Summary:    "remediation executed but richer validation did not run",
			BeforeRisk: maxFloat(state.incident.Confidence, topHypothesisConfidence(state.hypotheses)),
			AfterRisk:  verification.CurrentRisk,
		}
		if err != nil {
			summary.Summary = fmt.Sprintf("post-action validation failed: %v", err)
		} else if verification.Resolved {
			summary.Verdict = ValidationVerdictConfirmed
			summary.Summary = fmt.Sprintf("post-action validation observed lower risk score %.2f", verification.CurrentRisk)
		} else {
			summary.Verdict = ValidationVerdictContradicted
			summary.Summary = fmt.Sprintf("post-action validation kept risk score at %.2f", verification.CurrentRisk)
		}
...
func (e *WorkflowEngine) stepGuardedExecutionPlan(ctx context.Context, state *workflowState) error {
	...
	// Topology-aware blast radius assessment
	downstream := e.calculateDownstreamNodes(state.collectorID, state.topoData)
	downstreamStr := strings.Join(downstream, ",")

...
func newWorkflowToolManager(logger *zap.Logger, tools ...workflowTool) *workflowToolManager {
	...
	return &workflowToolManager{
		...
		contracts:   NewToolContractRegistry(descriptors),
...
func describeWorkflowTool(tool workflowTool) WorkflowToolDescriptor {
	...
	case ToolRemediation:
		...
		desc.InputSchema = `{"action_contract":"ValidationActionContract","validation_category":"string"}`
		desc.OutputSchema = `{"contract":"ValidationActionContract","summary":"string","mode":"string","rollback_plan":"string","requires_approval":"bool","safety_tier":"string","proposal_only":"bool","execution_eligible":"bool"}`
		desc.ReadOnly = false
		desc.RequiresApproval = true
		desc.SupportsDryRun = true
		desc.SupportsRollback = true
	...
	desc.Contract = workflowToolContractFromDescriptor(desc)
	desc.RichContract = toolContractFromLegacy(desc.Contract)
	desc.CapabilityFamily = desc.Contract.CapabilityFamily
	desc.ApprovalRequired = desc.Contract.Approval.Required
	desc.AutonomyEligibility = desc.RichContract.AutonomyEligibility
	desc.CostClass = desc.Contract.CostClass
	desc.FreshnessSensitivity = desc.Contract.FreshnessSensitivity
	desc.ScopeSensitivity = desc.Contract.ScopeSensitivity
	desc.ExpectedInformationGain = desc.Contract.ExpectedInformationGain
	desc.PolicyStatus = "registered"
	desc.LastResultQuality = "unknown"
	return desc
}
...
func (m *workflowToolManager) call(ctx context.Context, req workflowToolRequest, name ToolName) (WorkflowToolCall, workflowToolResult, error) {
	...
	call := WorkflowToolCall{
		...
		ExecutionCategory: executionCategoryFromToolQuery(req.Query),
		ActionIntent:      actionIntentFromToolQuery(req.Query),
		Status:            WorkflowToolOutcomePlannedOnly,
	...
	if !ok {
		call.InvocationStatus = "failed"
		call.Status = workflowToolCallOutcome(name, call.InvocationStatus, nil, call.Policy)
	...
	call.ToolContract = workflowToolContractID(desc.Contract)
	if err := validateWorkflowToolContract(desc.Contract); err != nil {
		call.InvocationStatus = "failed"
		call.Status = workflowToolCallOutcome(name, call.InvocationStatus, nil, call.Policy)
		call.Outcome = call.Status
	...
	richContract := desc.RichContract
	if m.contracts != nil {
		if registered, ok := m.contracts.Get(name); ok {
			richContract = registered
		}
	}
	if err := validateToolContract(richContract); err != nil {
		call.InvocationStatus = "failed"
		call.Status = workflowToolCallOutcome(name, call.InvocationStatus, nil, call.Policy)
		call.Outcome = call.Status
		call.ErrorMessage = err.Error()
	...
	if !workflowToolContractAllowsStage(desc.Contract, req.Stage) {
		err := fmt.Errorf("tool %s is not allowed in workflow stage %s by contract", name, req.Stage)
		call.InvocationStatus = "blocked"
		call.Status = workflowToolCallOutcome(name, call.InvocationStatus, nil, call.Policy)
		call.Outcome = call.Status
		call.ErrorMessage = err.Error()
		call.CompletedAt = time.Now().UTC()
	...
	if decision.ProposalOnly {
		call.ApprovalState = "not_applicable"
	}
	if decision.Status == "blocked" {
		call.InvocationStatus = "blocked"
		call.Status = workflowToolCallOutcome(name, call.InvocationStatus, nil, decision)
		call.Outcome = call.Status
	...
	if decision.RequiresApproval && !req.DryRun {
		call.InvocationStatus = "approval_required"
		call.Status = workflowToolCallOutcome(name, call.InvocationStatus, nil, decision)
		call.Outcome = call.Status
	...
	if req.IdempotencyKey != "" {
		...
		if ok {
			m.cacheMu.RUnlock()
			call.InvocationStatus = "skipped"
			call.Status = workflowToolCallOutcome(name, "success", cached.Data, decision)
			call.Outcome = call.Status
		...
		if !desc.ReadOnly && m.orchestrator != nil {
			if previous, lookupErr := m.orchestrator.FindToolCallByIdempotency(ctx, req.IdempotencyKey); lookupErr == nil && previous != nil {
				...
				call.Status = workflowToolStoredOutcome(*previous)
				call.Outcome = call.Status
	...
	timeout := workflowToolTimeoutFromContract(desc.Contract, name)
	maxAttempts := workflowToolMaxAttemptsFromContract(desc.Contract, m.cfg)
	var (
	...
	call.InvocationStatus = workflowToolCallStatus(name, result, decision)
	call.Status = workflowToolCallOutcome(name, call.InvocationStatus, result.Data, decision)
	call.Outcome = call.Status
...
func workflowToolCallStatus(name ToolName, result workflowToolResult, decision ActionPolicyDecision) string {
	if name == ToolRemediation {
		if data, ok := result.Data.(remediationToolData); ok {
...
func validateWorkflowToolRequest(name ToolName, req workflowToolRequest) error {
	...
	case ToolKnowledge, ToolRAGQuery, ToolHistoricalIncident, ToolRunbookRetrieval, ToolSimilarCase, ToolActionOutcome:
		...
		if raw := strings.TrimSpace(req.Query["top_k"]); raw != "" {
			...
			if err != nil || parsed <= 0 {
				return fmt.Errorf("knowledge retrieval top_k must be a positive integer")
			}
			if parsed > 5 {
				return fmt.Errorf("knowledge retrieval top_k must be <= 5")
			}
		}
	case ToolRemediation:
		if strings.TrimSpace(req.Query["action_contract"]) == "" && strings.TrimSpace(req.Query["action"]) == "" {
			return fmt.Errorf("remediation action contract is required")
		}
	}
	return nil
}

func executionCategoryFromToolQuery(query map[string]string) string {
	if len(query) == 0 {
		return ""
	}
	if contract, err := decodeValidationActionContract(query["action_contract"]); err == nil && contract != nil {
		return normalizeValidationCategory(contract.ExecutionCategory)
	}
	return normalizeValidationCategory(query["validation_category"])
}

func actionIntentFromToolQuery(query map[string]string) string {
	if len(query) == 0 {
		return ""
	}
	if contract, err := decodeValidationActionContract(query["action_contract"]); err == nil && contract != nil {
		return strings.TrimSpace(contract.Intent)
	}
	return actionIntentFromText(query["action"])
}
...
func decodeWorkflowToolPayload(name ToolName, payload string) (any, bool) {
	...
	case ToolRemediation:
		...
		if err := json.Unmarshal([]byte(payload), &plan); err == nil && (plan.Summary != "" || plan.Mode != "") {
			return plan, true
		}
	case ToolProfiling:
...
type remediationToolData struct {
	Contract          ValidationActionContract `json:"contract"`
	...
	ExecutedAction    *ActionSpec              `json:"executed_action,omitempty"`
}
...
func remediationContractFromRequest(req workflowToolRequest) (*ValidationActionContract, error) {
	contract, err := decodeValidationActionContract(req.Query["action_contract"])
	if err != nil {
		return nil, err
	}
	if contract != nil {
	...
	scope := strings.TrimSpace(firstNonEmpty(req.Query["scope"], req.CollectorID, "fleet"))
	legacy := ValidationActionCandidate{
		ID:               fmt.Sprintf("legacy-%d", time.Now().Unix()),
		Category:         normalizeValidationCategory(req.Query["validation_category"]),
		ActionIntent:     actionIntentFromText(req.Query["action"]),
		Summary:          firstNonEmpty(strings.TrimSpace(req.Query["action"]), "validate_and_hold"),
		Scope:            scope,
		RequiresApproval: true,
		DryRunDefault:    true,
		Reversible:       strings.TrimSpace(req.Query["rollback"]) != "",
		RollbackHint:     strings.TrimSpace(req.Query["rollback"]),
	}
	derived := buildValidationActionContract(legacy, req.CollectorID, compactStrings(strings.Split(req.Query["downstream_nodes"], ",")...))
	return &derived, nil
}

func (t *remediationActionTool) Run(ctx context.Context, req workflowToolRequest) (workflowToolResult, error) {
	contract, err := remediationContractFromRequest(req)
	if err != nil {
		return workflowToolResult{}, fmt.Errorf("decode remediation action contract: %w", err)
	}
	...
	safetyDecision := evaluateActuatorExecutionPolicy(t.cfg, contract, req.DryRun || t.cfg.DryRun)
	data := remediationToolData{
		Contract:          *contract,
		Action:            firstNonEmpty(contract.Summary, contract.Intent),
		ActionIntent:      contract.Intent,
		Category:          normalizeValidationCategory(firstNonEmpty(contract.ExecutionCategory, req.Query["validation_category"])),
		ExecutionCategory: normalizeValidationCategory(firstNonEmpty(contract.ExecutionCategory, req.Query["validation_category"])),
		SafetyTier:        firstNonEmpty(contract.ActuatorSafetyTier, safetyDecision.SafetyTier),
		ProposalOnly:      safetyDecision.ProposalOnly,
		ExecutionEligible: safetyDecision.ExecutionEligible,
		DryRun:            req.DryRun || t.cfg.DryRun,
		RequiresApproval:  contract.RequiresApproval || safetyDecision.RequiresApproval,
		Reversible:        contract.Rollback.Reversible || contract.Rollback.Required,
		RollbackPlan:      firstNonEmpty(contract.Rollback.Summary, "capture pre-change baseline -> execute single scoped change -> revert workload/config to previous revision"),
		Mode:              mode,
		BlastRadius:       contract.BlastRadiusEstimate,
		DownstreamNodes:   append([]string(nil), contract.BlastRadiusScope...),
		Preconditions:     append([]string(nil), contract.Preconditions...),
	}
	if safetyDecision.ProposalOnly {
		data.Mode = "proposal_only"
		data.Summary = fmt.Sprintf("proposal-only remediation for %s on %s (%s)", firstNonEmpty(contract.Summary, contract.Intent), firstNonEmpty(contract.Target.Scope, req.CollectorID, "fleet"), safetyDecision.Reason)
		return workflowToolResult{Summary: data.Summary, Data: data}, nil
	}

	if !req.DryRun && !t.cfg.DryRun {
		switch {
		case contract.Execution == nil:
			data.Mode = "planned_only"
		case !(t.cfg.ActionRunner.AllowUnsafe || t.cfg.AllowRemediationExec):
			data.Mode = "blocked_by_policy"
		case t.runner != nil:
			execution := cloneActionSpec(contract.Execution)
			if execution == nil {
				data.Mode = "planned_only"
			} else {
				results := t.runner.Execute(ctx, []ActionSpec{*execution}, false)
				if len(results) > 0 {
					res := results[0]
					data.ExecutedAction = execution
					if res.Status == ActionResultExecuted {
						data.Mode = "executed"
						data.Summary = fmt.Sprintf("executed %s for %s", firstNonEmpty(contract.Intent, contract.Summary), firstNonEmpty(contract.Target.Scope, req.CollectorID, "fleet"))
						return workflowToolResult{Summary: data.Summary, Data: data}, nil
					}
					data.Mode = "failed"
					data.Summary = fmt.Sprintf("remediation failed: %s", firstNonEmpty(res.Error, res.Output))
					return workflowToolResult{Summary: data.Summary, Data: data}, fmt.Errorf("remediation failed: %s", firstNonEmpty(res.Error, res.Output))
				}
				data.Mode = "approved_execution_requested"
			}
		default:
			data.Mode = "approved_execution_requested"
		}
	...
	scope := firstNonEmpty(contract.Target.Scope, req.CollectorID, "fleet")
	data.Summary = fmt.Sprintf("remediation plan for %s on %s (%s)", firstNonEmpty(contract.Summary, contract.Intent), scope, data.Mode)
	if data.BlastRadius > 0 {
		data.Summary += fmt.Sprintf(" | potential blast radius: %d scoped targets", data.BlastRadius)
	}
	return workflowToolResult{Summary: data.Summary, Data: data}, nil
}
...
func (e *PolicyEngine) Evaluate(req workflowToolRequest, tool workflowTool) ActionPolicyDecision {
	...
	if desc.ReadOnly {
		return ActionPolicyDecision{Status: "allowed", Reason: "read-only tool", ExecutionLevel: "read_only", SafetyTier: ActuatorSafetyTierReadOnly, ExecutionEligible: true}
	}

	if tool != nil && tool.Name() == ToolRemediation {
		contract, err := remediationContractFromRequest(req)
		if err != nil {
			return ActionPolicyDecision{
				Status:         "blocked",
				Reason:         "invalid remediation action contract",
				ExecutionLevel: "suggest_only",
				SafetyTier:     ActuatorSafetyTierImpacting,
			}
		}
		decision := evaluateActuatorExecutionPolicy(e.cfg, contract, req.DryRun)
		if decision.Status != "allowed" {
			return decision
		}
		rollback := strings.TrimSpace(req.Query["rollback"])
		if rollback == "" && contract != nil {
			rollback = strings.TrimSpace(firstNonEmpty(contract.Rollback.Summary, contract.Rollback.Command))
		}
		if rollback == "" {
			return ActionPolicyDecision{
				Status:            "blocked",
		...
		if req.DryRun {
			decision.Reason = "remediation stays in dry-run mode"
			decision.DryRunRequired = true
			return decision
		}
		if decision.RequiresApproval || desc.RequiresApproval {
			decision.Status = "pending"
			decision.Reason = "requires explicit approval"
			decision.ExecutionLevel = "approval_required"
			decision.RequiresApproval = true
			decision.DryRunRequired = decision.DryRunRequired || contractDryRunDefault(contract)
	...
	if req.DryRun {
		return ActionPolicyDecision{
			...
			SafetyTier:        ActuatorSafetyTierSafeReversible,

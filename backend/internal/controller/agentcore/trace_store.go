...
func (ts *TraceStore) RecordTrace(trace *AgentTrace) {
	...
	for idx := range trace.ToolCalls {
		normalizeWorkflowToolCall(&trace.ToolCalls[idx])
	}
	for idx := range trace.Stages {
		normalizePipelineStageResult(&trace.Stages[idx])
	}
	ts.mu.Lock()
	defer ts.mu.Unlock()
...
func (ts *TraceStore) GetTrace(traceID string) (*AgentTrace, bool) {
	...
	cp := *t
	for idx := range cp.ToolCalls {
		normalizeWorkflowToolCall(&cp.ToolCalls[idx])
	}
	for idx := range cp.Stages {
		normalizePipelineStageResult(&cp.Stages[idx])
	}

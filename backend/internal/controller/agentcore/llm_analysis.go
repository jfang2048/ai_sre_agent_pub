...
type LLMAnalysisResult struct {
	...
	Reasoning       LLMReasoningArtifact `json:"reasoning"`
	Review          *LLMReasoningReview  `json:"review,omitempty"`
	EvidenceCited   []string             `json:"evidence_cited"`
	ToolRequests    []LLMToolRequest     `json:"tool_requests,omitempty"`
	Limitations     []string             `json:"limitations"`
}

// LLMReasoningArtifact is the normalized cold-path reasoning contract exposed to operators and audits.
type LLMReasoningArtifact struct {
	Plan                  []string        `json:"plan"`
	Hypotheses            []LLMHypothesis `json:"hypotheses,omitempty"`
	Confidence            float64         `json:"confidence"`
	ExpectedSLOImpact     string          `json:"expected_slo_impact,omitempty"`
	MissingEvidence       []string        `json:"missing_evidence,omitempty"`
	RecommendedNextChecks []string        `json:"recommended_next_checks,omitempty"`
}

// LLMReasoningBranch is one bounded hypothesis branch considered during cold-path review.
type LLMReasoningBranch struct {
	ID         string   `json:"id"`
...
// LLMReasoningReview captures explicit plan -> critique -> refine metadata for auditability.
type LLMReasoningReview struct {
	Mode                      string                  `json:"mode"`
	ReviewApplied             bool                    `json:"review_applied"`
	SkippedReason             string                  `json:"skipped_reason,omitempty"`
	Initial                   LLMReasoningArtifact    `json:"initial"`
	Critique                  []string                `json:"critique,omitempty"`
	Final                     LLMReasoningArtifact    `json:"final"`
	Branches                  []LLMReasoningBranch    `json:"branches,omitempty"`
	Iterations                []LLMReasoningIteration `json:"iterations,omitempty"`
...
// LLMReasoningIteration tracks one refine pass in the bounded iterative loop.
type LLMReasoningIteration struct {
...
type ContextBundle struct {
	...
	RetrievedCases             []ContextRetrievedDoc         `json:"retrieved_cases,omitempty"`
	RetrievedRunbooks          []ContextRetrievedDoc         `json:"retrieved_runbooks,omitempty"`
	SimilarIncidentPatterns    []ContextRetrievedDoc         `json:"similar_incident_patterns,omitempty"`
...
func BuildContextBundle(state *workflowState) ContextBundle {
	...
	trends := append([]TrendAssessment(nil), state.trendAssessments...)
	if len(trends) > 6 {
		trends = trends[:6]
	}
	events := append([]InvestigationEvent(nil), state.investigationEvents...)
	if len(events) > 6 {
		events = events[:6]
	}
	retrievalDecisions := append([]RetrievalDecision(nil), state.retrievalDecisions...)
	if len(retrievalDecisions) > 6 {
		retrievalDecisions = retrievalDecisions[:6]
	}
	...
	for _, hit := range state.retrievedDocs {
		retrievedDocs = append(retrievedDocs, ContextRetrievedDoc{
			...
			Summary:          truncateString(hit.Summary, 160),
	...
	retrievedCases := make([]ContextRetrievedDoc, 0, len(state.retrievedCases))
	for _, hit := range state.retrievedCases {
		retrievedCases = append(retrievedCases, ContextRetrievedDoc{
			EvidenceID:    hit.EvidenceID,
			Title:         hit.Title,
			SourcePath:    hit.SourcePath,
			SourceType:    hit.SourceType,
			...
			CaseType:      hit.CaseType,
			Summary:       truncateString(hit.Summary, 160),
			Snippet:       truncateString(hit.Snippet, 180),
			Score:         hit.Score,
			LikelyCauses:  append([]string(nil), hit.LikelyCauses...),
			Signals:       append([]string(nil), hit.Signals...),
			Tags:          append([]string(nil), hit.Tags...),
		})
		if len(retrievedCases) >= 4 {
			break
		}
	}
	retrievedRunbooks := make([]ContextRetrievedDoc, 0, len(state.retrievedRunbooks))
	for _, hit := range state.retrievedRunbooks {
		retrievedRunbooks = append(retrievedRunbooks, ContextRetrievedDoc{
			EvidenceID:       hit.EvidenceID,
			Title:            hit.Title,
			SourcePath:       hit.SourcePath,
			SourceType:       hit.SourceType,
		...
		if len(retrievedRunbooks) >= 4 {
			break
		}
	}
	similarPatterns := make([]ContextRetrievedDoc, 0, len(state.similarPatterns))
	for _, hit := range state.similarPatterns {
		similarPatterns = append(similarPatterns, ContextRetrievedDoc{
			EvidenceID:    hit.EvidenceID,
			Title:         hit.Title,
			SourcePath:    hit.SourcePath,
			SourceType:    hit.SourceType,
	...
	return ContextBundle{
		...
		RetrievedRunbooks:       retrievedRunbooks,
		SimilarIncidentPatterns: similarPatterns,
		...
		RetrievalDecisions:      retrievalDecisions,
	}
}
...
Required JSON schema:
{
  ...
  "reasoning": {
    "plan": ["string"],
    "hypotheses": [{"title":"string","confidence":0.0-1.0,"evidence":["evidence_id"],"description":"string"}],
    "confidence": 0.0-1.0,
    "expected_slo_impact": "string",
    "missing_evidence": ["string"],
    "recommended_next_checks": ["string"]
  },
  "evidence_cited": ["evidence_id"],
  "tool_requests": [{"tool":"metrics_query|logs_query|security_check|topology_query|knowledge_retrieval|rag_query|historical_incident_retrieval|runbook_retrieval|similar_case_retrieval|trace_query|gpu_query|security_graph|process_lineage|profiling_trigger","query":{},"reason":"string"}],
  "limitations": ["string"]
}`
}
...
// BuildWorkflowReviewPrompt asks the model to critique and refine an existing draft without issuing more tool requests.
func BuildWorkflowReviewPrompt(bundle ContextBundle, initial LLMReasoningArtifact, critique []string, branches []LLMReasoningBranch) string {
	rawBundle, _ := json.MarshalIndent(bundle, "", "  ")
	rawInitial, _ := json.MarshalIndent(initial, "", "  ")
	rawBranches, _ := json.MarshalIndent(branches, "", "  ")
	rawCritique, _ := json.MarshalIndent(critique, "", "  ")

	parts := []string{
		fmt.Sprintf("Workflow type: %s", bundle.WorkflowType),
		fmt.Sprintf("Scope: %s | Time window: %s | Risk level: %s (score %.2f)",
			bundle.Scope, bundle.TimeWindow, bundle.RiskLevel, bundle.RiskScore),
		"Review and refine the existing cold-path reasoning draft.",
		"Rules:",
		"1. Critique the initial reasoning against the evidence bundle.",
		"2. If branch candidates are supplied, compare them and prefer the branch best supported by evidence.",
		"3. Refine the plan, hypotheses, expected_slo_impact, missing_evidence, and recommended_next_checks.",
		"4. Do NOT request more tools in this review step. tool_requests must be an empty array.",
		"5. Return ONLY the final JSON object matching the schema.",
		"Evidence bundle (JSON):",
		string(rawBundle),
		"Initial reasoning draft (JSON):",
		string(rawInitial),
		"Deterministic critique points (JSON array):",
		string(rawCritique),
		"Optional branch candidates (JSON array):",
		string(rawBranches),
	}
	return strings.Join(parts, "\n\n")
}
...
func ValidateLLMAnalysis(result LLMAnalysisResult) error {
	...
	if result.Reasoning.Confidence < 0 || result.Reasoning.Confidence > 1 {
		return ErrLLMSchemaConfidenceRange
	}
	...
	for _, h := range result.Reasoning.Hypotheses {
		if h.Confidence < 0 || h.Confidence > 1 {
			return fmt.Errorf("%w (reasoning hypothesis %s)", ErrLLMSchemaConfidenceRange, h.Title)
		}
	}
	for _, tr := range result.ToolRequests {
		switch tr.Tool {
		case ToolMetrics, ToolLogs, ToolSecurity, ToolTopology, ToolKnowledge, ToolRAGQuery, ToolHistoricalIncident, ToolRunbookRetrieval, ToolSimilarCase, ToolEBPFQuery, ToolGPU, ToolSecurityGraph, ToolProcessLineage, ToolProfiling:
		default:
...
func (s stubWorkflowLLMClient) Complete(_ context.Context, _, userPrompt string) (string, error) {
	...
	result := LLMAnalysisResult{
		...
		Reasoning: LLMReasoningArtifact{
			Plan: []string{
				"Confirm the strongest triggered telemetry signals against the current incident scope",
				"Cross-check logs and retrieved knowledge before proposing remediation",
			},
			Hypotheses:            hypotheses,
			Confidence:            confidence,
			ExpectedSLOImpact:     stubExpectedSLOImpact(bundle),
			MissingEvidence:       stubMissingEvidence(bundle),
			RecommendedNextChecks: append([]string(nil), nextSteps...),
		},
...
func newWorkflowLLMClient(cfg WorkflowConfig, logger *zap.Logger) llmClient {
	if !cfg.InsightsEnabled {
		logLLMRuntimeSelection(logger, "workflow_insights", "stub", strings.TrimSpace(cfg.InsightsAPIKeyEnv), false, "disabled")
		return stubWorkflowLLMClient{}
	}

	provider := canonicalLLMProvider(cfg.InsightsProvider)
	...
	if provider == "" || provider == "stub" || provider == "mock" {
		logLLMRuntimeSelection(logger, "workflow_insights", provider, keyEnv, apiKey != "", "stub")
		return stubWorkflowLLMClient{}
	}

	if provider == "jimmynight" {
		...
		if apiKey == "" {
			...
			logLLMRuntimeSelection(logger, "workflow_insights", provider, keyEnv, false, "stub")
			return stubWorkflowLLMClient{}
		}
		...
		if baseURL == "" {
			...
			logLLMRuntimeSelection(logger, "workflow_insights", provider, keyEnv, true, "stub")
			return stubWorkflowLLMClient{}
		}
		logLLMRuntimeSelection(logger, "workflow_insights", provider, keyEnv, true, "live")
		return &chatClient{
			provider:  provider,
			model:     model,
	...
	if client, err := newCanonicalLLMAdapter(provider, cfg.InsightsModel, "", apiKey, keyEnv, "workflow_insights", 30*time.Second, logger); err == nil {
		return client
	} else if logger != nil {
		logger.Warn("workflow insights LLM initialization failed; using stub",
...
func (e *WorkflowEngine) stepLLMAnalysis(ctx context.Context, state *workflowState) error {
	...
	reasoningCtx := ctx
	if e.cfg.AdvancedReasoningTimeout > 0 {
		var cancel context.CancelFunc
		reasoningCtx, cancel = context.WithTimeout(ctx, e.cfg.AdvancedReasoningTimeout)
		defer cancel()
	}
	...
	budget := newReasoningBudget(e.cfg.ReasoningTokenBudget)
	...
	for round := 0; round < maxRounds; round++ {
		if !budget.reservePrompt(systemPrompt + "\n" + userPrompt) {
			state.limitations = append(state.limitations, "LLM reasoning skipped additional rounds because the estimated token budget was exhausted")
			if e.telemetry != nil {
		...
		callCtx, cancel := context.WithTimeout(reasoningCtx, timeout)
		...
		for _, tr := range result.ToolRequests {
			if adaptiveRuntimeEnabled(e.cfg) {
				allowed, blockReason := state.recordModelDirectedToolDecision(ctx, tr, round+1)
				if !allowed {
					state.limitations = append(state.limitations,
						fmt.Sprintf("LLM-requested tool %s was not executable: %s", tr.Tool, blockReason))
					continue
				}
			}
	...
	if finalResult != nil {
		normalizeLLMAnalysisResult(state, finalResult, nil)
		refined, review := e.refineLLMAnalysis(reasoningCtx, state, bundle, finalResult, &budget)
		if review != nil {
			finalResult = refined
			normalizeLLMAnalysisResult(state, finalResult, review)
			if review.ReviewApplied {
				e.audit(state.workflowID, state.workflowType, "llm_analysis",
					"llm.review_refine", "success", state.collectorID, state.dryRun, false, true,
					map[string]string{
						"calls":             fmt.Sprintf("%d", review.Calls),
						"review_applied":    "true",
						"branch_count":      fmt.Sprintf("%d", len(review.Branches)),
						"prompt_tokens":     fmt.Sprintf("%d", review.EstimatedPromptTokens),
						"completion_tokens": fmt.Sprintf("%d", review.EstimatedCompletionTokens),
					},
					truncateString(firstNonEmpty(review.Final.ExpectedSLOImpact, "advanced reasoning review applied"), 220),
					nil,
				)
			} else if strings.TrimSpace(review.SkippedReason) != "" {
				state.limitations = append(state.limitations, "advanced reasoning review skipped: "+review.SkippedReason)
				if e.telemetry != nil && strings.Contains(strings.ToLower(review.SkippedReason), "failed") {
					e.telemetry.recordReasoningFailure()
				}
			}
		}
	...
	if fallback := deterministicFallbackAnalysis(bundle); fallback != nil {
		...
		review := &LLMReasoningReview{
			...
			ReviewApplied:             false,
			SkippedReason:             "llm output unavailable or rejected; deterministic fallback applied",
			Initial:                   buildReasoningArtifact(state, fallback),
			Final:                     buildReasoningArtifact(state, fallback),
			EstimatedPromptTokens:     budget.promptUsage(),
			EstimatedCompletionTokens: budget.completionUsage(),
			Calls:                     budget.calls,
			BudgetExhausted:           budget.exhausted(),
		}
		normalizeLLMAnalysisResult(state, fallback, review)

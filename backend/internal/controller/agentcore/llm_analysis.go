...
Required JSON schema:
{
  ...
  "tool_requests": [{"tool":"metrics_query|log_query|security_check|topology_query|ebpf_query|security_graph|process_lineage|profiling_trigger","query":{},"reason":"string"}],
  "limitations": ["string"]
}`
}
...
func ValidateLLMAnalysis(result LLMAnalysisResult) error {
	...
	for _, evidence := range result.EvidenceCited {
		...
		if !strings.HasPrefix(strings.ToLower(evidence), "ev-") {
	...
	for _, tr := range result.ToolRequests {
		switch tr.Tool {
		case ToolMetrics, ToolLogs, ToolSecurity, ToolTopology, ToolEBPFQuery, ToolSecurityGraph, ToolProcessLineage, ToolProfiling:
		default:
...
func (s stubWorkflowLLMClient) Model() string    { return "deterministic-v0.5" }
...
func (s stubWorkflowLLMClient) Complete(_ context.Context, _, userPrompt string) (string, error) {
	...
	for _, sig := range bundle.TopSignals {
		...
		evidenceID := nextEvidenceID(sig.Name)
...
func mergeLLMIntoState(state *workflowState) {
	...
	for i, step := range result.NextSteps {
		state.recommendation = append(state.recommendation, WorkflowRecommendation{
			ID:               fmt.Sprintf("llm-step-%d", i+1),
			Priority:         "medium",
			Summary:          step,
			Details:          "recommended by LLM analysis",
			Safe:             true,
			DryRunDefault:    true,
			RequiresApproval: false,
			Reversible:       true,
			RollbackHint:     "read-only investigation step",
		})
	}
}

...
func incidentConfidence(state *workflowState, signals []IncidentGroupedSignal) float64 {
	...
	return applyTelemetryConfidenceLimit(confidence, state.telemetryQuality)
}
...
func detectIncidentRootCauseCluster(state *workflowState, signals []IncidentGroupedSignal) string {
	...
	threshold := maxFloat(0.18, total*0.15)
	for _, row := range rows {
		if row.score >= threshold {
			significant++
		}
	}
	if significant >= 3 && total > 0 && rows[0].score < total*0.55 {
...
func generateDeterministicHypotheses(state *workflowState) []RCAHypothesis {
	...
	case strings.Contains(cluster, "distributed resource contention"), strings.Contains(cluster, "multi-resource"):
		appendHypothesis("h-distributed-resource", "distributed resource contention", clamp01(maxFloat(state.incident.Confidence, 0.58)), "incident synthesis found balanced pressure across CPU, network, and storage domains in the same window")
	}
	if len(state.logsData.RecentDeploys) > 0 {
	...
	for _, link := range state.changeLinks {
		title := firstNonEmpty(link.HypothesisHint, hypothesisTitleForChangeCategory(link.Category), link.Summary)
		appendHypothesis(
			fmt.Sprintf("h-change-%s", sanitizeID(firstNonEmpty(link.ChangeID, title))),
			title,
			clamp01(maxFloat(link.CorrelationScore, 0.54)),
			firstNonEmpty(link.ImpactSummary, link.Summary),
		)
	}
	for _, insight := range state.adaptiveBaselines {
		if insight.Classification == "" || insight.Classification == "stable" {
			continue
		}
		appendHypothesis(
			fmt.Sprintf("h-baseline-%s", sanitizeID(insight.Dimension)),
			hypothesisTitleForAdaptiveBaseline(insight),
			clamp01(0.42+absFloat(insight.DeltaPercent)/200.0),
			insight.Explanation,
		)
	}
...
func updateHypothesesFromToolResult(state *workflowState, step AgentPlanStep, result workflowToolResult) {
	...
	case ToolChangeQuery:
		data, _ := result.Data.(changeToolData)
		for _, event := range data.Events {
			upsertHypothesis(state,
				fmt.Sprintf("h-change-%s", sanitizeID(firstNonEmpty(event.Event.ChangeID, event.Event.Category))),
				firstNonEmpty(strings.Join(event.HypothesisHints, " / "), hypothesisTitleForChangeCategory(event.Event.Category), event.Event.Summary),
				maxFloat(0.06, event.ChangeScore*0.12),
				firstNonEmpty(event.ImpactSummary, event.Event.Summary),
			)
		}
	case ToolEBPFQuery:
...
func rerankHypotheses(state *workflowState) {
	...
	for i := range state.hypotheses {
		...
		for _, link := range state.changeLinks {
			if strings.Contains(title, strings.ToLower(strings.TrimSpace(link.Category))) || strings.Contains(title, strings.ToLower(strings.TrimSpace(link.HypothesisHint))) {
				state.hypotheses[i].Confidence = clamp01(state.hypotheses[i].Confidence + 0.04*link.CorrelationScore)
			}
		}
		if state.causal.SuspectedRootCauseEntity != "" && strings.Contains(strings.ToLower(state.causal.SuspectedRootCauseEntity), title) {
	...
	if ceiling < 1 {
		reason := firstNonEmpty(telemetryQualityReason(state.telemetryQuality), "telemetry quality limited hypothesis certainty")
		for i := range state.hypotheses {
			if state.hypotheses[i].Confidence > ceiling {
				recordHypothesisConfidence(state, i, ceiling-state.hypotheses[i].Confidence, reason)
			}
		}
	}
...
func dominantResourceSignalWeights(state *workflowState) (storageWeight, cpuWeight float64) {
	if state == nil {
		return 0, 0
	}
	for _, signal := range state.riskSignals {
		...
		switch {
		case strings.Contains(id, "io_latency"), strings.Contains(id, "io_pressure"), strings.Contains(id, "disk"):
			storageWeight += signal.Score
		case strings.Contains(id, "cpu"):
			cpuWeight += signal.Score
		}
	}
	return storageWeight, cpuWeight
}

func hypothesisTitleForChangeCategory(category string) string {
...
func hypothesisTitleForAdaptiveBaseline(insight AdaptiveBaselineInsight) string {
	switch strings.ToLower(strings.TrimSpace(insight.Dimension)) {
	case "gpu":
		return "gpu saturation or cross-layer contention"
	case "network":
		return "network congestion or retransmit pressure"
	case "disk_io":
		return "storage or IO amplification"
	case "memory":
		return "memory pressure or cache churn"
	default:
		return "cpu or scheduler contention"
	}
}
...
func dedupeHypotheses(in []RCAHypothesis) []RCAHypothesis {
	...
	for _, item := range in {
		...
		if _, ok := seen[key]; ok {
			for idx := range out {
				existingKey := strings.ToLower(strings.TrimSpace(out[idx].Title))
				if existingKey == "" {
					existingKey = strings.ToLower(strings.TrimSpace(out[idx].ID))
				}
				if existingKey != key {
					continue
				}
				if item.Confidence > out[idx].Confidence {
					out[idx].Confidence = item.Confidence
				}
				if len(strings.TrimSpace(item.Description)) > len(strings.TrimSpace(out[idx].Description)) {
					out[idx].Description = item.Description
				}
				if out[idx].ID == "" {
					out[idx].ID = item.ID
				}
				out[idx].EvidenceIDs = dedupeStrings(append(out[idx].EvidenceIDs, item.EvidenceIDs...))
				out[idx].ContradictingEvidenceIDs = dedupeStrings(append(out[idx].ContradictingEvidenceIDs, item.ContradictingEvidenceIDs...))
				break
			}
			continue
		}
...
func unresolvedGapsFromState(state *workflowState) []string {
	...
	if state.sceneClassification.SceneFamily == "" || state.sceneClassification.Confidence < 0.55 {
		gaps = append(gaps, "scene classification remains weak")
	}
	gaps = append(gaps, state.validationReport.UnresolvedUncertainty...)
	return dedupeStrings(append(gaps, workflowLimitations(state)...))
}

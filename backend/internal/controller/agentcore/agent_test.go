...
func TestNewLLMClientSupportsGeminiProvider(t *testing.T) {
	cfg := DefaultConfig()
	cfg.Provider = "gemini"
	cfg.Model = ""
	cfg.APIKey = "present"

	client := newLLMClient(cfg, zap.NewNop())
...
func TestNormalizeConfigUsesCanonicalLLMAPIKeyOnly(t *testing.T) {
	t.Setenv("OPENAI_API_KEY", "legacy-openai")
	t.Setenv("LLM_API_KEY", "legacy-generic")
	t.Setenv("SRE_AGENT_JIMMYNIGHT_API_KEY", "legacy-jimmynight")

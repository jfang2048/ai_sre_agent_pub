...
func TestAggregateTopProgramsCarriesProcessContextLabels(t *testing.T) {
	store := ingest.NewMemoryStore()
	now := time.Now()
	...
	store.StoreMetrics("c-1", []*telemetryv1.Metric{
		{
			...
			Labels: []*telemetryv1.Label{
				...
				{Key: "job", Value: "llm-run"},
				{Key: "comm_pattern", Value: "nccl"},
				{Key: "pod_uid", Value: "pod-uid-xyz"},
			},
		},
	}, now)
	...
	programs := ctrl.aggregateTopPrograms(10)
	if assert.Len(t, programs, 1) {
		assert.Equal(t, "training", programs[0].WorkloadClass)
		assert.Equal(t, "llm-run", programs[0].Job)
		assert.Equal(t, "nccl", programs[0].CommPattern)
		assert.Equal(t, "pod-uid-xyz", programs[0].PodUID)
	}
}
...
func TestHandleTopProgramsCollectorFilter(t *testing.T) {
	store := ingest.NewMemoryStore()
	now := time.Now()
	store.StoreProcesses("collector-a", []*telemetryv1.ProcessSample{
		{Pid: 1, Name: "alpha-cpu", CpuPercent: 90},
	}, now)
	store.StoreProcesses("collector-b", []*telemetryv1.ProcessSample{
		{Pid: 2, Name: "beta-cpu", CpuPercent: 80},
	}, now)

	ctrl := &Controller{ingestStore: store}
	req := httptest.NewRequest(http.MethodGet, "/api/v1/top/programs?collector_id=collector-a&limit=10", nil)
	...
	if assert.Equal(t, http.StatusOK, w.Code) {
		...
		if assert.NoError(t, err) {
			...
			if assert.Len(t, resp.Programs, 1) {
				assert.Equal(t, "collector-a", resp.Programs[0].CollectorID)
				assert.Equal(t, "alpha-cpu", resp.Programs[0].Name)
			}
		}
	}
}

func TestApplySignalDerivedFieldsIncludesSchedulerAndBlockIODelay(t *testing.T) {
	ps := &ProgramStats{
		Name: "trainer",
		SignalValues: map[string]float64{
			"rca_cpu_process_sched_wait_ratio":              0.72,
			"rca_cpu_process_sched_wait_seconds_total":      15.5,
			"rca_cpu_process_sched_run_seconds_total":       41.0,
			"rca_io_process_block_delay_seconds_total":      9.0,
			"rca_io_process_block_delay_seconds_per_second": 0.23,
		},
		...
		SignalFrequency:   map[string]uint64{},
		CategoryTotals:    map[string]float64{},
		CategoryFrequency: map[string]uint64{},
	}

	applySignalDerivedFields(ps)
	hydrateCategoryStats(ps)

	assert.Equal(t, 0.72, ps.SchedWaitRatio)
	assert.Equal(t, 15.5, ps.SchedWaitSecondsTotal)
	assert.Equal(t, 41.0, ps.SchedRunSecondsTotal)
	assert.Equal(t, 9.0, ps.BlockIODelaySecondsTotal)
	assert.Equal(t, 0.23, ps.BlockIODelaySecondsPerSecond)
	assert.Greater(t, ps.CategoryTotals["cpu"], 0.0)
	assert.Greater(t, ps.CategoryTotals["disk_io"], 0.0)
}

func TestDeriveCategoriesUsesSchedulerAndBlockDelayThresholds(t *testing.T) {
	cpuStalled := ProgramStats{Name: "cpu-stalled", SchedWaitRatio: 0.8}
	catsCPU := deriveCategories(cpuStalled)
	assert.Contains(t, catsCPU, "cpu")

	ioDelayed := ProgramStats{Name: "io-delayed", BlockIODelaySecondsPerSecond: 0.2}
	catsIO := deriveCategories(ioDelayed)
	assert.Contains(t, catsIO, "disk_io")
}

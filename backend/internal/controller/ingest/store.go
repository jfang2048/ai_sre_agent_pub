...
type NodeSnapshot struct {
	...
	RuntimeModeRequested string                            `json:"runtime_mode_requested,omitempty"`
	RuntimeModeDegraded  bool                              `json:"runtime_mode_degraded,omitempty"`
	RuntimeContainerized bool                              `json:"runtime_containerized,omitempty"`
	RuntimeReasons       []string                          `json:"runtime_reasons,omitempty"`
	RuntimeCapabilities  map[string]float64                `json:"runtime_capabilities,omitempty"`
	RuntimeSignals       map[string]float64                `json:"runtime_signals,omitempty"`
...
// SecurityFinding is the normalized collector-side security finding envelope.
...
func (s *MemoryStore) StoreMetrics(collectorID string, metrics []*telemetryv1.Metric, receivedAt time.Time) {
	...
	partialCollectorUpdate := hasCollectorPartialMetricUpdate(metrics)
	compatHardwareSuppressed := hasCompatibilityPayloadSuppressed(metrics, "hardware")
	carriedMetrics := cloneCarryForwardMetrics(node.Metrics, shouldCarryForwardCollectorMetric)
	carriedCompatHardwareMetrics := cloneCarryForwardMetrics(node.Metrics, shouldCarryForwardCompatibilityHardwareMetric)
	carriedProbeSource := node.ProbeSource
	carriedRuntimeMode := node.RuntimeMode
	carriedRuntimeModeRequested := node.RuntimeModeRequested
	carriedRuntimeModeDegraded := node.RuntimeModeDegraded
	carriedRuntimeContainerized := node.RuntimeContainerized
	carriedRuntimeCapabilities := cloneFloatMap(node.RuntimeCapabilities)
	carriedRuntimeSignals := cloneFloatMap(node.RuntimeSignals)
	carriedProbeCoreModules := cloneProbeCoreModuleMap(node.ProbeCoreModules)

	node.Metrics = make(map[string]float64, len(metrics)+len(carriedMetrics))
	if partialCollectorUpdate {
		for name, value := range carriedMetrics {
			node.Metrics[name] = value
		}
		node.ProbeSource = carriedProbeSource
		node.RuntimeMode = carriedRuntimeMode
		node.RuntimeModeRequested = carriedRuntimeModeRequested
		node.RuntimeModeDegraded = carriedRuntimeModeDegraded
		node.RuntimeContainerized = carriedRuntimeContainerized
		node.RuntimeCapabilities = carriedRuntimeCapabilities
		node.RuntimeSignals = carriedRuntimeSignals
		node.ProbeCoreModules = carriedProbeCoreModules
	} else {
		node.ProbeSource = ""
		node.RuntimeMode = ""
		node.RuntimeModeRequested = ""
		node.RuntimeModeDegraded = false
		node.RuntimeContainerized = false
		node.RuntimeCapabilities = nil
		node.RuntimeSignals = nil
		node.ProbeCoreModules = nil
	}
	if compatHardwareSuppressed {
		for name, value := range carriedCompatHardwareMetrics {
			node.Metrics[name] = value
		}
	}
	...
	node.LastIngestAt = receivedAt
	node.ProcessGraphSnapshot = buildProcessGraphSnapshot(node)
	pruneProcessResources(node)
	sampleAt := receivedAt
	if !node.LastCollectionAt.IsZero() {
		sampleAt = node.LastCollectionAt
	}
	s.recordMetricHistory(collectorID, sampleAt, receivedAt, node.Metrics)
...
func captureProbeCoreMetric(node *NodeSnapshot, metric *telemetryv1.Metric) {
	...
	case "collector_runtime_mode":
		node.RuntimeMode = strings.TrimSpace(labelValue(metric, "mode"))
	case "collector_runtime_mode_requested":
		node.RuntimeModeRequested = strings.TrimSpace(labelValue(metric, "mode"))
	case "collector_runtime_mode_degraded":
		node.RuntimeModeDegraded = metric.Value >= 0.5
	case "collector_runtime_containerized":
		node.RuntimeContainerized = metric.Value >= 0.5
	case "collector_runtime_capability_available":
		capability := strings.TrimSpace(labelValue(metric, "capability"))
		if capability == "" {
			return
		}
		if node.RuntimeCapabilities == nil {
			node.RuntimeCapabilities = make(map[string]float64, 8)
		}
		node.RuntimeCapabilities[capability] = metric.Value
	case "collector_runtime_signal_coverage":
		signal := strings.TrimSpace(labelValue(metric, "signal"))
		if signal == "" {
			return
		}
		if node.RuntimeSignals == nil {
			node.RuntimeSignals = make(map[string]float64, 8)
		}
		node.RuntimeSignals[signal] = metric.Value
	case "collector_runtime_degraded_reason":
		reason := strings.TrimSpace(labelValue(metric, "reason"))
		if reason != "" && !containsString(node.RuntimeReasons, reason) {
			node.RuntimeReasons = append(node.RuntimeReasons, reason)
		}
...
const collectorMetricsPartialUpdateMetric = "collector_metrics_partial_update"
const compatPayloadSuppressedMetric = "collector_compat_payload_suppressed"

func hasCollectorPartialMetricUpdate(metrics []*telemetryv1.Metric) bool {
	for _, metric := range metrics {
		if metric == nil {
			continue
		}
		if metric.Name == collectorMetricsPartialUpdateMetric && metric.Value >= 0.5 {
			return true
		}
	}
	return false
...
func hasCompatibilityPayloadSuppressed(metrics []*telemetryv1.Metric, component string) bool {
	component = strings.TrimSpace(component)
	if component == "" {
		return false
	}
	for _, metric := range metrics {
		if metric == nil || metric.Name != compatPayloadSuppressedMetric || metric.Value < 0.5 {
			continue
		}
...
func shouldCarryForwardCollectorMetric(name string) bool {
	switch name {
	case "collector_probe_source",
	...
	return strings.HasPrefix(name, "collector_hardware_capability_") ||
		strings.HasPrefix(name, "collector_hardware_threshold_")
}

func shouldCarryForwardCompatibilityHardwareMetric(name string) bool {
	switch {
	case strings.HasPrefix(name, "node_thermal_"):
		return true
	case strings.HasPrefix(name, "node_network_interface_"):
		return true
	case strings.HasPrefix(name, "node_rdma_"):
		return true
	}
	return name == "node_network_interrupts_per_second"
}

func cloneCarryForwardMetrics(src map[string]float64, keep func(string) bool) map[string]float64 {
	if len(src) == 0 || keep == nil {
		return nil
	}
...
func cloneProbeCoreModuleMap(src map[string]*ProbeCoreModuleSample) map[string]*ProbeCoreModuleSample {
	if len(src) == 0 {
		return nil
	}
	out := make(map[string]*ProbeCoreModuleSample, len(src))
...
const maxSecurityFindingsPerNode = 256
...
func captureRuntimeEBPFMetric(node *NodeSnapshot, metric *telemetryv1.Metric, receivedAt time.Time) bool {
	...
	switch metric.Name {
	case "node_security_finding":
		finding := SecurityFinding{
			FindingID:           strings.TrimSpace(labelValue(metric, "finding_id")),
			...
			Scope:               firstNonEmpty(strings.TrimSpace(labelValue(metric, "scope")), "host"),
			Summary:             strings.TrimSpace(labelValue(metric, "summary")),
			Confidence:          metric.Value,
			RecommendedNextStep: strings.TrimSpace(labelValue(metric, "next_step")),
			Source:              strings.TrimSpace(labelValue(metric, "source")),
			PID:                 strings.TrimSpace(labelValue(metric, "pid")),
			Process:             strings.TrimSpace(labelValue(metric, "process")),
		...
		if ts := strings.TrimSpace(labelValue(metric, "ts_unix_nano")); ts != "" {
			if ns, err := strconv.ParseInt(ts, 10, 64); err == nil {
				finding.Timestamp = time.Unix(0, ns).UTC()
			}
		}
		if finding.Timestamp.IsZero() {
			finding.Timestamp = receivedAt
		}
		rawEvidence := strings.TrimSpace(labelValue(metric, "evidence"))
		if rawEvidence != "" {
			for _, item := range strings.Split(rawEvidence, "||") {
				item = strings.TrimSpace(item)
				if item != "" {
					finding.Evidence = append(finding.Evidence, item)
				}
			}
		}
		if node.SecurityFindings == nil {
			node.SecurityFindings = make([]SecurityFinding, 0, 32)
		}
		if !securityFindingExists(node.SecurityFindings, finding.FindingID) {
			node.SecurityFindings = append(node.SecurityFindings, finding)
		}
		if len(node.SecurityFindings) > maxSecurityFindingsPerNode {
			node.SecurityFindings = node.SecurityFindings[len(node.SecurityFindings)-maxSecurityFindingsPerNode:]
		}
		return true
...
func securityFindingExists(findings []SecurityFinding, findingID string) bool {
	for _, finding := range findings {
		if strings.EqualFold(finding.FindingID, findingID) {
			return true
		}
	}
	return false
...
func (s *MemoryStore) StoreBatchMeta(collectorID string, batch *telemetryv1.TelemetryBatch, receivedAt time.Time) {
	...
	if batch.WallTimeUnixNano > 0 {
		collectionAt := time.Unix(0, batch.WallTimeUnixNano).UTC()
		node.LastCollectionAt = collectionAt
		lag := receivedAt.Sub(collectionAt).Seconds() * 1000
		if lag < 0 {
			lag = 0
		}
...
func shouldStoreTrendMetric(name string) bool {
	...
	case "node_cpu_usage_percent",
		...
		"service_request_latency_p95_ms",
		"service_request_latency_p99_ms",
		"service_log_burst_count",
		"service_log_error_warn_count",
		...
		"node_gpu_power_draw_watts",
		"node_gpu_power_limit_watts",
		"node_gpu_memory_used_total_mib",
		"node_gpu_memory_used_percent",
		"node_gpu_memory_total_all_mib",
		"node_gpu_pcie_rx_mb_s",
		"node_gpu_pcie_tx_mb_s",
		"node_gpu_pcie_link_utilization_percent",
		"node_gpu_pcie_bandwidth_max_mb_s",
		"node_gpu_process_total",
		"collector_probe_core_client_available",
		"collector_probe_core_active",
		"collector_probe_core_collector_selection_valid",
		...
		"collector_protection_mode_severity",
		"collector_protection_signal_pressure",
		"collector_protection_spool_fill_ratio",
		"collector_protection_cpu_budget_ratio",
		"collector_protection_cpu_time_budget_ratio",
		"collector_protection_memory_budget_ratio",
		"collector_hardware_cpu_anomaly_score",
		"collector_hardware_memory_anomaly_score",
		"collector_hardware_disk_anomaly_score",
		"collector_hardware_gpu_anomaly_score",
		"collector_hardware_network_anomaly_score",
...
func cloneNode(node *NodeSnapshot) *NodeSnapshot {
	...
	if node.SecurityFindings != nil {
		clone.SecurityFindings = make([]SecurityFinding, 0, len(node.SecurityFindings))
		for _, finding := range node.SecurityFindings {
			copyFinding := finding
			copyFinding.Evidence = append([]string(nil), finding.Evidence...)
			clone.SecurityFindings = append(clone.SecurityFindings, copyFinding)
		}
	}
	if node.SyscallStatistics != nil {

...
func TestParseIncidentActionApprovePath(t *testing.T) {
	alertID, actionID, err := parseIncidentActionPath("alert-1/actions/action-1/approve", "approve")
	require.NoError(t, err)
	require.Equal(t, "alert-1", alertID)
	require.Equal(t, "action-1", actionID)

	_, _, err = parseIncidentActionPath("alert-1/actions/action-1/execute", "approve")
	require.Error(t, err)
}

func TestParseIncidentActionExecutePayload(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/api/v1/agent/incidents/a/actions/b/execute", strings.NewReader(`{"dry_run":false,"approval_token":"abc","approval_id":"ap-1"}`))
	...
	require.Equal(t, "ap-1", payload.ApprovalID)
	...
	require.Empty(t, payload.ApprovalID)
}

func TestParseIncidentActionRollbackPayload(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/api/v1/agent/incidents/a/actions/b/rollback", strings.NewReader(`{"dry_run":true,"approval_token":"abc","approval_id":"ap-1","rollback_id":"rb-1"}`))
...
func TestParseIncidentActionApprovePayload(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/api/v1/agent/incidents/a/actions/b/approve", strings.NewReader(`{"approval_token":"abc","operator_justification":"reviewed rollback","approved_by":"alice"}`))
	payload, err := parseIncidentActionApprovePayload(req)
	require.NoError(t, err)
	require.Equal(t, "abc", payload.ApprovalToken)
	require.Equal(t, "reviewed rollback", payload.OperatorJustification)
	require.Equal(t, "alice", payload.ApprovedBy)

	req = httptest.NewRequest(http.MethodPost, "/api/v1/agent/incidents/a/actions/b/approve", strings.NewReader(`{"unexpected":1}`))
	_, err = parseIncidentActionApprovePayload(req)
	require.Error(t, err)
}
...
func TestHandleAgentIncidentActionExecute(t *testing.T) {
	...
	req = httptest.NewRequest(http.MethodPost, "/api/v1/agent/incidents/alert-1/actions/"+unsafeActionID+"/approve", strings.NewReader(`{"approval_token":"secret-token","operator_justification":"rollback reviewed","approved_by":"ic"}`))
	resp = httptest.NewRecorder()
	controller.handleAgentIncidentByID(resp, req)
	require.Equal(t, http.StatusOK, resp.Code)
	...
	require.NoError(t, json.NewDecoder(resp.Body).Decode(&approvePayload))
	require.NotEmpty(t, approvePayload.Result.ApprovalID)
	...
	req = httptest.NewRequest(http.MethodPost, "/api/v1/agent/incidents/alert-1/actions/"+unsafeActionID+"/execute", strings.NewReader(`{"dry_run":false,"approval_id":"`+approvePayload.Result.ApprovalID+`"}`))
	resp = httptest.NewRecorder()
	controller.handleAgentIncidentByID(resp, req)
	require.Equal(t, http.StatusOK, resp.Code)
...
func TestHandleAgentJointRiskAndRCAWorkflow(t *testing.T) {
	...
	var rcaPayload struct {
		...
		Summaries []agentcore.RCAWorkflowListItem `json:"summaries"`
		Count     int                             `json:"count"`
	}
	...
	require.NotEmpty(t, rcaPayload.Reports[0].ToolCalls[0].Outcome)
	require.NotEmpty(t, rcaPayload.Reports[0].ToolCalls[0].Status)
	require.NotEmpty(t, rcaPayload.Reports[0].AgentLoop.PlanSteps)
	require.NotEmpty(t, rcaPayload.Summaries)
	require.Equal(t, rcaPayload.Reports[0].WorkflowID, rcaPayload.Summaries[0].WorkflowID)
	require.Equal(t, 8, rcaPayload.Summaries[0].ArtifactCount)
	...
	hasReadOnlyOutcome := false
	for _, call := range rcaPayload.Reports[0].ToolCalls {
		if call.Status == agentcore.WorkflowToolOutcomeReadOnlySuccess && call.Outcome == agentcore.WorkflowToolOutcomeReadOnlySuccess && strings.TrimSpace(call.InvocationStatus) != "" {
			hasReadOnlyOutcome = true
			break
		}
	}
	require.True(t, hasReadOnlyOutcome)
}
...
func TestHandleAgentWorkflowRunsAndEvidence(t *testing.T) {
	store := ingest.NewMemoryStore()
	index := logindex.NewIndex(logindex.DefaultConfig())
	seedWorkflowTelemetry(t, store, index, "collector-risk-runs")

	cfg := agentcore.DefaultWorkflowConfig()
	cfg.WorkflowDataPath = t.TempDir()
	cfg.WorkflowStorePath = filepath.Join(cfg.WorkflowDataPath, "workflow_runs.db")
	workflow := agentcore.NewWorkflowEngine(cfg, store, index, nil, zap.NewNop())

	report, err := workflow.BuildRCAWorkflow(context.Background(), agentcore.WorkflowRequest{
		CollectorID: "collector-risk-runs",
	...
	listReq := httptest.NewRequest(http.MethodGet, "/api/v1/agent/workflow/runs?limit=10", nil)
	listResp := httptest.NewRecorder()
	controller.handleAgentWorkflowRuns(listResp, listReq)
	require.Equal(t, http.StatusOK, listResp.Code)

	var listPayload struct {
		Runs  []agentcore.DurableRun `json:"runs"`
		Count int                    `json:"count"`
	}
	require.NoError(t, json.NewDecoder(listResp.Body).Decode(&listPayload))
	require.GreaterOrEqual(t, listPayload.Count, 1)

	detailReq := httptest.NewRequest(http.MethodGet, "/api/v1/agent/workflow/runs/"+report.WorkflowID, nil)
	detailResp := httptest.NewRecorder()
	controller.handleAgentWorkflowRuns(detailResp, detailReq)
	require.Equal(t, http.StatusOK, detailResp.Code)

	var detailPayload struct {
		Run              agentcore.DurableRun                `json:"run"`
		ArtifactManifest *agentcore.WorkflowArtifactManifest `json:"artifact_manifest"`
	}
	require.NoError(t, json.NewDecoder(detailResp.Body).Decode(&detailPayload))
	require.Equal(t, report.WorkflowID, detailPayload.Run.RunID)
	require.NotNil(t, detailPayload.Run.EvidencePackage)
	require.NotNil(t, detailPayload.ArtifactManifest)
	require.Len(t, detailPayload.ArtifactManifest.Artifacts, 8)

	evidenceReq := httptest.NewRequest(http.MethodGet, "/api/v1/agent/workflow/evidence/"+report.WorkflowID, nil)
	evidenceResp := httptest.NewRecorder()
	controller.handleAgentWorkflowEvidence(evidenceResp, evidenceReq)
	require.Equal(t, http.StatusOK, evidenceResp.Code)
	require.Contains(t, evidenceResp.Body.String(), report.WorkflowID)

	artifactsReq := httptest.NewRequest(http.MethodGet, "/api/v1/agent/workflow/artifacts/"+report.WorkflowID, nil)
	artifactsResp := httptest.NewRecorder()
	controller.handleAgentWorkflowArtifacts(artifactsResp, artifactsReq)
	require.Equal(t, http.StatusOK, artifactsResp.Code)

	var artifactsPayload struct {
		RunID            string                             `json:"run_id"`
		ArtifactChain    agentcore.WorkflowArtifactChain    `json:"artifact_chain"`
		ArtifactManifest agentcore.WorkflowArtifactManifest `json:"artifact_manifest"`
	}
	require.NoError(t, json.NewDecoder(artifactsResp.Body).Decode(&artifactsPayload))
	require.Equal(t, report.WorkflowID, artifactsPayload.RunID)
	require.Equal(t, report.WorkflowID, artifactsPayload.ArtifactChain.RunID)
	require.Equal(t, report.WorkflowID, artifactsPayload.ArtifactManifest.RunID)
	require.Equal(t, "observation_summary", string(artifactsPayload.ArtifactChain.Observation.Meta.Kind))
}

func TestHandleAgentWorkflowEvidenceReturnsDegradedErrorWhenPayloadMissing(t *testing.T) {
	store := ingest.NewMemoryStore()
	index := logindex.NewIndex(logindex.DefaultConfig())
	seedWorkflowTelemetry(t, store, index, "collector-risk-evidence-missing")

	cfg := agentcore.DefaultWorkflowConfig()
	cfg.WorkflowDataPath = t.TempDir()
	cfg.WorkflowStorePath = filepath.Join(cfg.WorkflowDataPath, "workflow_runs.db")
	workflow := agentcore.NewWorkflowEngine(cfg, store, index, nil, zap.NewNop())
	t.Cleanup(func() { require.NoError(t, workflow.Close()) })

	report, err := workflow.BuildRCAWorkflow(context.Background(), agentcore.WorkflowRequest{
		CollectorID: "collector-risk-evidence-missing",
		Window:      45 * time.Minute,
		Trigger:     "incident_alert",
	})
	require.NoError(t, err)
	require.NotNil(t, report.EvidencePackageArtifact)
	require.NoError(t, os.Remove(report.EvidencePackagePath))

	controller := &Controller{agentWorkflow: workflow}
	req := httptest.NewRequest(http.MethodGet, "/api/v1/agent/workflow/evidence/"+report.WorkflowID, nil)
	resp := httptest.NewRecorder()
	controller.handleAgentWorkflowEvidence(resp, req)

	require.Equal(t, http.StatusServiceUnavailable, resp.Code)
	require.Contains(t, resp.Body.String(), "payload is unavailable")
}

...
func TestParseIncidentActionExecutePath(t *testing.T) {
	alertID, actionID, err := parseIncidentActionExecutePath("alert-1/actions/action-1/execute")
	require.NoError(t, err)
	require.Equal(t, "alert-1", alertID)
	require.Equal(t, "action-1", actionID)

	_, _, err = parseIncidentActionExecutePath("alert-1/context")
	require.Error(t, err)
}

func TestParseIncidentActionRollbackPath(t *testing.T) {
	alertID, actionID, err := parseIncidentActionRollbackPath("alert-1/actions/action-1/rollback")
	require.NoError(t, err)
	require.Equal(t, "alert-1", alertID)
	require.Equal(t, "action-1", actionID)

	_, _, err = parseIncidentActionRollbackPath("alert-1/actions/action-1/execute")
	require.Error(t, err)
}

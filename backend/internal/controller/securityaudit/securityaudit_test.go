...
func TestEvaluatorIncludesCollectorSecurityFindingEnvelope(t *testing.T) {
	store := ingest.NewMemoryStore()
	index := logindex.NewIndex(logindex.DefaultConfig())
	collectorID := "collector-sec-envelope"
	now := time.Now().UTC()
	store.UpsertCollector(&telemetryv1.CollectorInfo{CollectorId: collectorID, Hostname: "sec-envelope"}, now)
	store.StoreMetrics(collectorID, []*telemetryv1.Metric{{
		Name:              "node_security_finding",
		Value:             0.89,
		TimestampUnixNano: now.UnixNano(),
		Labels: []*telemetryv1.Label{
			{Key: "finding_id", Value: "sf-envelope"},
			{Key: "category", Value: "sensitive_path_access"},
			{Key: "severity", Value: "high"},
			{Key: "scope", Value: "runtime"},
			{Key: "summary", Value: "Runtime access to sensitive files or directories was observed"},
			{Key: "evidence", Value: "path=/etc/shadow || pid=42"},
			{Key: "next_step", Value: "Confirm access intent"},
		},
	}}, now)

	findings := NewEvaluator(store, index).Findings(Options{CollectorID: collectorID, Window: time.Hour, Limit: 20})
	require.NotEmpty(t, findings)
	require.Equal(t, "sf-envelope", findings[0].FindingID)
	require.Equal(t, "sensitive_path_access", findings[0].Category)
}

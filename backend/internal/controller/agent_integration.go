...
func (c *Controller) registerAgentHandlers(mux *http.ServeMux) {
	...
	mux.HandleFunc("/api/v1/agent/workflow/runs", c.withCORS(func(w http.ResponseWriter, r *http.Request) {
		c.handleAgentWorkflowRuns(w, r)
	}))
	mux.HandleFunc("/api/v1/agent/workflow/runs/", c.withCORS(func(w http.ResponseWriter, r *http.Request) {
		c.handleAgentWorkflowRuns(w, r)
	}))
	mux.HandleFunc("/api/v1/agent/workflow/evidence/", c.withCORS(func(w http.ResponseWriter, r *http.Request) {
		c.handleAgentWorkflowEvidence(w, r)
	}))
	mux.HandleFunc("/api/v1/agent/workflow/artifacts/", c.withCORS(func(w http.ResponseWriter, r *http.Request) {
		c.handleAgentWorkflowArtifacts(w, r)
	}))
...
func (c *Controller) handleAgentStatus(w http.ResponseWriter, r *http.Request) {
	...
	var queryServiceStatus any
	var controlPlaneSummary any
	var controlPlaneMetrics any
	reportEngineStatus := c.agentEngine.Status()
	if c.agentWorkflow != nil {
		...
		controlPlaneSummary = c.agentWorkflow.ControlPlaneSummary()
		controlPlaneMetrics = c.agentWorkflow.Metrics()
	}
	if c.agentService != nil {
		queryServiceStatus = c.agentService.Status()
	}
	writeJSON(w, map[string]interface{}{
		"status":                  "active",
		...
		"report_engine":           reportEngineStatus,
		"query_service":           queryServiceStatus,
		"timestamp":               time.Now(),
	})
}
...
func (c *Controller) handleAgentIncidentByID(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasSuffix(path, "/approve") && strings.Contains(path, "/actions/") {
		if !allowMethod(w, r, http.MethodPost) {
			return
		}
		c.handleAgentIncidentActionApprove(w, r, path)
		return
	}
...
func (c *Controller) handleAgentIncidentActionApprove(w http.ResponseWriter, r *http.Request, path string) {
	if !c.requireActiveController(w) {
		return
	}
	alertID, actionID, err := parseIncidentActionPath(path, "approve")
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}

	req, err := parseIncidentActionApprovePayload(r)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}

	result, err := c.agentEngine.ApproveIncidentAction(alertID, actionID, req)
	if err != nil {
		...
		case errors.Is(err, agent.ErrIncidentActionApprovalExpired):
...
func (c *Controller) handleAgentIncidentActionExecute(w http.ResponseWriter, r *http.Request, path string) {
	...
	alertID, actionID, err := parseIncidentActionPath(path, "execute")
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
...
func (c *Controller) handleAgentIncidentActionRollback(w http.ResponseWriter, r *http.Request, path string) {
	...
	alertID, actionID, err := parseIncidentActionPath(path, "rollback")
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
...
func (c *Controller) handleAgentRCAWorkflow(w http.ResponseWriter, r *http.Request) {
	...
	summaries := make([]agentcore.RCAWorkflowListItem, 0, len(reports))
	for _, report := range reports {
		summaries = append(summaries, agentcore.BuildRCAWorkflowListItem(report))
	}
...
func (c *Controller) handleAgentWorkflowRuns(w http.ResponseWriter, r *http.Request) {
	if !allowMethod(w, r, http.MethodGet) {
		return
	}
	if !c.requireAgentWorkflow(w) {
	...
	runID := ""
	if r.URL.Path != "/api/v1/agent/workflow/runs" {
		runID = strings.Trim(strings.TrimPrefix(r.URL.Path, "/api/v1/agent/workflow/runs/"), "/")
	}
	if runID != "" {
		run, err := c.agentWorkflow.DurableRun(r.Context(), runID)
		if err != nil {
			http.Error(w, "workflow run not found", http.StatusNotFound)
			return
		}
		var manifest any
		if artifactManifest, _, manifestErr := c.agentWorkflow.ArtifactManifest(r.Context(), runID); manifestErr == nil {
	...
	workflowType := strings.TrimSpace(r.URL.Query().Get("workflow_type"))
	runs, err := c.agentWorkflow.DurableRuns(r.Context(), workflowType, limit)
	if err != nil {
...
func (c *Controller) handleAgentWorkflowEvidence(w http.ResponseWriter, r *http.Request) {
	if !allowMethod(w, r, http.MethodGet) {
		return
	}
	if !c.requireAgentWorkflow(w) {
	...
	runID := strings.Trim(strings.TrimPrefix(r.URL.Path, "/api/v1/agent/workflow/evidence/"), "/")
	...
	if run.EvidencePackage == nil {
		http.Error(w, "evidence package not found", http.StatusNotFound)
		return
	}
	raw, readErr := c.agentWorkflow.ReadArtifact(r.Context(), (*agentcore.DurableArtifactRef)(run.EvidencePackage))
	if readErr != nil {
		http.Error(w, "evidence package metadata exists but payload is unavailable: "+readErr.Error(), http.StatusServiceUnavailable)
		return
	}
...
func (c *Controller) handleAgentWorkflowArtifacts(w http.ResponseWriter, r *http.Request) {
	if !allowMethod(w, r, http.MethodGet) {
		return
	}
	if !c.requireAgentWorkflow(w) {
	...
	runID := strings.Trim(strings.TrimPrefix(r.URL.Path, "/api/v1/agent/workflow/artifacts/"), "/")
	if runID == "" || strings.Contains(runID, "/") {
		http.Error(w, "workflow id required", http.StatusBadRequest)
		return
	}
	chain, evidenceRef, err := c.agentWorkflow.ArtifactChain(r.Context(), runID)
	if err != nil {
		status := http.StatusNotFound
...
func parseNodePath(path string) (string, error) {
	...
	node = strings.Trim(node, "/")
	if node == "" {
		return "", errors.New("node required")
	}
...
func parseAgentActionUpdatePayload(r *http.Request) (string, string, error) {
	...
	if status == "" && note == "" {
		return "", "", errors.New("status or note required")
	}
...
func parseIncidentActionPath(path, operation string) (string, string, error) {
	parts := strings.Split(strings.Trim(path, "/"), "/")
	if len(parts) != 4 || parts[1] != "actions" || parts[3] != operation {
		if operation == "rollback" {
			return "", "", errors.New("invalid incident rollback path")
		}
		if operation == "approve" {
			return "", "", errors.New("invalid incident approval path")
		}
		return "", "", errors.New("invalid incident action path")
	}
...
func parseIncidentActionExecutePayload(r *http.Request) (agent.IncidentActionExecuteRequest, error) {
	...
	if err := decodeJSONBodyStrict(r, &payload, true); err != nil {
		return agent.IncidentActionExecuteRequest{}, err
	}
...
func parseIncidentActionRollbackPayload(r *http.Request) (agent.IncidentActionRollbackRequest, error) {
	...
	if err := decodeJSONBodyStrict(r, &payload, true); err != nil {
		return agent.IncidentActionRollbackRequest{}, err
	}
...
func parseIncidentActionApprovePayload(r *http.Request) (agent.IncidentActionApproveRequest, error) {
	var payload struct {
		ApprovalToken         string `json:"approval_token"`
		OperatorJustification string `json:"operator_justification"`
		ApprovedBy            string `json:"approved_by"`
	}
	if err := decodeJSONBodyStrict(r, &payload, true); err != nil {
		return agent.IncidentActionApproveRequest{}, err
	}
	return agent.IncidentActionApproveRequest{
		ApprovalToken:         strings.TrimSpace(payload.ApprovalToken),
		OperatorJustification: strings.TrimSpace(payload.OperatorJustification),
		ApprovedBy:            strings.TrimSpace(payload.ApprovedBy),
	}, nil
}

func decodeJSONBodyStrict(r *http.Request, target any, allowEmpty bool) error {

...
func New(cfg Config, logger *zap.Logger) (*Controller, error) {
	...
	if cfg.Agent.Enabled {
		...
		queryCfg.APIKey = nonEmptyString(os.Getenv("SRE_AGENT_LLM_API_KEY"), os.Getenv("SRE_AGENT_JIMMYNIGHT_API_KEY"), os.Getenv("OPENAI_API_KEY"), os.Getenv("LLM_API_KEY"))
		queryCfg.RAG = parseBoolEnv("SRE_AGENT_RAG_ENABLED", queryCfg.RAG)
		queryCfg.RAGPath = nonEmptyString(os.Getenv("SRE_AGENT_RAG_INDEX_PATH"), queryCfg.RAGPath)
		queryCfg.RAGTopK = parseIntEnv("SRE_AGENT_RAG_TOP_K", queryCfg.RAGTopK)
		queryCfg.RAGChars = parseIntEnv("SRE_AGENT_RAG_MAX_SNIPPET_CHARS", queryCfg.RAGChars)
		...
		c.agentWorkflow = agentcore.NewWorkflowEngine(
			agentcore.DefaultWorkflowConfig(),
			c.ingestStore,
			c.logIndex,
			topologyProvider,
			c.logger,
...
func (c *Controller) Start(ctx context.Context) error {
	...
	standby := c.isStandby()
	if standby {
		c.logger.Info("controller HA standby mode enabled; write and automation loops are disabled")
	}

	if c.analysisExt != nil && !standby {
		if err := c.analysisExt.engine.Start(c.ctx); err != nil {
			return err
		}
	}
	if c.agentEngine != nil && !standby {
		if err := c.agentEngine.Start(c.ctx); err != nil {
			return err
		}
	}
	if c.incidentCoordinator != nil && !standby {
		c.incidentCoordinator.Start(c.ctx)
	}

	if c.gpuStore != nil && !standby {
	...
	if c.orchestrationManager != nil && !standby {
		if err := c.orchestrationManager.Start(c.ctx); err != nil {
			return err
		}
	}

	if c.checks != nil && !standby {

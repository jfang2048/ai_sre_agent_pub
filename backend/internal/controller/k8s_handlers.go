...
func (c *Controller) handleK8sStatus(w http.ResponseWriter, r *http.Request) {
	...
	if c.k8sManager == nil {
		...
		_ = json.NewEncoder(w).Encode(k8sview.Status{
			Enabled:         false,
			Running:         false,
			RefreshInterval: "0s",
			RequestTimeout:  "0s",
			Metrics:         k8sview.MetricsSnapshot{},
		})
		return
	}
...
func (c *Controller) handleK8sClusters(w http.ResponseWriter, r *http.Request) {
	...
	if c.k8sManager == nil {
		...
		_ = json.NewEncoder(w).Encode(map[string]interface{}{
			"enabled":      false,
			"clusters":     []k8sview.ClusterSummary{},
			"count":        0,
...
func (c *Controller) handleK8sTopology(w http.ResponseWriter, r *http.Request) {
	...
	if c.k8sManager == nil {
		...
		_ = json.NewEncoder(w).Encode(map[string]interface{}{
			"enabled":      false,
			"generated_at": time.Now(),
			"nodes":        []k8sview.ServiceMapNode{},
			"edges":        []k8sview.ServiceMapEdge{},
		})
		return
	}

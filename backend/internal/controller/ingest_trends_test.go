...
func TestHandleFleetTimeseriesIncludesTelemetryQuality(t *testing.T) {
	store := ingest.NewMemoryStore()
	ctrl := &Controller{ingestStore: store}
	now := time.Now().Add(-10 * time.Minute).Truncate(time.Second)

	store.UpsertCollector(&telemetryv1.CollectorInfo{CollectorId: "collector-quality", Hostname: "node-quality"}, now)
	store.StoreBatchMeta("collector-quality", &telemetryv1.TelemetryBatch{
		BatchId:          "quality-1",
		WallTimeUnixNano: now.UnixNano(),
		Collector: &telemetryv1.CollectorInfo{
			CollectorId: "collector-quality",
			Hostname:    "node-quality",
		},
	}, now.Add(45*time.Second))
	store.StoreMetrics("collector-quality", []*telemetryv1.Metric{
		{Name: "node_cpu_usage_percent", Value: 42},
		{Name: "collector_probe_source", Value: 1, Labels: []*telemetryv1.Label{{Key: "source", Value: "go"}}},
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/fleet/timeseries?collector_id=collector-quality&window=30m", nil)
	w := httptest.NewRecorder()
	ctrl.handleFleetTimeseries(w, req)
	...
	if assert.Equal(t, http.StatusOK, w.Code) {
		...
		err := json.NewDecoder(w.Body).Decode(&resp)
		if assert.NoError(t, err) {
			assert.Equal(t, telemetryStateStale, resp.TelemetryQuality.State)
			assert.Equal(t, "go", resp.TelemetryQuality.SourceMode)
			assert.Greater(t, resp.TelemetryQuality.IngestDelaySeconds, 40.0)
			assert.Contains(t, resp.TelemetryQuality.DegradedReasons, "collector replay backlog is still draining")
			assert.NotEmpty(t, resp.TelemetryQuality.MissingSignals)
			assert.Less(t, resp.TelemetryQuality.Confidence, 1.0)
		}
	}
}

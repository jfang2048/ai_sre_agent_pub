...
func New(cfg Config) *Store {
	...
	if strings.HasSuffix(filepath.Base(os.Args[0]), ".test") && cfg.PersistDir == def.PersistDir {
		cfg.PersistDir = filepath.Join(os.TempDir(), "ai_sre_agent_gpuobs_tests", strings.TrimSuffix(filepath.Base(os.Args[0]), ".test"))
	}

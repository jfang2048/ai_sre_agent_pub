...
func TestNewControllerAuthEnabledWithoutTokenSecretFails(t *testing.T) {
	logger, _ := zap.NewDevelopment()
	cfg := DefaultConfig()
	...
	if !strings.Contains(err.Error(), "token secret env") {
		t.Fatalf("expected auth configuration error, got: %v", err)
	}
}

func TestNewControllerAuthEnabledWithTokenSecretSucceeds(t *testing.T) {
	logger, _ := zap.NewDevelopment()
	cfg := DefaultConfig()
	cfg.Auth.Enabled = true
	cfg.Auth.TokenSecretEnv = "TEST_SRE_AGENT_CONTROLLER_TOKEN_SECRET"
...
func TestNewControllerAuthDisabledOutsideLocalDevRequiresOverride(t *testing.T) {
	logger, _ := zap.NewDevelopment()
	cfg := DefaultConfig()
	...
	if ctrl != nil {
		t.Fatal("New() returned non-nil controller on insecure non-local auth posture")
	}
	if !strings.Contains(err.Error(), "allow_insecure_disable") {
		t.Fatalf("expected insecure override guidance, got: %v", err)
	}
}

func TestNewControllerAuthDisabledOutsideLocalDevWithOverrideSucceeds(t *testing.T) {
	logger, _ := zap.NewDevelopment()
	cfg := DefaultConfig()
	cfg.Deployment.Mode = "cluster-lite"
	...
	if !ctrl.auth.InsecureOverride {
		t.Fatal("expected insecure override to be visible in resolved auth state")
	}
}
...
func TestControllerHandleStatus(t *testing.T) {
	...
	if apiBlock["action_rate_limit_enabled"] != cfg.API.ActionRateLimitEnabled {
		t.Errorf("api.action_rate_limit_enabled = %v, want %v", apiBlock["action_rate_limit_enabled"], cfg.API.ActionRateLimitEnabled)
	}
	haBlock, ok := resp["ha"].(map[string]interface{})
	if !ok {
		t.Fatalf("ha block missing or wrong type: %#v", resp["ha"])
	}
	if haBlock["write_sensitive_requests_guarded"] != cfg.HA.Enabled {
		t.Errorf("ha.write_sensitive_requests_guarded = %v, want %v", haBlock["write_sensitive_requests_guarded"], cfg.HA.Enabled)
	}
	if haBlock["grpc_ingest_writes_guarded"] != true {
		t.Errorf("ha.grpc_ingest_writes_guarded = %v, want true", haBlock["grpc_ingest_writes_guarded"])
	}
}
...
func TestControllerHandleHAStatus(t *testing.T) {
	...
	var resp struct {
		...
		WriteSensitiveRequestsGuarded bool   `json:"write_sensitive_requests_guarded"`
		WriteSensitiveRequestsBlocked bool   `json:"write_sensitive_requests_blocked"`
	...
	if !resp.WriteSensitiveRequestsGuarded {
		t.Fatalf("write_sensitive_requests_guarded = false, want true")
	}
	if !resp.WriteSensitiveRequestsBlocked {
		t.Fatalf("write_sensitive_requests_blocked = false, want true")
	}
	if !resp.GRPCIngestWritesGuarded {
		t.Fatalf("grpc_ingest_writes_guarded = false, want true")
	}
	if !resp.GRPCIngestWritesBlocked {
		t.Fatalf("grpc_ingest_writes_blocked = false, want true in standby")
	}
}
...
func TestControllerStandbyCentralWriteGuardRejectsActionScope(t *testing.T) {
	logger, _ := zap.NewDevelopment()
	cfg := DefaultConfig()
	cfg.HA.Enabled = true
	cfg.HA.Mode = "standby"
	...
	handler := ctrl.wrapHTTPHandler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))

	actionReq := httptest.NewRequest(http.MethodPost, "/api/v1/controller/actions/execute", nil)
	actionResp := httptest.NewRecorder()
	handler.ServeHTTP(actionResp, actionReq)
	if actionResp.Code != http.StatusServiceUnavailable {
		t.Fatalf("action status = %d, want %d", actionResp.Code, http.StatusServiceUnavailable)
	}

	readReq := httptest.NewRequest(http.MethodGet, "/api/v1/status", nil)
	readResp := httptest.NewRecorder()
	handler.ServeHTTP(readResp, readReq)
	if readResp.Code != http.StatusOK {
		t.Fatalf("read status = %d, want %d", readResp.Code, http.StatusOK)
	}
}
...
func TestControllerHTTPMutationAudit(t *testing.T) {
	...
	if last.Outcome != agentcore.WorkflowToolOutcomeExecutedSuccess {
		t.Fatalf("outcome = %q, want %q", last.Outcome, agentcore.WorkflowToolOutcomeExecutedSuccess)
	}
...
func TestControllerHTTPActionRateLimitOnlyBlocksActionScope(t *testing.T) {
	logger, _ := zap.NewDevelopment()
	cfg := DefaultConfig()
	cfg.API.ActionRateLimitEnabled = true
	...
	readReq := httptest.NewRequest(http.MethodGet, "/api/v1/status", nil)
	readFirst := httptest.NewRecorder()
	handler.ServeHTTP(readFirst, readReq)
	if readFirst.Code != http.StatusOK {
		t.Fatalf("read first status = %d, want %d", readFirst.Code, http.StatusOK)
	}
	readSecond := httptest.NewRecorder()
	handler.ServeHTTP(readSecond, readReq)
	if readSecond.Code != http.StatusOK {
		t.Fatalf("read second status = %d, want %d", readSecond.Code, http.StatusOK)
	}

	actionReq := httptest.NewRequest(http.MethodPost, "/api/v1/controller/actions/execute", nil)
	actionFirst := httptest.NewRecorder()
	handler.ServeHTTP(actionFirst, actionReq)
	if actionFirst.Code != http.StatusOK {
		t.Fatalf("action first status = %d, want %d", actionFirst.Code, http.StatusOK)
	}
	actionSecond := httptest.NewRecorder()
	handler.ServeHTTP(actionSecond, actionReq)
	if actionSecond.Code != http.StatusTooManyRequests {
		t.Fatalf("action second status = %d, want %d", actionSecond.Code, http.StatusTooManyRequests)
	}
}
...
func TestControllerHandlePrometheusMetricsIncludesWorkflowObservability(t *testing.T) {
	...
	for _, metric := range []string{
		"sre_agent_skill_invocations_total",
		"sre_agent_skill_duration_seconds_count",
		"sre_agent_skill_low_yield_total",
		"sre_agent_skill_policy_block_total",
		"sre_agent_skill_approval_required_total",
		"sre_agent_skill_score_count",
		"sre_agent_adaptive_stop_total",
		"sre_agent_rag_skill_calls_total",
		"sre_agent_artifact_persist_failures_total",
		"sre_agent_replay_validation_failures_total",
	} {
		if !strings.Contains(body, metric) {
			t.Fatalf("missing %s metric", metric)
		}
	}
}
...
func TestControllerCORS(t *testing.T) {
	...
	if !strings.Contains(w.Header().Get("Access-Control-Allow-Methods"), "PATCH") {
		t.Error("PATCH missing from CORS allow methods")
	}
	if !strings.Contains(w.Header().Get("Access-Control-Allow-Headers"), "X-API-Key") {
		t.Error("X-API-Key missing from CORS allow headers")
	}
	...
	if w.Code != http.StatusForbidden {
		t.Fatalf("disallowed origin status = %d, want %d", w.Code, http.StatusForbidden)
	}

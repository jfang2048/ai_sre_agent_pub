...
import (
	...
	"github.com/jfang2048/ai_sre_agent_pub/internal/controller/change"
...
func (m *SREManager) handleSLOSummary(w http.ResponseWriter, r *http.Request) {
	...
	m.mu.RLock()
	violationsCount := len(m.sloViolations)
	activeIncidentsCount := len(m.activeIncidents)
	activeChangesCount := len(m.activeChanges)
	m.mu.RUnlock()

	summary := map[string]interface{}{
		"violations_count": violationsCount,
		"active_incidents": activeIncidentsCount,
		"active_changes":   activeChangesCount,
	}

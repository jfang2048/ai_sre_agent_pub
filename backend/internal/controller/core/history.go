...
func (h *MetricsHistory) GetSince(since time.Time) []MetricsSample {
	...
	h.ring.ForEachOldest(func(s MetricsSample) {
		if s.Timestamp.After(since) {
			out = append(out, cloneMetricsSample(s))
		}
	})
	return out
}
...
func (h *MetricsHistory) GetLastN(n int) []MetricsSample {
	...
	for i := range out {
		out[i] = cloneMetricsSample(out[i])
	}
	return out
}
...
func cloneMetricsSample(sample MetricsSample) MetricsSample {
	clone := sample
	if len(sample.Metrics) == 0 {
		return clone
	}
	clone.Metrics = make(map[string]float64, len(sample.Metrics))

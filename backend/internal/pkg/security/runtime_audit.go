...
func checkControllerAuthAndExposure(root string) RuntimeSecurityCheck {
	...
	if publicHTTP && !authEnabled {
		...
		check.Recommendations = append(check.Recommendations,
			"Enable auth.enabled and provide SRE_AGENT_CONTROLLER_API_KEY before exposing controller HTTP externally.",
	...
	if authEnabled && strings.TrimSpace(cfg.Auth.APIKeyEnv) == "" {
		if check.Status == RuntimeStatusPass {
			check.Status = RuntimeStatusWarn
		}
		check.Message = "controller API auth is enabled but api_key_env is empty"
		check.Evidence = append(check.Evidence, "auth.api_key_env is empty")
		check.Recommendations = append(check.Recommendations,
			"Set auth.api_key_env explicitly to SRE_AGENT_CONTROLLER_API_KEY (or an equivalent managed secret env var).",
		)
	}

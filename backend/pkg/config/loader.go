...
type ServerConfig struct {
	...
	ControllerURL string `yaml:"controller_url" mapstructure:"controller_url"`
}
...
const envConfigPathKey = "SRE_AGENT_CONFIG"
...
func (l *Loader) Load(configPath string) (*Config, error) {
	...
	if configPath == "" {
		configPath = strings.TrimSpace(os.Getenv(envConfigPathKey))
	}

	if configPath != "" {
		l.v.SetConfigFile(configPath)
	} else {
		// Try common project and system locations.
		l.v.AddConfigPath(".")
...
func (l *Loader) prepareBaseConfig() {
	l.v.SetConfigName("default")
	l.v.SetConfigType("yaml")
	l.prepareEnvOverrides()
}

func (l *Loader) prepareEnvOverrides() {

...
func buildRuntime(cmd *cobra.Command, flags *cliFlags) (*zap.Logger, collector.Config, *collector.Collector, *http.Server, func(), error) {
	...
	metricsServer, err := startMetricsServer(cfg.MetricsListenAddress, logger, collectorRuntime)
	if err != nil {
		return nil, collector.Config{}, nil, nil, nil, err
	}
...
func startMetricsServer(addr string, logger *zap.Logger, runtimeCollector *collector.Collector) (*http.Server, error) {
	if addr == "" {
		return nil, nil
	}

	mux := buildMetricsHandler(runtimeCollector)
...
func buildMetricsHandler(runtimeCollector *collector.Collector) http.Handler {
	...
	mux.HandleFunc("/api/v1/status", func(w http.ResponseWriter, r *http.Request) {
		...
		status := runtimeCollector.StatusSnapshot()
		if status.Version == "" {
			status.Version = release.EffectiveVersion()
		}

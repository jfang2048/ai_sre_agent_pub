...
func newMintTokenCmd() *cobra.Command {
	defaults := controller.DefaultAuthConfig()
	var (
		secretEnv   string
		subject     string
		actorType   string
	...
	cmd := &cobra.Command{
		Use:   "mint-token",
		Short: "Mint a signed controller or ingest bearer token",
		RunE: func(cmd *cobra.Command, args []string) error {
			secretEnv = strings.TrimSpace(firstNonEmpty(secretEnv, defaults.TokenSecretEnv))
			secret := strings.TrimSpace(os.Getenv(secretEnv))
			if secret == "" {
				return fmt.Errorf("token secret env %q is empty", secretEnv)
			}
			roles := identity.ParseRolesCSV(rolesCSV)
			if len(roles) == 0 {
			...
			token, err := identity.MintToken(
				...
				identity.NormalizeActorType(actorType),
				roles,
				firstNonEmpty(strings.TrimSpace(issuer), defaults.TokenIssuer),
				firstNonEmpty(strings.TrimSpace(audience), defaults.TokenAudience),
	...
	cmd.Flags().StringVar(&secretEnv, "secret-env", defaults.TokenSecretEnv, "Environment variable holding the signing secret")
	cmd.Flags().StringVar(&subject, "subject", "", "Token subject")
	cmd.Flags().StringVar(&actorType, "actor-type", string(identity.ActorTypeUser), "Actor type: user, service, internal")
	cmd.Flags().StringVar(&rolesCSV, "roles", "", "Comma-separated roles: viewer,operator,approver,admin,collector")
	cmd.Flags().StringVar(&audience, "audience", defaults.TokenAudience, "Token audience")
	cmd.Flags().StringVar(&issuer, "issuer", defaults.TokenIssuer, "Token issuer")
	cmd.Flags().StringVar(&collectorID, "collector-id", "", "Collector ID bound into a collector service token")
...
func runController(cmd *cobra.Command, args []string) {
	if showVersion {
		fmt.Printf("SRE Controller %s (commit: %s, built: %s)\n", release.EffectiveVersion(), release.Commit, release.BuildDate)
		os.Exit(0)
	}
	...
	logger = logger.With(zap.String("version", release.EffectiveVersion()), zap.String("commit", release.Commit))
...
func buildConfig(logger *zap.Logger) (controller.Config, error) {
	...
	if env := os.Getenv("SRE_CONTROLLER_INSECURE_OVERRIDE"); env != "" {
		cfg.Deployment.InsecureOverride = strings.EqualFold(env, "1") || strings.EqualFold(env, "true")
	} else if env := os.Getenv("SRE_DEPLOYMENT_INSECURE_OVERRIDE"); env != "" {
		cfg.Deployment.InsecureOverride = strings.EqualFold(env, "1") || strings.EqualFold(env, "true")
	}
	...
	if env := os.Getenv("SRE_CONTROLLER_API_ACTION_RATE_LIMIT_ENABLED"); env != "" {
		cfg.API.ActionRateLimitEnabled = strings.EqualFold(env, "1") || strings.EqualFold(env, "true")
	}
	if env := os.Getenv("SRE_CONTROLLER_API_ACTION_RATE_LIMIT_RPS"); env != "" {
		if v, err := strconv.ParseFloat(strings.TrimSpace(env), 64); err == nil {
			cfg.API.ActionRateLimitRPS = v
		}
	}
	if env := os.Getenv("SRE_CONTROLLER_API_ACTION_RATE_LIMIT_BURST"); env != "" {
		if v, err := strconv.Atoi(strings.TrimSpace(env)); err == nil {
			cfg.API.ActionRateLimitBurst = v
		}
	}
	...
	if env := strings.TrimSpace(os.Getenv("SRE_CONTROLLER_API_ALLOWED_ORIGINS")); env != "" {
		cfg.API.AllowedOrigins = parseCSV(env)
	}
	if env := os.Getenv("SRE_CONTROLLER_AUTH_ENABLED"); env != "" {
		cfg.Auth.Enabled = strings.EqualFold(env, "1") || strings.EqualFold(env, "true")
	}
	if env := os.Getenv("SRE_CONTROLLER_AUTH_ALLOW_INSECURE_DISABLE"); env != "" {
		cfg.Auth.AllowInsecureDisable = strings.EqualFold(env, "1") || strings.EqualFold(env, "true")
	}
	if env := strings.TrimSpace(os.Getenv("SRE_CONTROLLER_AUTH_MODE")); env != "" {
		cfg.Auth.Mode = env
	}
	if env := strings.TrimSpace(os.Getenv("SRE_CONTROLLER_AUTH_TOKEN_SECRET_ENV")); env != "" {
		cfg.Auth.TokenSecretEnv = env
	}
	if env := strings.TrimSpace(os.Getenv("SRE_CONTROLLER_AUTH_TOKEN_ISSUER")); env != "" {
		cfg.Auth.TokenIssuer = env
	}
	if env := strings.TrimSpace(os.Getenv("SRE_CONTROLLER_AUTH_TOKEN_AUDIENCE")); env != "" {
		cfg.Auth.TokenAudience = env
	}
	if env := os.Getenv("SRE_CONTROLLER_AUTH_INGEST_ENABLED"); env != "" {
		cfg.Auth.IngestAuthEnabled = strings.EqualFold(env, "1") || strings.EqualFold(env, "true")
	}
	if env := strings.TrimSpace(os.Getenv("SRE_CONTROLLER_AUTH_INGEST_TOKEN_AUDIENCE")); env != "" {
		cfg.Auth.IngestTokenAudience = env
	}
	if env := strings.TrimSpace(os.Getenv("SRE_CONTROLLER_AUTH_API_KEY_ENV")); env != "" {
		cfg.Auth.APIKeyEnv = env
	}
	if env := strings.TrimSpace(os.Getenv("SRE_CONTROLLER_AUTH_READ_API_KEY_ENV")); env != "" {
		cfg.Auth.ReadAPIKeyEnv = env
	}
	if env := strings.TrimSpace(os.Getenv("SRE_CONTROLLER_AUTH_ACTION_API_KEY_ENV")); env != "" {
		cfg.Auth.ActionAPIKeyEnv = env
	}
	...
	if env := os.Getenv("SRE_INGEST_TLS_ENABLED"); env != "" {
		cfg.Ingest.Transport.TLS.Enabled = strings.EqualFold(env, "1") || strings.EqualFold(env, "true")
	}
	if env := os.Getenv("SRE_INGEST_TLS_REQUIRE_CLIENT_CERT"); env != "" {
		cfg.Ingest.Transport.TLS.RequireClientCert = strings.EqualFold(env, "1") || strings.EqualFold(env, "true")
	}
	if env := strings.TrimSpace(os.Getenv("SRE_INGEST_TLS_CA_FILE")); env != "" {
		cfg.Ingest.Transport.TLS.CAFile = env
	}
	if env := strings.TrimSpace(os.Getenv("SRE_INGEST_TLS_CERT_FILE")); env != "" {
		cfg.Ingest.Transport.TLS.CertFile = env
	}
	if env := strings.TrimSpace(os.Getenv("SRE_INGEST_TLS_KEY_FILE")); env != "" {
		cfg.Ingest.Transport.TLS.KeyFile = env
	}
	if env := strings.TrimSpace(os.Getenv("SRE_INGEST_ALLOWED_COLLECTOR_IDS")); env != "" {
		cfg.Ingest.Transport.AllowedCollectorIDs = parseCSV(env)
	}
	if env := strings.TrimSpace(os.Getenv("SRE_INGEST_ALLOWED_CLUSTER_NAMES")); env != "" {
		cfg.Ingest.Transport.AllowedClusterNames = parseCSV(env)
	}
	if env := strings.TrimSpace(os.Getenv("SRE_INGEST_ALLOWED_PEER_SUBJECTS")); env != "" {
		cfg.Ingest.Transport.AllowedPeerSubjects = parseCSV(env)
	}

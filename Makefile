...
LDFLAGS := -ldflags "-X github.com/jfang2048/ai_sre_agent_pub/internal/pkg/release.Version=$(VERSION) -X github.com/jfang2048/ai_sre_agent_pub/internal/pkg/release.Commit=$(COMMIT) -X github.com/jfang2048/ai_sre_agent_pub/internal/pkg/release.BuildDate=$(DATE)"
...
build-probe-core:
	...
	@if command -v pkg-config >/dev/null 2>&1 && pkg-config --exists protobuf zlib; then \
		...
		if [ "$${PROBE_CORE_REGENERATE_PROTO:-0}" = "1" ] && command -v protoc >/dev/null 2>&1; then \
			mkdir -p "$$GEN_DIR"; \
		...
		g++ -std=c++20 -O2 -Wall -Wextra -pthread \
			-DPROTOBUF_CONSTEXPR=constexpr \
			$$(pkg-config --cflags protobuf zlib) \
			-I"$$HEADER_DIR" \
			-o $(BUILD_DIR)/sre-probe-core \
			cpp/probe_core/main.cpp cpp/probe_core/subprocess.cpp cpp/probe_core/gpu_nvml.cpp \
			cpp/probe_core/kernel_event_protocol.cpp cpp/probe_core/network_kernel_collector.cpp \
			cpp/probe_core/procfs_fallback_collector.cpp cpp/probe_core/process_kernel_collector.cpp \
			"$$SRC_CC" \
...
## test-agent-workflow: Run the closed-loop workflow regression set (durability + APIs + fast eval)
test-agent-workflow:
	@mkdir -p $(GO_CACHE)
	@$(GO) -C backend test ./internal/controller/agentcore
	@$(GO) -C backend test ./internal/controller -run 'TestHandleAgentWorkflowRunsAndEvidence|TestHandleAgentWorkflowAudit|TestHandleAgentJointRiskAndRCAWorkflow|TestControllerHandlePrometheusMetricsIncludesWorkflowObservability'
	@$(GO) -C backend test ./internal/controller/eval -run '^TestGoldenEvaluationFast$$'

## test-agent-replay: Run deterministic replay stability checks over the golden incident fixtures
test-agent-replay:
	@mkdir -p $(GO_CACHE)
	@$(GO) -C backend test ./internal/controller/evaluation -run TestRunReplayFastIsStable
...
## harness-boundary-check: Run static agent harness architecture boundary checks
harness-boundary-check:
	@./scripts/check_agent_harness_boundaries.sh
...
verify-version:
	...
	@current="$$(tr -d '[:space:]' < $(VERSION_FILE))"; \
	chart_version="$${current#v}.0"; \
	grep -n -F "version: $$chart_version" deploy/charts/sre-agent/Chart.yaml >/dev/null || { \
		echo "Helm chart version drift: expected $$chart_version in deploy/charts/sre-agent/Chart.yaml"; \
		exit 1; \
	}; \
...
## gpu-platform-validate: Validate the GPU Platform SRE demo without requiring GPU/Kubernetes
gpu-platform-validate:
	@./examples/gpu-platform-sre/scripts/validate-all.sh

## gpu-platform-smoke: Run CPU-safe GPU Platform SRE smoke checks
gpu-platform-smoke:
	@./examples/gpu-platform-sre/scripts/run-smoke.sh --dry-run

## gpu-platform-evidence-template: Copy the incident recap template to build/
gpu-platform-evidence-template:
	@./examples/gpu-platform-sre/scripts/collect-evidence.sh --template-only

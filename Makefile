...
## test-ui: Run Chrome headless UI tests (requires running controller)
test-ui:
	@echo "Running Chrome headless UI tests..."
	@if command -v npm >/dev/null 2>&1; then \
		if [ ! -d "tests/ui/node_modules" ]; then \
			echo "Installing UI test dependencies..."; \
			npm -C tests/ui install; \
		fi; \
		npm -C tests/ui run test; \
	else \
		echo "npm not found; skipping UI tests"; \
	fi

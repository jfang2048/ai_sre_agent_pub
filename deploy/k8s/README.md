...
These assets map to the same controller/collector code paths described in `backend/cmd/controller/main.go`, `backend/cmd/collector/main.go`, and the deployment posture checks in `backend/internal/controller/deployment_posture.go`.
